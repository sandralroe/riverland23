...
CKEDITOR.editorConfig = function( config )
{
	...
	<cfif renderer.getheadline() eq "h1">
		...
		config.format_h4 = { element : '#renderer.getHeaderfTag('subHead5')#' };
	<cfelse>
	...
	*/
	...
	config.toolbar_Default = [
		...
		{name: 'group6', items:['Find','A11ychecker','Source','Maximize','ShowBlocks']}
	];
	...
	config.toolbar_QuickEdit = [
		...
		'/',
		{name: 'group5', items:['PasteText','PasteFromWord','-','Image','oembed','leaflet','-','Table','HorizontalRule','SpecialChar','-','Selectlink','SelectComponent','Templates'<cfif application.configBean.getEnableMuraTag()>,'muratag'</cfif>]},
		{name: 'group6', items:['Find','A11ychecker','Sourcedialog','Maximize','ShowBlocks']}
	];
	...
	config.toolbar_Summary = [
		...
		{name: 'group2', items: ['A11ychecker','Source','ShowBlocks']}
	];
	...
	config.toolbar_htmlEditor = [
		...
		{name: 'group5', items: ['A11ychecker','Source','ShowBlocks']}

	];


<!--- examples: verbose toolbar configs --->
<!---
		config.toolbar_Default = [
...
<cfoutput>
	...
	<!--- customConfig --->
		...
		<cfif isDefined("renderer.ckeditor")>
			<cfif isDefined("renderer.ckeditor.contentcss") and len(renderer.ckeditor.contentcss)>
				config.contentsCss.push('#renderer.ckeditor.contentcss#');
			</cfif>
			<cfif isDefined("renderer.ckeditor.templates") and len(renderer.ckeditor.templates)>
				config.templates='default';
				if(!Array.isArray(config.templates_files)){
					config.templates_files=[];
				}
				config.templates_files.push('#renderer.ckeditor.templates#');
			</cfif>
			<cfif isDefined("renderer.ckeditor.stylesset") and len(renderer.ckeditor.stylesset)>
				config.stylesSet='default:#renderer.ckeditor.stylesset#';
			</cfif>
			<cfif isDefined("renderer.ckeditor.customconfig") and len(renderer.ckeditor.customconfig)>
				config.customConfig='#renderer.ckeditor.customconfig#';
			</cfif>
		</cfif>
