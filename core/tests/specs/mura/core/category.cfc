...
/*********************************** LIFE CYCLE Methods ***********************************/
	...
	function afterAll(){
		if(isDefined('bean.exists') && bean.exists()){
			bean.delete();
		}
	...
	function run(){
		...
		describe("Testing persistence", function() {
			...
			config={
				name="My Unit Test",
				siteID="default",
				categoryID="0655DC32-C8FE-494D-98FC593D386A88XC",
				remoteID="remoteID",
			...
			it(
				title="Should have exists method",
			 	body=function() {
				 	expect(isDefined('bean.exists') ).toBeTrue();
				},
				bean=bean
			);
					...
					title="Should be able to load by #key#",
			 		body=function() {
						bean=$.getBean(entityName).loadBy(argumentCollection=args);
					 	expect( isArray(bean) || bean.exists() ).toBeTrue();
					},
