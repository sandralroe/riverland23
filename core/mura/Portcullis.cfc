<cfcomponent output="false" hint="This provides a utility to prevent injection attacks">
	...
	<cfif isdefined('application.Mura')>
		<cfset variables.instance.thisServer = lcase(application.Mura.getBean('utility').getRequestHost())/>
	<cfelse>
	...
	<cffunction name="stripMSWord" output="false">
		...
		<cfif isDefined('arguments.value') and not isNull(arguments.value)>
	...
	<cffunction name="isSafeReferer" output="false">
		<cfif isdefined('application.Mura')>
			<cfset var thisserver = lcase(application.Mura.getBean('utility').getRequestHost())/>
		<cfelse>
