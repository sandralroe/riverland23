<cfsilent>
		...
		<cfparam name="attributes.maxBase64StringLength" default="64000">
		...
		<cfset maxBase64=application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,'sitemanager.requestmaxbase64'),attributes.maxBase64StringLength/1000)/>
		<cfset invalidBase64=application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,'sitemanager.requesthasinvalidbase64'),attributes.maxBase64StringLength/1000)/>
					...
					<script>
						...
						$(document).on('change', '##mura-fileselector-#esapiEncode('html_attr',guid)# .mura-file-selector-#esapiEncode('html_attr',attributes.name)#', function(){
							...
							// url input
								...
								if (imageUrl.indexOf("data:image/") >= 0 ){
									...
									let maxStringSizeAllowed = #val(attributes.maxBase64StringLength)#;
									if (imageUrl.length > maxStringSizeAllowed){
										alert("#maxBase64#");
									...
									//load image 
									$(prevImage).handlers().preview(true)
										.on('load error', function(){$(this).handlers().preview()});
									$(nextImage).handlers().loading(true).attr('src',imageUrl)
										.on('load', function(){$(this).handlers().loading(false)})
										.on("error",function(){
											alert("#invalidBase64#"); 
											inputEl.val('').change(); 
											$(this).handlers().loading(false)
										});
									<cfif attributes.property eq 'fileid'>
										$(imagePreview).handlers().preview(true)
											.on('load error', function(){$(this).handlers().preview();})
										$(imagePreview).handlers().loading(true).attr('src',imageUrl)
											.on('load', function(){$(this).handlers().loading(false)})
									</cfif>
									inputEl.prop("disabled",false)
						...
						$(document).on('click', 'img##mura-file-ph-#esapiEncode('html_attr',attributes.name)#',function(){
						 ...
						 $(document).on('click', '.mura-file-selector-tabs .mura-file-type-selector', function(){
							 $(this).parents('.mura-file-selector').find('.assocFileTools').show();
		...
		<div id="mura-file-url-#esapiEncode('html_attr',attributes.name)#" class="mura-file-option mura-file-url fileTypeOption#attributes.name#">
			<div class="mura-control-group">
				<div class="mura-input-set assocUrlControls">
					<input type="text" name="#esapiEncode('html_attr',attributes.name)#" class="mura-file-selector-#attributes.name# mura-url-input-#attributes.name#" type="url" placeholder="http://www.domain.com/myfile.#attributes.examplefileext#"	value=""
