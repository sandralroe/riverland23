...
<cfcomponent extends="mura.baseobject" output="false" hint="This provides global utility methods">
...
<cffunction name="getRequestHost" output="false">
	...
	<cfelseif StructKeyExists(headers,"Host") and len(headers["Host"])>
		<cfreturn listFirst(headers["Host"],":")>
...
<cffunction name="getRequestRefererHost" output="false">
	<cfset var headers=getHttpRequestData().headers>

	<cfif StructKeyExists(headers,"Referer") and isValid('url',headers["Referer"])>
		<cfreturn listFirst(createObject("java","java.net.URL").init(headers["Referer"]).getHost(),":")>
	<cfelseif len(cgi.http_referer)>
		<cfreturn listFirst(cgi.http_referer,":")>
	<cfelse>
		<cfreturn "">
	</cfif>
...
<cffunction name="getRequestPort" output="false">
	<cfset var headers=getHttpRequestData().headers>

	<cfif isDefined('arguments.defaultValue')>
		<cfset var port=arguments.defaultValue>
	<cfelse>
		<cfset var port=getBean("configBean").getServerPort()>
	</cfif>

	<cfif StructKeyExists(headers,"X-Forwarded-Port") and len(headers["X-Forwarded-Port"])>
		<cfif listFind("443,80",headers["X-Forwarded-Port"])>
			<cfset port="">
		<cfelse>
			<cfset port=":" & headers["X-Forwarded-Port"]>	
		</cfif>
	<cfelseif StructKeyExists(headers,"X-Forwarded-Host")>
		<cfif listLen(headers["X-Forwarded-Host"],":") gt 1>
			<cfset port=listLast(headers["X-Forwarded-Host"],":")>
			<cfif listFind("443,80",port)>
				<cfset port="">
			<cfelse>
				<cfset port=":" & port>	
			</cfif>
...
<cffunction name="cfformprotect" output="false">
	...
	<cfset var cffp=new cfformprotect.cffpVerify()>
			...
			<cfif isDefined('sessionTokens.cfid') and (isBoolean(arguments.reset) and arguments.reset or not isDefined('cookie.cfid'))>
				...
				<cfif server.coldfusion.productname neq 'Coldfusion Server'>
					<cfif application.configBean.getSessionCookiesExpires() EQ "" OR application.configBean.getSessionCookiesExpires() EQ "session" OR application.configBean.getSessionCookiesExpires() EQ "session only">
						<cfset setCookieLegacy(name='cfid', value=sessionTokens.CFID, preserveCase=true)>
						<cfset setCookieLegacy(name='cftoken', value=sessionTokens.CFTOKEN, preserveCase=true)>
					<cfelse>
						<cfset setCookieLegacy(name='cfid', value=sessionTokens.CFID, expires=application.configBean.getSessionCookiesExpires(), preserveCase=true, encodevalue=false)>
						<cfset setCookieLegacy(name='cftoken', value=sessionTokens.CFTOKEN, expires=application.configBean.getSessionCookiesExpires(), preserveCase=true, encodevalue=false)>
					</cfif>
				<cfelse>
					<cfif application.configBean.getSessionCookiesExpires() EQ "" OR application.configBean.getSessionCookiesExpires() EQ "session" OR application.configBean.getSessionCookiesExpires() EQ "session only">
...
<cffunction name="deleteCookie" output="false">
	...
	<cfargument name="preserveCase" type="boolean" default="true">
...
<cffunction name="getCookieDomain" output="false">
	<cfset var domain=variables.configBean.getCookieDomain()>
	<cfif domain eq "*">
		<cfset var requestHost=getRequestHost()>
		<cfif listLen(requestHost,".") gt 2>
			<cfreturn "." & listRest(requestHost,".")>
		<cfelse>
			<cfreturn requestHost>
		</cfif>
	<cfelse>
		<cfreturn domain>
	</cfif>
...
<cffunction name="getCookieDomain" output="false">
	...
	<cfif domain eq "*">
		<cfset var requestHost=getRequestHost()>
		<cfset hostArray=listToArray(requestHost,'.')>
		<cfif arrayLen(hostArray) gt 2>
			<cfreturn "." & hostArray[arrayLen(hostArray)-1] & "." &  hostArray[arrayLen(hostArray)]>
		<cfelse>
			<cfreturn requestHost>
		</cfif>
	<cfelse>
...
<cffunction name="setCookie" output="false">
	...
	<cfargument name="samesite" type="string" default="">

	<cfif variables.configBean.getSecureCookies()>
		<cfset arguments.secure=true>
	</cfif>

	<cfif not len(arguments.samesite)>
		<cfset arguments.samesite= variables.configBean.getValue(property="cookieSameSite",defaultValue="None")>
	</cfif>

	<cfif len(variables.configBean.getCookieDomain())>
		<cfset arguments.domain=getCookieDomain()>
	</cfif>
  
	<cfif isDefined('arguments.cookiedomain')>
	  <cfif len(variables.configBean.getCookiePath()) and len(arguments.cookiedomain)>
		  <cfset arguments.path=variables.configBean.getCookiePath()>
	  <cfelseif len(arguments.cookiedomain) and variables.configBean.getCompiler() eq 'Lucee'>
		  <cfset arguments.path="/">
	  </cfif>
	</cfif>

	<cfset arguments.preserveCase=arguments.preserveCase>

	<cfif not (
			isDate(arguments.expires)
			or isNumeric(arguments.expires)
			or listFindNoCase('now,never',arguments.expires)
		)>
...
<cffunction name="setCookieLegacy" hint="Replacement for cfcookie that handles httponly cookies" output="false" returntype="void">
	...
	<cfargument name="samesite" type="string" default="">
	<cfargument name="encodevalue" type="boolean" default="false">

	<cfset var responseObject=getpagecontext().getResponse()>
	<cfset responseObject.setHeader( 'Cache-Control', 'no-cache="Set-Cookie"' )>
	
	<cfif isDefined('arguments.encode') and isBoolean(arguments.encode)>
		<cfset arguments.encodevalue=arguments.encode>
	</cfif>
	...
	<cfif arguments.encodevalue>
		<cfset arguments.value=URLEncodedFormat(arguments.value)>
	</cfif>

	<cfif arguments.preserveCase>
		<cfset c = "#arguments.name#=#arguments.value#;">
	<cfelse>
		<cfset c = "#UCase(arguments.name)#=#arguments.value#;">
	</cfif>

    <cfswitch expression="#Arguments.expires#">
        <cfcase value="Session Only">
        </cfcase>
        <cfcase value="now">
    ...
    <cfif Arguments.secure>
		<cfset c = c & "Secure;SameSite=#Arguments.samesite#;">
    </cfif>
	...
	<cfif not listFindNocase('jsessionid,cfid,cftoken',arguments.name)>
		<cfif not len(arguments.expires) 
			and not (
				listFindNoCase("now,never",arguments.expires)
				or isDate(arguments.expires)
				or isNumeric(arguments.expires)
			)>			
			<cfset structDelete(arguments,'expires')>
		</cfif>
...
<cfscript>
	...
	function isPathLegal(path){
		...
		var filePath=replace(variables.configBean.getFileDir(), "\", "/", "ALL");
		...
		return (
			...
			|| len(arguments.path) >= len(filePath) && left(arguments.path,len(filePath)) == filePath
	...
	function excludeFromClientCache(){
		var responseObject=getpagecontext().getResponse();
		responseObject.setHeader( 'Cache-Control', 'no-cache, no-store' );
		responseObject.setHeader( 'Expires', 'Mon, 01 Jan 1990 00:00:00 GMT');
		responseObject.setHeader( 'Pragma', 'no-cache' );
	}

	function checkOutputCacheState(event){
		var sessionData=application.Mura.getSession();

		param name="request.muraOutputCacheOffsetAdjusted" default=false;
		
		if(!isDefined('arguments.event')){
			if(isDefined('request.servletEvent')){
				arguments.event=request.servletEvent;
			} else {
				arguments.event=new mura.event();
			}
		}

		if(request.muraSessionManagement && request.muraOutputCacheOffsetAdjusted){

			sessionData.mura.outputCacheOffset=request.muraOutputCacheOffset;

			if(
				!isDefined('cookie.MURA_OCS') && len(sessionData.mura.outputCacheOffset)
				|| isDefined('cookie.MURA_OCS') && cookie.MURA_OCS!= sessionData.mura.outputCacheOffset
			)	
			{
				if(len(sessionData.mura.outputCacheOffset)){
					setCookie(name='MURA_OCS',value=sessionData.mura.outputCacheOffset,expires="session only");
				} else {
					deleteCookie('MURA_OCS');
				}
				excludeFromClientCache();
			}
		}

		if(isDefined('cookie.MURA_UPC') && !(isDefined('sessionData.mura.isLoggedIn') && sessionData.mura.isLoggedIn)){
			deleteCookie('MURA_UPC');
		}
	}
...
<cffunction name="setPreviewDomain" output="false">
	<cfscript>
		...
		if(!isDefined('request.muraPreviewDomain') || !len(request.muraPreviewDomain)){
			request.muraPreviewDomain=application.Mura.getBean('utility').getRequestHost();
		}
		var site=getBean('settingsManager').getSite(sessionData.siteid);
		if(!site.isValidDomain(domain=request.muraPreviewDomain,mode='complete')){
			request.muraPreviewDomain=site.getDomain();
		}
	</cfscript>
</cffunction>
