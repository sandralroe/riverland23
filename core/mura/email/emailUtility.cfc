			...
			<cfif rsemail.grouplist neq '' and (rsemail.bodyhtml neq '' or rsemail.bodytext neq '')>
				...
				<cfloop query="rsAddresses">
					<cfif REFindNoCase("^[^@%*<>' ]+@[^@%*<>' ]{1,255}\.[^@%*<>' ]{2,5}", trim(rsAddresses.email)) neq 0 and prevEmail neq rsAddresses.email>
						...
						<cfset trackOpen='<img src="#site.getResourcePath(complete=1)##application.configBean.getIndexPath()#/_api/email/trackopen/?email=#rsaddresses.email#&emailid=#rsemail.emailid#" style="display:none;">'/>
	...
	<cffunction name="forward" output="false">
		...
		<cfloop list="#arguments.data.to#" index="t">
			...
			<cfif REFindNoCase("^[^@%*<>' ]+@[^@%*<>' ]{1,255}\.[^@%*<>' ]{2,5}", trim(t)) neq 0>
				...
				<cfset trackOpen='<img src="#site.getResourcePath(complete=1)##application.configBean.getIndexPath()#/_api/email/trackopen/?email=#t#&emailid=#rsemail.emailid#" style="display:none;">'/>
	...
	<cffunction name="track" output="false">
		...
		<cfif arguments.type eq "returnClick">
			<cfif isDefined('url.path')>
				<cfset returnUrl = "http://#domain##url.path#">
			<cfelse>
				<cfset returnUrl = "http://#domain##cgi.script_name#">
			</cfif>
