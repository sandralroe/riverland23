...
License goes here
--->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides user gateway queries">
	...
	<cffunction name="getSearch" output="false">
		...
		<cfset var tuserType=arguments.isGroupSearch?1:2/>
		...
		where tusers.type=<cfqueryparam cfsqltype="cf_sql_numeric" value="#tuserType#">  and tusers.isPublic = <cfqueryparam cfsqltype="cf_sql_numeric" value="#arguments.isPublic#"> and
		tusers.siteid = <cfif arguments.isPublic eq 0 >
			'#variables.settingsManager.getSite(arguments.siteid).getPrivateUserPoolID()#'
			<cfelse>
			'#variables.settingsManager.getSite(arguments.siteid).getPublicUserPoolID()#'
		...
		--->
		<cfif arguments.isGroupSearch>
			and (tusers.groupname like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.search#%">)
		<cfelse>
		 and (
		...
		<cfif rsParams.recordcount>
			...
			<cfloop query="rsParams">
				...
				<cfif param.getIsValid() and param.getField() neq 'tusers.siteid'>
					<cfset var isNullVal = param.getCriteria() eq 'null' ? true : false />
	...
	<cffunction name="getAllGroups" output="false">
		<cfargument name="siteid" type="string" default="" />
		<cfset var rsGroups = "" />
		...
		LEFT JOIN tfiles on tusers.photofileID=tfiles.fileID
		where (
			tusers.siteID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#variables.settingsManager.getSite(arguments.siteid).getPublicUserPoolID()#"> and tusers.isPublic=1
			or tusers.siteID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#variables.settingsManager.getSite(arguments.siteid).getPrivateUserPoolID()#"> and tusers.isPublic = 0
		) and tusers.type=1
		ORDER BY tusers.GroupName
		</cfquery>
		<cfreturn rsGroups />
