<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provide user specific utility methods">
...
<cffunction name="lookupByCredentials"  output="false">
	...
	<cfset var strikes = new mura.user.userstrikes(arguments.username,variables.configBean) />
...
<cffunction name="login" returntype="boolean">
	...
	<cfif rsUser.RecordCount GREATER THAN 0
		and not strikes.isBlocked()>
			...
			<cfelse>
				<cfif variables.configBean.getValue(property="sessionBasedLockdown",defaultValue=true)>
...
<cffunction name="loginByQuery">
	...
	<cfif request.muraSessionManagement>
		<cfif getBean('permUtility').getModulePerm("00000000000000000000000000000000000",rsuser.siteid)>
			<cfset getBean('utility').setCookie(name='MURA_UPC',value="false",expires="session only")>
		<cfelseif isDefined('cookie.MURA_UPC')>
			<cfset getBean('utility').deleteCookie(name="MURA_UPC")>
		</cfif>
...
<cffunction name="sendLoginByEmail" output="false" >
	<cfargument name="email" type="string">
	<cfargument name="siteid" type="string" required="yes" default="">
	<cfargument name="returnURL" type="string" required="yes" default="">
	<cfargument name="subject" required="yes" type="string" default=""/>
	<cfargument name="message" type="string" default="">

	<cfif not len(arguments.returnURL)>
		<cfset arguments.returnURL="#getBean('utility').getRequestHost()##cgi.SCRIPT_NAME#">
	</cfif>
...
<cfif arguments.siteid neq ''>
	...
	<cfset urlBase="#domain##site.getServerPort()##site.getContext()#">
	...
	<cfelse>
		<cfset editProfileURL=site.getWebPath(complete=1) & site.getEditProfileURL()>
	</cfif>

	<cfset returnURL="#protocol##urlBase##site.getContentRenderer().getURLStem(site.getSiteID(),returnID)#?userID=#arguments.args.userID#">
<cfelse>
	<cfset urlBase="#domain##variables.configBean.getServerPort()##variables.configBean.getContext()#">
...
<cfif not len(arguments.siteID)>
	...
	<cfset editProfileURL=editProfileURL & "&siteID=#esapiEncode('url',arguments.args.siteID)#">
</cfif>

<!--- add extra attributes --->
<cfset editProfileURL=editProfileURL & "&returnID=#esapiEncode('url',returnID)#&returnUserID=#esapiEncode('url',arguments.args.userID)#">
...
Thank you for using #contactName#!</cfoutput>
</cfsavecontent>
...
<cfargument name="$">
	...
	<cfif len(arguments.$.event('returnID')) && len(arguments.$.event('returnUserID')) 
		and not (arguments.$.currentUser().isLoggedIn() and arguments.$.currentUser('userid') eq redirect.getUserId())>
		...
		<cfif redirect.exists() and redirect.getCreated() GTE dateAdd("d",-variables.configBean.get('mfadayslinkvalid'),now()) and arguments.$.event('returnUserID') eq redirect.getUserID() >
			...
			<cfif user.exists()>
				...
				<cfelse>
					<cfset userUtility.loginByUserID(userid=user.getUserID(),siteid=$.event('siteid')) />
				</cfif>
			</cfif>
		...
		<cfelse>
			...
			<cfset $.redirect(location='/?display=login&linkinvalid=true',addToken=false,statusCode='302' ) />
		</cfif>
	</cfif>
...
<cffunction name="removePrevRedirects" output="false" hint="Used to remove previous magic link redirects.">
	...
	<!--- see if the user requesting a redirect has a record already --->
	<cfset var redirectBean = getBean('userRedirect') />
	<!--- create a feed using the userid --->
	<cfset var redirectChecker = getFeed('userRedirect').where().prop('userid').isEQ(arguments.userid).getIterator() />
	<!--- loop feed to check for existing --->
	<cfloop condition="#redirectChecker.hasNext()#" >
		<cfset var usersRedirects = redirectChecker.next() />
...
<cffunction name="removeOldRedirects" output="false" hint="Used to remove previous magic link redirects.">

	<cfargument name="userid">
	<!--- see if the user requesting a redirect has a record already --->
	<cfset var redirectBean = getBean('userRedirect') />
	<!--- create a feed using the userid --->
	<cfset var redirectChecker = getFeed('userRedirect').where()
		.prop('userid').isEQ(arguments.userid)
		.andProp('created').isLT(dateAdd('d',-30,now()))
		.getIterator() />
