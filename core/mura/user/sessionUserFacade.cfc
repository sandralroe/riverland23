 ...
 */
component extends="mura.baseobject" public="true" entityName="currentUser" output="false" hint="This provides access to the current user's session" {
	variables.userBean="";
	...
	public boolean function isAdminUser() output=false {
		if ( hasSession() ) {
			return (isInGroup('Admin',0) || isSuperUser());
		} else {
			return false;
		}
	}
	...
	function getBean(beanName){
		return getServiceFactory().getBean(arguments.beanName);
	}

	remote function approvals(siteid){
		var Mura=getBean('Mura').init(arguments.siteid);
		var rs=Mura.getBean("contentManager").getApprovalsQuery(arguments.siteid);
		var loadedMemberLists={};
		var memberlistsArray=[];
		for(var row in rs){
			if(!structKeyExists(loadedMemberLists,'#row.chainid#')){
				var members=Mura.getFeed('approvalChainMembership').where().prop('chainid').isEQ(row.chainid).getQuery();
				loadedMemberLists['#row.chainid#']=valueList(members.groupid);
			}
			arrayAppend(memberlistsArray,loadedMemberLists['#row.chainid#']);
		}
		QueryAddColumn(rs, "grouplist", "varchar", memberlistsArray);
		var iterator=getBean('contentIterator').setQuery(rs);
		return Mura.siteConfig().getAPI().packageIteratorArray(iterator,iterator.getArray(),'approvals');
	}

	remote function submissions(siteid){
		var Mura=getBean('Mura').init(arguments.siteid);
		var rs=getBean("contentManager").getSubmissionsQuery(argumentCollection=arguments);
		var loadedMemberLists={};
		var memberlistsArray=[];
		for(var row in rs){
			if(!structKeyExists(loadedMemberLists,'#row.chainid#')){
		...
		QueryAddColumn(rs, "grouplist", "varchar", memberlistsArray);
		var iterator=getBean('contentIterator').setQuery(rs);
		return Mura.siteConfig().getAPI().packageIteratorArray(iterator,iterator.getArray(),'submissions');
	}

	remote function expiring(siteid){
		var Mura=getBean('Mura').init(arguments.siteid);
		
		arguments.userid=getValue('userid');

		var rs=getBean("contentManager").getExpiringContent(argumentCollection=arguments);
		var iterator=getBean('contentIterator').setQuery(rs);
		return Mura.siteConfig().getAPI().packageIteratorArray(iterator,iterator.getArray(),'expiring');
	}

	remote function workflowStatus(siteid){
		var contentManager=getBean("contentManager");
	
		return {
			data:{
				submissions:contentManager.getMySubmissionsCount(argumentCollection=arguments),
				approvals:contentManager.getMySubmissionsCount(argumentCollection=arguments),
				expiring:contentManager.getMyExpiresCount(argumentCollection=arguments)
			}
		};
	}
