<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides plugin service level logic functionality">
	...
	<cffunction name="init" output="false">
		...
		<cfset application.eventManager=this>
	...
	<cffunction name="loadPlugins" output="false">
		...
		<cfset var missingDirectoryModuleIds=''>
		...
		<cflock name="loadPluginsBlock#application.instanceID#" type="exclusive" timeout="200">
			...
			<cfloop query="variables.rsplugins">
				<cfif not directoryExists(getLocation(variables.rsplugins.directory))>
					<cfset missingDirectoryModuleIds=listAppend(missingDirectoryModuleIds,variables.rsplugins.moduleid)>
				</cfif>
			</cfloop>
			...
			<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='variables.rsPluginSiteAsignments')#">
				...
				<cfif len(missingDirectoryModuleIds)>
					and moduleid not in (<cfqueryparam cfsqltype="cf_sql_varchar" list="true" value="#missingDirectoryModuleIds#">)
				</cfif>
			</cfquery>
			...
			<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsScripts1')#">
				...
				<cfif len(missingDirectoryModuleIds)>
					and tplugins.moduleid not in (<cfqueryparam cfsqltype="cf_sql_varchar" list="true" value="#missingDirectoryModuleIds#">)
				</cfif>
			</cfquery>
			...
			<cfif not variables.eventHandlersLoaded or len(arguments.siteid)>
				<cfloop query="variables.rsScripts">
					...
					<cfif left(variables.rsScripts.runat,8) neq "onGlobal" and variables.rsScripts.runat neq "onApplicationLoad">
						<cfif not len(arguments.siteid) or arguments.siteid eq variables.rsScripts.siteID>
					...
					<cfelseif not len(arguments.siteid) and not variables.eventHandlersLoaded>
			...
			<cfset purgeStandardEventFactories(arguments.siteid)/>
			<cfset purgeCacheFactories(arguments.siteid)/>
			<cfset purgePluginConfigs(arguments.siteid)/>
		</cflock>
	</cffunction>
	...
	<cffunction name="deploy" output="false" hint="This method is primarily used internally by Mura. See deployPlugin." >
		...
		<cfargument name="id" required="true" default="" hint="Either ModuleID, PluginID or Package. Can be used instead of moduleID arguments.">
	...
	<cffunction name="discover" output="false">
		...
		<cflock name="pluginDiscovery#application.instanceID#" type="exclusive" timeout="200">
			...
			<cfloop query="rsRequirements">
				...
				<cfif rsRequirements.type eq "dir" and rsRequirements.name neq '.svn' and rsRequirements.name neq '.git'>
				...
				<cfelseif listLast(rsRequirements.name,".") eq 'zip'>
					...
					<cfelse>
						<cfset item="#baseDir#/#rsRequirements.name#">
						<cfset rsCheckDiscoveredPlugin=new mura.Zip().list(item)>
	...
	<cffunction name="getConfig" output="false">
		...
		<cfif not arguments.cache or not structKeyExists(variables.pluginConfigs,"p#rs.pluginID#")>
			<cfset pluginConfig=new mura.plugin.pluginConfig()>
	...
	<cffunction name="updateSettings" output="false">
		...
		<cfif not isDefined("arguments.args.autoDeploy") or arguments.args.autoDeploy>
			...
			<cfif fileExists(variables.configBean.getPluginDir() & "/" & pluginConfig.getDirectory() & "/plugin/plugin.cfc")>
				<cfset pluginCFC= new "plugins.#pluginConfig.getDirectory()#.plugin.plugin"(pluginConfig) />
			...
			<cftry>
				<cfif not isObject(arguments.event)>
					<cfif isStruct(arguments.event)>
						<cfset arguments.event=new mura.event(arguments.event)/>
					<cfelse>
						<cfif structKeyExists(request,"servletEvent")>
							<cfset arguments.event=request.servletEvent />
						<cfelse>
							<cfset arguments.event=new mura.event()/>
						</cfif>
					</cfif>
				</cfif>
				...
				<cfset muraScope.event('_eventName',arguments.runat)>
					...
					<cfif len(arguments.objectid)
							...
							<cfif arrayLen(listenerArray)>
								...
								<cfelse>
									...
									<cfif eventHandler.getValue('entityName') eq 'proxy'>
										<cfset eventHandler.handleEvent(muraScope)>
									<cfelse>
	...
	<cffunction name="getComponent" output="false">
		...
		<cfif isBoolean(arguments.cache) and arguments.cache>
			<cfif NOT getCacheFactory(arguments.siteid).has( componentPath ) or variables.configBean.getMode() eq "development" or variables.configBean.getMode() eq "archived">
				<cfif arguments.pluginID>
					<cfset pluginConfig=getConfig(arguments.pluginID)>
					<cfreturn getCacheFactory(arguments.siteid).get( componentPath, new "#componentPath#"(pluginConfig,configBean) ) />
				<cfelse>
					<cfreturn getCacheFactory(arguments.siteid).get( componentPath, new "#componentPath#"(configBean) ) />
				</cfif>
			<cfelse>
		...
		<cfelse>
			<cfset pluginConfig=getConfig(arguments.pluginID)>
			<cfreturn new "#componentPath#"(pluginConfig,configBean) />
		</cfif>
	</cffunction>

	<cffunction name="getExecutor" output="false">
		<cfset var executor=new pluginExecutor(variables.configBean,variables.settingsManager,this) />
		<cfreturn executor />
	</cffunction>
	...
	<cffunction name="purgeStandardEventFactories" output="false">
		<cfargument name="siteid" required="true" default="">
		<cfif len(arguments.siteid)>
			<cfset structDelete(variables.standardEventFactories,arguments.siteid)>
		<cfelse>
			<cfset variables.standardEventFactories=structNew()/>
		</cfif>
	</cffunction>
	...
	<cffunction name="getStandardEventFactory" output="false">
		...
		<cfif not structKeyExists(variables.standardEventFactories,arguments.siteid)>
			<cfset variables.standardEventFactories[arguments.siteid]=new pluginStandardEventFactory(arguments.siteID,variables.standardEventsHandler,this)>
		</cfif>
	...
	<cffunction name="getCacheFactory" output="false">
		...
		<cfif not structKeyExists(variables.cacheFactories,arguments.siteid)>
			<cfset variables.cacheFactories[arguments.siteid]=new mura.cache.cacheFactory(isSoft=false)>
		</cfif>
	...
	<cffunction name="purgeCacheFactories" output="false">
		<cfargument name="siteid" required="true" default="">
		<cfif len(arguments.siteid)>
			<cfset structDelete(variables.cacheFactories,arguments.siteid)>
		<cfelse>
			<cfset variables.cacheFactories=structNew()/>
		</cfif>
	</cffunction>
	...
	<cfscript>	
		function resetSiteEvents(siteid){
			pruneSiteEvents(siteid=arguments.siteid);
			loadPlugins(siteid=arguments.siteid);
		}

		function rerunModuleBasedApplicationLoad(){
			var $=getBean('$').init();

			executeScripts(runat='onApplicationLoad',event= $.event());

			for(var handlerKey in application.appHandlerLookUp){
				var handler=application.appHandlerLookUp['#handlerKey#'];
		...
		function pruneSiteEvents(siteid){
			cfparam( default=structNew(), name="variables.handlerLookup" );

			var siteIDadjusted=adjustSiteID(arguments.siteID);

			for(var key in variables.handlerLookup){
				var handlerData=variables.handlerLookup['#key#'];
				structDelete(handlerData.appliedSites,siteIDadjusted);
			}

			structDelete(variables.siteListeners,siteIDadjusted);
		}

		function addEventHandler(component, required siteID, required persist="true", required applyglobal="true", required objectid="") output=false {
			lock name="addEventHandler#application.instanceID#" type="exclusive" timeout="200" {
				var i = "";
				var eventHandlerData=structNew();
				var handlerData=structNew();
				var eventhandler=arguments.component;
				var siteIDadjusted=adjustSiteID(arguments.siteID);
				var isProxy=false;
				var handlerIndex=0;
				...
				if ( structKeyExists(arguments.component,"getValue") 
						and arguments.component.getValue('entityName') == 'proxy' ) {
					var proxyEvents=arguments.component.getEvents();
					
					if ( proxyEvents.hasNext() ) {
						while (proxyEvents.hasNext()) {
							proxyEvent=proxyEvents.next();
							if ( isValid('variableName',proxyEvent.getName()) ) {
								arguments.component[proxyEvent.getName()]=true;
							}
						}
					}
				...
				if ( len(siteIDadjusted) && !StructKeyExists(variables.siteListeners,siteIDadjusted) ) {
					variables.siteListeners[siteIDadjusted]=structNew();
				}
				if ( !(isObject(eventHandler) || isStruct(eventHandler)) ) {
					eventhandler=getEventHandlerFromPath(eventHandler);
				}

				if(isProxy){
					eventHandler.metadata={
						name='proxy-' & arguments.component.get('proxyid')
					};
				} else {
					eventHandler.metadata=duplicate(getMetaData(eventhandler));
				}

				if ( !structKeyExists(variables.handlerLookup,'#eventHandler.metadata.name#') ) {
					arrayAppend(variables.eventHandlers,eventhandler);
					variables.handlerLookup['#eventHandler.metadata.name#']={
							index=arrayLen(variables.eventHandlers),
							appliedSites={}
						};
				}
				handlerData=variables.handlerLookup['#eventHandler.metadata.name#'];
				handlerIndex=handlerData.index

				if(!structKeyExists(handlerData.appliedSites,'#siteIDadjusted#')){
					if ( !isDefined('arguments.component.injectMethod') ) {
						arguments.component.injectMethod=injectMethod;
					}
					if ( !isDefined('eventhandler.getValue') ) {
						eventHandler.injectMethod('getValue',getValue);
					}
					if ( !isDefined('eventhandler.setValue') ) {
						eventHandler.injectMethod('setValue',setValue);
					}

					eventhandler.setValue('_objectName',eventhandler.metadata.name);

					for ( i in eventhandler ) {
						if ( left(i,2) == "on" || left(i,8) == "standard" ) {
							eventHandlerData=structNew();
							eventHandlerData.index=handlerIndex;
							if ( isDefined("eventHandler.pluginName") ) {
								eventHandlerData.pluginName=eventHandler.pluginName;
							} else if ( isDefined("eventHandler.getPluginName") ) {
								eventHandlerData.pluginName=eventHandler.getPluginName();
							} else if ( len(eventHandler.getValue('pluginName')) ) {
								eventHandlerData.pluginName=eventHandler.getValue('pluginName');
							} else if ( isDefined("eventHandler.moduleName") ) {
								eventHandlerData.pluginName=eventHandler.moduleName;
							} else if ( isDefined("eventHandler.getModuleName") ) {
								eventHandlerData.pluginName=eventHandler.getModuleName();
							} else if ( len(eventHandler.getValue('moduleName')) ) {
								eventHandlerData.pluginName=eventHandler.getValue('moduleName');
							} else {
								eventHandlerData.pluginName='undefined';
							}
							if ( len(arguments.objectid) ) {
								if ( len(siteIDadjusted) ) {
									if ( !structKeyExists(variables.siteListeners[siteIDadjusted],'objects') ) {
										variables.siteListeners[siteIDadjusted].objects={};
									}
									if ( !structKeyExists(variables.siteListeners[siteIDadjusted].objects,'#arguments.objectid#') ) {
										variables.siteListeners[siteIDadjusted].objects['#arguments.objectid#']={};
									}
									if ( !structKeyExists(variables.siteListeners[siteIDadjusted].objects['#arguments.objectid#'],i) ) {
										variables.siteListeners[siteIDadjusted].objects['#arguments.objectid#'][i]=arrayNew(1);
									}
									arrayAppend( variables.siteListeners[siteIDadjusted].objects['#arguments.objectid#'][i] , eventHandlerData);
								} else {
									if ( !structKeyExists(variables.globalListeners,'objects') ) {
										variables.globalListeners.objects={};
									}
									if ( !structKeyExists(variables.globalListeners.objects,'#arguments.objectid#') ) {
										variables.globalListeners.objects['#arguments.objectid#']={};
									}
									if ( !structKeyExists(variables.globalListeners.objects['#arguments.objectid#'],i) ) {
										variables.globalListeners.objects['#arguments.objectid#'][i]=arrayNew(1);
									}
									arrayAppend( variables.globalListeners.objects['#arguments.objectid#'][i] , eventHandlerData);
								}
							} else if ( len(arguments.siteid) && !findNoCase('global',i) ) {
								if ( !structKeyExists(variables.siteListeners[siteIDadjusted],i) ) {
									variables.siteListeners[siteIDadjusted][i]=arrayNew(1);
								}
								arrayAppend( variables.siteListeners[siteIDadjusted][i] , eventHandlerData);
							} else if ( arguments.applyglobal ) {
								if ( !structKeyExists(variables.globalListeners,i) ) {
									variables.globalListeners[i]=arrayNew(1);
								}
								arrayAppend( variables.globalListeners[i] , eventHandlerData);
							}
						}
					}
					

					handlerData.appliedSites['#siteIDadjusted#']=true;
	...
	<cffunction name="getEventHandlerFromPath" output="false">
		...
		<cfreturn new "#arguments.component#"()>
	</cffunction>
