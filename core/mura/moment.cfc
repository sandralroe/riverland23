...
component displayname="moment" {
	...
	*/
	...
	public function getZoneCurrentOffset( required string zone ) hint="returns the offset in seconds (considering DST) of the specified zone" {
		return getArbitraryTimeOffset(time=now(),zone=arguments.zone);
	}
	...
	public struct function getZoneTable(){
		var list = createObject('java', 'java.time.ZoneId').getAvailableZoneIds();
		var data = {};
		for (tz in list){
			//display *CURRENT* offsets
			var ms = getArbitraryTimeOffset(time=now(),zone=tz) * 1000;
	...
	public function getArbitraryTimeOffset( required time, required string zone ) hint="returns what the offset was at that specific moment"{
		...
		var epochTime=javacast('long', dateDiff('s', epic, arguments.time)*1000)
		var instant=createObject('java','java.time.Instant').ofEpochSecond(epochTime);
		var seconds = createObject('java','java.time.ZonedDateTime').ofInstant(instant,zoneId).getOffset().getTotalSeconds();
		return seconds;
	}
	...
	public numeric function getOffset(time=now(),zone=getZone()) hint="returns the offset in seconds (considering DST) of the current moment" {
		return getArbitraryTimeOffset( arguments.time, arguments.zone );
	}
