component extends="mura.baseobject" hint="This provides JSON/REST API functionality" {
	...
	function init(siteid){
		...
		*/
		...
		var apiEndpoint=site.getResourcePath(complete=1);

		if(configBean.getIndexfileinurls() || configBean.getValue(property="indexFileInAPI", defaultValue=true) ){
			variables.endpoint="#apiEndpoint#/index.cfm#getAPIDir()#/json/v1/#variables.siteid#/";
			variables.endpointRootRelative="/index.cfm#getAPIDir()#/json/v1/#variables.siteid#/";
		} else {
			variables.endpoint="#apiEndpoint##getAPIDir()#/json/v1/#variables.siteid#/";
			variables.endpointRootRelative="/index.cfm#getAPIDir()#/json/v1/#variables.siteid#/";
		}

		variables.defaultcontentfields="releasedate,created,credits,lastupdatelinks,image,parentid,moduleid,path,contentid,contenthistid,changesetid,siteid,fileid,fileext,filesize,active,approved,title,menutitle,summary,target,tags,type,subtype,displayStart,displayStop,display,filename,url,assocurl,isNew,remoteid,remoteurl";
		
		if(configBean.getValue(property="pluralimagenames",defaultValue=true)){
			variables.defaultcontentfields="releasedate,created,credits,lastupdate,links,images,parentid,moduleid,path,contentid,contenthistid,changesetid,siteid,fileid,fileext,filesize,active,approved,title,menutitle,summary,target,tags,type,subtype,displayStart,displayStop,display,filename,url,assocurl,isNew,remoteid,remoteurl";
		}

		variables.userUtility=getBean('userUtility');

		param name="request.MuraAPIReset" default="false";

		if(!isDefined('application.MuraAPIConfig')){
			request.MuraAPIReset=true;
		}

		if(request.MuraAPIReset && !isDefined('request.MuraAPIConfig')){
			request.MuraAPIConfig={
				linkMethods=[],
				publicMethods="findDomain,findSiteID,undeclareEntity,declareEntity,checkSchema,siteExists,createSite,clientconfig,configurator,findText,findOne,findMany,findAll,findPermissions,findProperties,findNew,findQuery,save,delete,findCrumbArray,generateCSRFTokens,hasPendingRequest,validateEmail,login,logout,submitForm,findCalendarItems,validate,processAsyncObject,findRelatedContent,getURLForImage,findVersionHistory,auditTrail,findCurrentUser,currentUser,swagger",
				entities={
					'contentnav'={
						public=true,
						moduleid='00000000000000000000000000000000000',
						fields=variables.defaultcontentfields
					}
				},
				serializer=new mura.jsonSerializer()
					...
					.asInteger('expiring')
					.asInteger('approvals')
					.asInteger('submissions')
					.asInteger('isondisplay')
					.asInteger('statuscode')
					.asInteger('count')
					.asInteger('nocache')
					.asInteger('restrict')
					.asBoolean('adminpreview')
			};
			...
			registerEntity('user',{
				...
				fields="links,userid,fname,lname,email,company,images,mobilephone,jobtitle,website,ispublic,siteid,type,subtype,remoteid,lastupdate,lastupdateby,lastupdatebyid,lastlogin,photofileid,s2,isnew"
			});
			registerEntity('group',{
				public=false,
				moduleid='00000000000000000000000000000000008',
				fields="links,userid,groupname,ispublic,siteid,type,subtype,remoteid,isnew"
			});
			...
			registerEntity('alturl',{public=false,moduleid='00000000000000000000000000000000000'});
			...
			registerEntity('entityCategoryAssign',{public=true,moduleid='00000000000000000000000000000000000'});
			registerEntity('entityTagAssign',{public=true,moduleid='00000000000000000000000000000000000'});
			registerEntity('lambda',{public=false,moduleid='00000000000000000000000000000000000'});
			registerEntity('lambdaenv',{public=false,moduleid='00000000000000000000000000000000000'});
			registerEntity('proxy',{public=false,moduleid='00000000000000000000000000000000000'});
			registerEntity('proxyEvent',{public=false,moduleid='00000000000000000000000000000000000'});
			registerEntity('proxyCredential',{public=false,moduleid='00000000000000000000000000000000000'});
			registerEntity('oauthclient',{public=false,moduleid='00000000000000000000000000000000000'});
			registerEntity('moduleTemplate',{public=false,moduleid='00000000000000000000000000000000017'});
			registerEntity('approvalChain',{public=false,moduleid='00000000000000000000000000000000019'});
			registerEntity('approvalChainMembership',{public=false,moduleid='00000000000000000000000000000000019'});
			registerEntity('approvalRequest',{public=false,moduleid='00000000000000000000000000000000019'});
			registerEntity('approvalAction',{public=false,moduleid='00000000000000000000000000000000019'});
			registerEntity('approvalChainAssignment',{public=false,moduleid='00000000000000000000000000000000019'});
			registerEntity('proxy',{public=false,moduleid='00000000000000000000000000000000020'});
			registerEntity('proxyCredential',{public=false,moduleid='00000000000000000000000000000000020'});
			registerEntity('proxyEvent',{public=false,moduleid='00000000000000000000000000000000020'});
			...
			application.MuraAPIConfig=request.MuraAPIConfig;
	...
	function findDomain(){
		if(!isDefined('url.siteid')){
			throw(type="invalidParameters");
		}
		var siteManager=getBean('settingsManager');
		if(!siteManager.siteExists(url.siteid)){
			return siteManager.getSite('default').getDomain();
		} else {
			return siteManager.getSite(url.siteid).getDomain();
		}
	}
	...
	function siteExists(){
		...
		return serializeResponse(statusCode=200,response={'apiversion'=getApiVersion(),'method'='siteExists','data'=getBean('settingsManager').siteExists(url.siteid)});
	}

	function findSiteID(domain=''){
		if(!len(arguments.domain)){
			arguments.domain=getBean('utility').getRequestHost();
		}
		var siteid=getBean('contentServer').bindToDomain(domain=arguments.domain,isAdmin=false,doRedirect=false,completeOnly=true);
	...
	function configurator(){
		...
		return new mura.executor().execute('/mura/customtags/configurator/ui/configurator.cfm');
	}

	function getSerializer(){
		return getConfig().serializer;
	}

	
	function getConfig(){
		if(isdefined('request.MuraAPIConfig')){
			return request.MuraAPIConfig;
		} else{
			return application.MuraAPIConfig;
		}
	
	}

	function setConfig(conifg){
		application.MuraAPIConfig=arguments.config;
		return this;
	}

	function getEntityConfig(entityName){
		if(!structKeyExists(getConfig().entities,arguments.entityName)){
			getConfig().entities[arguments.entityName]={public=false};
		}

		return getConfig().entities[arguments.entityName];
	}

	function registerMethod(methodName, method){
		if(!listFindNoCase(getConfig().publicMethods,arguments.methodName)){
			getConfig().publicMethods=listAppend(getConfig().publicMethods,arguments.methodName);
		}
	...
	function registerDisplayObject(displayObjectName, config){
		getConfig()['#arguments.displayObjectName#']=arguments.config;
	...
	function registerEntity(entityName, config={public=false,fields=''},beanInstance=''){
		...
		if(request.MuraAPIReset && structKeyExists(getConfig().entities,'#arguments.entityName#')){
			return;
		}
		...
		getConfig().entities['#arguments.entityName#']=arguments.config;
		...
		if(isDefined('beanInstance.registerAsEntity')){
			beanInstance.registerAsEntity();
		}

		if(!isDefined('arguments.config.displayname')){
			if(isDefined('arguments.beanInstance.getEntityDisplayName')){
				arguments.config.displayname=arguments.beanInstance.getEntityDisplayName();
			} else {
				arguments.config.displayname=arguments.entityName;
			}
		}

		if(!structKeyExists(getConfig().entities['#arguments.entityName#'],'moduleid')){
			getConfig().entities['#arguments.entityName#'].moduleid='00000000000000000000000000000000000';  //beanInstance.getRegisteredEntity().getEntityid();
		}

		if(isDefined('arguments.beanInstance.getProperties')){
			...
			for(var p in properties){
				try{
					...
					} else if(properties[p].datatype=='json'){
						serializer.asJSON(properties[p].name);
					}
				} catch(Any e){}
			}
		}
	...
	function registerLinkMethod(method){
		var name="getLinks#arrayLen(getConfig().linkMethods)#";
		arrayAppend(getConfig().linkMethods,name);
	...
	function addResponseToCache(result,$){
		//dump(request.translatedcachekey);abort;
		param name="request.cacheItem" default=true;
		if(getHTTPRequestData().method=='get' 
			&& getpagecontext().getResponse().getStatus() == 200 
			&& $.siteConfig('cache') && isBoolean(request.cacheItem) 
			&& request.cacheItem && len(request.translatedcachekey)
			&& !structKeyExists(arguments.result,'errors')
			&& !structKeyExists(arguments.result,'error')
		){	arguments.result.cachedSince=now();
			var cache=$.siteConfig().getCacheFactory(name=request.cacheStore);
			var cachedwithin=(len($.event('cachedwithin')) && isNumeric($.event('cachedwithin'))) ? $.event('cachedwithin') : 3600;
			cachedwithin=createTimespan(0,0,0,cachedwithin);
			cache.set( key=request.translatedcachekey,context=duplicate(arguments.result),timespan=cachedwithin);
		}
	}
	
	function checkCacheForResponse(method,params,$){
		
		if(getHTTPRequestData().method=='get' && $.siteConfig().getCache()
			&& (
				(
					structKeyExists(params,'cachekey')
					&& len(params.cachekey) 
				)
				|| (
					structKeyExists(params,'entityName')
					&& params.entityName=='content'
					&& !isdefined('url.liveonly')
				)
			) 
		){
			if(structKeyExists(params,'cachekey')){
				request.translatedcachekey=variables.siteid & 'api' & params.cachekey & getEndpoint() & request.muraOutputCacheOffset;
			} else {
				request.translatedcachekey=variables.siteid & arguments.method ;
				for(var u in url){
					if(!listFindNoCase('purgecache,cacheid,_cacheid,#application.appreloadkey#',u)){
						var checkval=url['#u#'];
						if(isDefined('checkval')){
							request.translatedcachekey=request.translatedcachekey & u & '=' & url['#u#'];
						}
					}
				}
				request.translatedcachekey=request.translatedcachekey & getEndpoint() & request.muraOutputCacheOffset;
			}
			
			if(structKeyExists(params,'entityName') && params.entityName=='content'){
				var cache=$.siteConfig().getCacheFactory(name="output");
				request.cacheStore='output';
			} else {
				var cache=$.siteConfig().getCacheFactory(name="data");
				request.cacheStore='data';
			}

			param name="request.purgeCache" default=false;

			if(structKeyExists(params,'purgeCache') && isBoolean(params.purgeCache)){
				request.purgeCache=params.purgeCache;
			} else if(structKeyExists(params,'cachedwithin') 
				&& isNumeric(params.cachedwithin)
				&& !params.cachedwithin ){
				request.purgeCache=true;
			}
			
			if(isBoolean(request.purgeCache) && request.purgeCache){
				cache.purge(key=request.translatedcachekey);
				getBean('clusterManager').purgeCacheKey(cacheName=request.cacheStore,cacheKey=request.translatedcachekey,siteid=variables.siteid);
			} else if(cache.has(request.translatedcachekey)){
				var result=cache.get(request.translatedcachekey);
				if(isDefined('result')){
					return result;
				}
			}
	...
	function processRequest(path=cgi.path_info){
		try {
			...
			if( structKeyExists( headers, 'Origin' )){
				if(configBean.getValue(property="cors",defaultValue=true)){
					var origin =  headers['Origin'];
					if(configBean.getValue(property="individualsitebuilds",defaultValue=false)){
			...
			if(httpRequestData.method=='OPTIONS'){
				if(configBean.getValue(property="cors",defaultValue=true)){
					responseObject.setHeader( 'Access-Control-Allow-Methods', 'POST,GET,OPTIONS,PUT,DELETE,PATCH,HEADERS' );
					responseObject.setHeader( 'Access-Control-Allow-Headers',configBean.getValue(property='AccessControlAllowHeaders',defaultValue='Content-Type,Accept,Accept-Language,Content-Language,X-Requested-With,Authorization'));
				}
				responseObject.setHeader( 'Allow', 'POST,GET,OPTIONS,PUT,DELETE,PATCH,HEADERS' );
				return '';
			}
			...
			if(structKeyExists(httpRequestData,'content') && isJSON(httpRequestData.content)){
				structAppend(params,deserializeJSON(httpRequestData.content));
			}

			if(structKeyExists(params,'muraPointInTime')){
				try {
					params.muraPointInTime=parseDateTime(params.muraPointInTime);
					if(isDate(params.muraPointInTime)){
						request.muraPointInTime=params.muraPointInTime;
					}
				} catch(any e){
					logError(e);	
				}
			...
			if(!request.muraSessionManagement){
				...
				//writeLog(type="information",log="application",text="params: " & serializeJSON(params) & " pathInfo: " & serializeJSON(pathInfo));
			...
			arrayDeleteAt(pathInfo,1); //delete #getAPIDir()#
			arrayDeleteAt(pathInfo,1); //delete json
			arrayDeleteAt(pathInfo,1); //default v1
			...
			if (!isDefined('params.method') && arrayLen(pathInfo)){
				if(isValid('variableName',pathInfo[1])){
					if(pathInfo[1]==variables.siteid){
						if(arraylen(pathInfo) == 2 && isValid('variableName',pathInfo[2]) && isDefined('#pathInfo[2]#')){
			...
			request.cacheItem=true;
			request.translatedcachekey='';
			request.cacheStore='output';
			var siteManager=getBean('settingsManager');
			
			if (isDefined('params.method') && isDefined('#params.method#')){
				if(!listFindNoCase(getConfig().publicMethods, params.method) ){
				...
				if(!getCurrentUser().isSuperUser()  && !(isDefined('params.method') && listFindNoCase('findDomain,siteExists',params.method)) && !siteManager.siteExists(params.siteid)){	
					throw(type="invalidParameters");
				}

				if(isDefined('#params.method#')){
					var argParams=structCopy(params);
					argParams.pathInfo=pathInfo;
					getBean('$').init(argParams).announceEvent('onApiRequest');
			...
			} else if(!isDefined('params.siteid')){
				throw(type="invalidParameters");
			}
			...
			if(arrayLen(pathInfo) > 1){
				...
				if(arrayLen(pathInfo) == 2 && isDefined(pathInfo[2]) && pathInfo[2] != 'file'){
			...
			if(isDefined('params.entityname') 
				&& params.entityname == 'proxy' 
				&& arrayLen(pathInfo) >= 3
				&& !(arraylen(pathInfo) >= 4 && isValid('uuid',pathInfo[4]))){
				var proxy=getBean('proxy').loadBy(resource=pathInfo[3],siteid=variables.siteid);
				var resourcepath=[];

				if(proxy.exists()){
					if(!proxy.allowProxyAccess()){
						throw(type="authorization");
					} 
					if(arraylen(pathInfo) > 3){
						for(var p=4;arrayLen(pathInfo) >= p;p++){
							arrayAppend(resourcepath,pathInfo[p]);
						}
					}
					...
					if(structKeyExists(httpRequestData.headers,'Content-Type')){
						arrayAppend(
							reqHeaders,
							{
								name="Content-Type",
								value=httpRequestData.headers['Content-Type']
							}
						);
					}

					resourcePath=arrayToList(resourcepath,"/");

					var cacheArgs={cachekey=$.event('cachekey')};

					if(isBoolean($.event('purgeCache'))){
						cacheArgs.purgeCache=$.event('purgeCache');
					}
					if(isNumeric($.event('cachedwithin'))){
						cacheArgs.cachedwithin=$.event('cachedwithin');
					}
				
					if(httpRequestData.method=='GET'){
						var result=checkCacheForResponse(resourcePath,cacheArgs,$);
						if(!isDefined('result')){
							result=proxy.call(formScope=form,urlScope=url,headers=reqHeaders,body=httpRequestData.content,resourcepath=resourcePath,parse=false,httpMethod=httpRequestData.method);
							addResponseToCache(result,$);

						}
						return proxy.formatResponse(result);
					} else {
						return proxy.formatResponse(proxy.call(formScope=form,urlScope=url,headers=reqHeaders,body=httpRequestData.content,resourcepath=resourcePath,parse=false,httpMethod=httpRequestData.method));
					}
			...
			if (params.entityName == 'site'){
				...
				if(arrayLen(pathInfo) > 2){
					if(len(pathInfo[3])==35 || isValid('integer',pathInfo[3])){
						...
						//Remote methods defined in module beans
			...
			if(params.entityName=='currentuser' && listFindNoCase('findOne,findMany,findQuery',params.method)){
				params.method='currentUser';
				result=currentUser(argumentCollection=params);		
				return serializeResponse(statusCode=200,response={'apiversion'=getApiVersion(),'method'=params.method,'params'=getParamsWithOutMethod(params),'data'=result});
			}
				...
				case "GET":
					...
					if((isDefined('params.id') || (params.entityName=='content') && isDefined('params.contenthistid'))){
						...
						if(params.id=='new') {
							...
							if(params.entityName=='site' && arrayLen(pathInfo) < 3){
								params.method='findOne';
								params.id=variables.siteid;
								result=findOne(argumentCollection=params);
							} else if(isValid('uuid',params.id) || isValid('integer',pathInfo[3]) || params.id=='00000000000000000000000000000000001'){
								params.method='findOne';
								result=findOne(argumentCollection=params);
							} else {
								return doubleCheckEntityMethod(params.entityname,params.id,params,false);
							}
						}
						...
						if(arrayLen(pathInfo) == 3 
							&& !(
								...
								|| isValid('integer',pathInfo[3]) 
								|| pathInfo[3]=='00000000000000000000000000000000001'
								)
						){
							return doubleCheckEntityMethod(params.entityname,pathInfo[3],params,true);
						} else {
							if(structCount(url)){
								params.method='findQuery';
			...
			if(len(cgi.query_string)){
				writeLog(type="Error", log="exception", text="#cgi.query_string#");
			}
			writeLog(type="Error", log="exception", text="API Request Error");
			logError(e);
			param name="params.method" default="undefined";
			if(!isDefined('#params.method#')){
				params.method='invalid';
	...
	function doubleCheckEntityName(entityName){
		...
		if(!structKeyExists(variables.entityChecks,'#arguments.entityname#')
			...
			var lookupdata=getBean('settingsManager').getDeferredThemeModuleAssets(theme=site.getTheme());
			...
			for(var asset in lookupdata.assets){
				if(structKeyExists(asset,'modelDir') and len(asset.modelDir)){
					...
					|| fileExists(asset.modelDir & "/beans/" & arguments.entityName & ".cfc")
						...
						getBean('configBean').registerBeanDir(dir=asset.modelDir,package=asset.package,siteid=siteid,forceSchemaCheck=true);
						found=true;
						break;
					}
				}
			...
			if(!found){
				lookupdata=getBean('settingsManager').getDeferredSiteModuleAssets(siteid=variables.siteid);
				...
				for(var asset in lookupdata.assets){
					if(structKeyExists(asset,'modelDir') and len(asset.modelDir)){
						...
						|| fileExists(asset.modelDir & "/beans/" & arguments.entityName & ".cfc")
							...
							getBean('configBean').registerBeanDir(dir=asset.modelDir,package=asset.package,siteid=variables.siteid,forceSchemaCheck=true);
							found=true;
							break;
						}
					}
			...
			if(!found){
				var lookupdata=getBean('settingsManager').getDeferredGlobalModuleAssets();
	...
	function doubleCheckEntityMethod(entityName,method,params,throwError=false){
		...
		if(isValid('variableName',arguments.method)){
			...
			if(arguments.entityName=='currentUser'){
				var currentUser=getCurrentUser();
				if(structKeyExists(currentUser,arguments.method)){
					arguments.params.method=arguments.method;
					var result=invoke(entity,arguments.method,arguments.params);	
					if(!isJson(result)){
						result=serializeResponse(statusCode=200,response={'apiversion'=getApiVersion(),'method'=params.method,'params'=getParamsWithOutMethod(params),'data'=result});
					} else {
						getpagecontext().getResponse().setContentType('application/json; charset=utf-8');
			...
			if(isDefined('application.objectmappings.#arguments.entityName#.remoteFunctions.#arguments.method#')) {
				...
				structDelete(arguments.params,'id');
				...
				arguments.params.method='findOne';
				var result=findOne(argumentCollection=arguments.params);
				return serializeResponse(statusCode=200,response={'apiversion'=getApiVersion(),'method'=params.method,'params'=getParamsWithOutMethod(params),'data'=result});
			}
		} else if (arguments.throwError){
			throw(type="invalidMethodCall");
	...
	function serializeResponse(response,statusCode=200,fireEvents=true){

		var responseObject=getpagecontext().getResponse();
		responseObject.setContentType('application/json; charset=utf-8');
		if(request.muraShowTrace){
			var sessionData=getSession();
			if(isDefined("sessionData.mura.showTrace") and sessionData.mura.showTrace and listFindNoCase(sessionData.mura.memberships,"S2IsPrivate") ){
				arguments.response.stackstrace=request.muraTraceRoute;
				arguments.response.stackstracetotal=getTickCount()-request.muraRequestStart;
			}
		}
		try{
			if(request.mura404){
		...
		//writeLog(type="information",log="application",text="response; " & serializeJSON(arguments.response));
		...
		if(!$.globalConfig().getValue(property="compression",defaultValue=false)){
			cfheader( name="ETag", value="W/#hash(result)#");
		}
	...
	function AllowAccess(bean,$,throwError=true){
		...
		if(entityName == 'entity'){
			return true;
		} else if(entityName=='oauthclient' && !$.currentUser().isSuperUser()){
			return false;
		}
		...
		if(!structKeyExists(getConfig().entities,entityName)){
			throw(type='invalidEntityCall');
		} else if (
				...
				&& !(
					...
					|| getBean('permUtility').getModulePerm(getConfig().entities['#entityName#'].moduleid,variables.siteid)
				)
			){
			return false;
		}

		var config=getConfig().entities[entityName];
	...
	function AllowAction(bean,$,isCurrent=false){
		if(!isDefined('arguments.bean.getEntityName')){
			if(!isDefined('arguments.bean.get') || !len(arguments.bean.get('entityName'))){
				throw(type='invalidParameters');
			}
		}
		var tracePoint=initTracePoint(detail="Checking Allow Action for #arguments.bean.getEntityName()#");
		...
		switch(arguments.bean.getEntityName()){
			case 'content':
				...
				// 	default:
				// 		param name="request.muraSharedContentPermLookup" default={};
				// 		if(structIsEmpty(request.muraSharedContentPermLookup)){
				// 			if(!getBean('permUtility').getModulePerm('00000000000000000000000000000000000',variables.siteid)){
				// 				commitTracePoint(tracePoint);
				// 				return false;
				// 			}
				...
				if(arguments.isCurrent){
					local.currentBean=arguments.bean;
				} else {
				...
				if(local.currentBean.getIsNew() && len(arguments.bean.getParentID())){
					local.crumbData=getBean('contentManager').getCrumblist(arguments.bean.getParentID(), arguments.bean.getSiteID());
					local.perm=getBean('permUtility').getNodePerm(local.crumbData);
				}

				if(!listFindNoCase('author,editor',local.perm)){
					//Api Only Event
					local.bean=local.currentBean;
					local.contentbean=local.currentBean;
					var checkScope=getBean('$').init(local);
					checkScope.announceEvent('contentSaveDeny');
					if(!listFindNoCase('author,editor',checkScope.event('perm'))){
						commitTracePoint(tracePoint);
						return false;
					}
				}
				...
				if(local.perm=='author'){
					...
					local.contentbean=local.currentBean;
					var checkScope=getBean('$').init(local);
					checkScope.announceEvent('contentPublishDeny');
					if(checkScope.event('perm')=='author'){
			...
			case 'group':
			case 'address':
				if(getBean('permUtility').getModulePerm(getConfig().entities['#arguments.bean.getEntityName()#'].moduleid,variables.siteid)){
					//Only super users can update super users
					if(arguments.bean.getEntityName()=='user' && arguments.bean.exists() && arguments.bean.getS2() && !arguments.$.currentUser().isSuperUser()){
						commitTracePoint(tracePoint);
						return false;
					}
					commitTracePoint(tracePoint);
			...
			default:
				...
				if (isDefined('config.entities.#arguments.bean.getEntityName()#.moduleid')) {
					commitTracePoint(tracePoint);
					return listFindNoCase('editor,module',getBean('permUtility').getModulePermType(getConfig().entities['#arguments.bean.getEntityName()#'].moduleid,variables.siteid));
				} else {
					commitTracePoint(tracePoint);
					return getBean('permUtility').getModulePerm('00000000000000000000000000000000000',variables.siteid);
				}
	...
	function findCalendarItems(calendarid, siteid, start, end, categoryid, tag,format='', searchtext ='', useCategoryIntersect=0) {
	...
	function save(siteid,entityname,id='new',expand=''){
		...
		if(listFindNoCase('user,group',arguments.entityName)){
			...
			var hasUserModuleAcces=getBean('permUtility').getModulePerm(getConfig().entities.user.moduleid,arguments.siteid);
		...
		if(len($.event(pk)) && (isValid('uuid',$.event(pk)) || isValid('integer',$.event(pk)) || $.event(pk)=='00000000000000000000000000000000001' || pk=='siteid') ){
		...
		if(arguments.id=='new'){
			if(arguments.entityname == 'site'){
				if(len($.event('siteid'))){
					$.event('id',$.event('siteid'));
					arguments.id=$.event('id');
					arguments.siteid=$.event('siteid');
				} else {
					throw(type="invalidParameters");
				}
			} else {
	...
	function getFilteredValues(entity,expanded=0,entityConfigName,siteid,expandLinks='',pk='',editablecheck=false,expandedProp='',$,fields=''){
		var vals={};
		...
		if(doFields && isDefined('url.fields') && !len(arguments.fields)){
			arguments.fields=url.fields;
		}

		var config=getConfig();
		var allowfieldselect=true;

		if(isDefined('config.entities.#arguments.entityConfigName#.allowfieldselect')){
			allowfieldselect=config.entities[entityConfigName].allowfieldselect;
		}

		var isFieldSelectOverride=false;

		if(!allowfieldselect){
			if(arguments.$.currentUser().isAdminUser()){
				allowfieldselect=true;
				isFieldSelectOverride=true;
			}
		}
		
		if(allowfieldselect && (doFields && isDefined('arguments.fields') && len(arguments.fields))){

			/* 
				Admin users are able to use the fields argument for all entities. 
				However, if doing so on an entity that normally doesn't allow it make sure not to cache it
			*/
			if(isFieldSelectOverride){
				getBean('utility').excludeFromClientCache();
				request.cacheItem=false;
			}

			if(!isAggregateQuery()){
				if(arguments.entity.getEntityName()=='content' && !listFindNoCase(arguments.fields,'contentid')){
					arguments.fields=listAppend(arguments.fields,'contentid');
				} else if (arguments.entity.getEntityName()!='content' && !listFindNoCase(arguments.fields,arguments.entity.getPrimaryKey())){
					arguments.fields=listAppend(arguments.fields,arguments.entity.getPrimaryKey());
				}
				
				if(!listFindNoCase(arguments.fields,'siteid')){
					arguments.fields=listAppend(arguments.fields,'siteid');
				}
				...
				if(listFindNoCase('content,contentnav,comment,category,contentCategoryAssign',arguments.entity.getEntityName())){
					if(listFindNoCase('*,all',arguments.expandlinks) || listFindNoCase(arguments.expandlinks,'crumbs')){
						if(!listFindNoCase(arguments.fields,'links')){
							arguments.fields=listAppend(arguments.fields,'links');
						}
						if(!listFindNoCase(arguments.fields,'path')){
							arguments.fields=listAppend(arguments.fields,'path');
						}
					}
				}
			}
		} else if(isDefined('config.entities.#arguments.entityConfigName#.fields') && len(getConfig().entities[arguments.entityConfigName].fields)){
			arguments.fields=getConfig().entities[arguments.entityConfigName].fields;
		}
		
		if(len(arguments.fields) && !listFindNoCase(arguments.fields,'isnew')){
			arguments.fields=listAppend(arguments.fields,'isnew');
		}

		arguments.fields=lcase(arguments.fields);
		
		arguments.fields=listToArray(arguments.fields);

		var allFields=!arrayLen(arguments.fields) || arrayFind(arguments.fields,'*') || arrayFind(arguments.fields,'all');
		...
		if(!allFields){
			...
			for(var f in arguments.fields){
				var _f=listLast(f,'.');

				if(!structKeyExists(skipFields,_f)){
					if(f=='extendeddata'){
						...
						if(f=='displayInterval'){
							...
							var prop=arguments.entity.getValue(property=_f);
						}
		...
		if(allFields || arrayFind(arguments.fields,'links')){
			arguments.editablecheck=(isDefined('url.id') && isDefined('vals.id') && vals.id==url.id || arguments.editablecheck)? true :false;
			vals.links=getLinks(entity,arguments.editablecheck);
		}
		
		if(listFindNoCase('content,contentnav',arguments.entityConfigName)){
			if(allFields){
				if($.globalConfig().getValue(property="pluralimagenames",defaultValue=true)){
					vals.images=setImageURLS(entity=entity,$=$);
				} else {
					vals.image=setImageURLS(entity=entity,$=$);
				}
			} else if(arrayFind(arguments.fields,'images')){
				vals.images=setImageURLS(entity=entity,$=$);
			} else if(arrayFind(fields,'image')){
				vals.image=setImageURLS(entity=entity,$=$);
			}
			if(allFields || arrayFind(arguments.fields,'url')){
			...
			for(var imgAttr in imageAttributes){
				if(structKeyExists(vals,'#imgAttr#') && isValid('uuid',vals['#imgAttr#'])){
					if($.globalConfig().getValue(property="pluralimagenames",defaultValue=true)){
						...
						vals['#imgAttr#image']=setImageURLS(entity,imgAttr,$);
					}
				}
			}
		}
		...
		if(listFindNoCase('group,user',arguments.entityConfigName)){
			...
			if(vals.type=='user'){
				if(allFields){
					if($.globalConfig().getValue(property="pluralimagenames",defaultValue=true)){
						vals.images=setImageURLS(entity=entity,attr='photofileid',$=$);
					} else {
						vals.image=setImageURLS(entity=entity,attr='photofileid',$=$);
					}
				} else if(arrayFind(arguments.fields,'images')){
					vals.images=setImageURLS(entity=entity,attr='photofileid',$=$);
				} else if(arrayFind(fields,'image')){
		...
		if(arguments.expanded < url.expanddepth && len(arguments.expandLinks)){
			expandEntity(entity=arguments.entity,itemStruct=vals,siteid=arguments.siteid,expand=arguments.expandLinks,expanded=arguments.expanded,$=arguments.$);
		}
	...
	function findPermissions(entityName,id,siteid){
		...
		if(isValid('UUID',arguments.id) || isValid('integer',arguments.id) || len(arguments.id)==35){
			if(arguments.entityName=='content'){
				var pk = 'contentid';
			} else {
				var pk = entity.getPrimaryKey();
	...
	function currentUser(entityName,id,siteid,render=false,variation=false,expand=''){
		return findCurrentUser(argumentCollection=arguments);
	}
	...
	function findOne(entityName,id,siteid,render=false,variation=false,expand='',method='findOne',expanded=0){
		...
		if(!listFindNoCase('currentUser,findCurrentUser',arguments.method) && !(entity.getEntityName() == 'fileMetaData' && arguments.expanded)){
	...
	function expandEntity(entity,itemStuct,siteid,expand='',expanded=0,$=''){
		...
		if(len(arguments.expand)){
			...
			var tracePoint=initTracePoint(detail="Expanding #arguments.entity.getEntityName()#: #arguments.expand#");

			//These are relationships defined in the component pointing at other entitities
				...
				for(p in arguments.entity.getHasManyPropArray()){
					if(expandAll || listFindNoCase(arguments.expand,p.name)){
						...
						if(p.cfc=='content'){
							...
							if(isDefined('url.applyChangesets')){
								expandParams.shownavonly=url.shownavonly;
							}
						...
						if(isDefined('url.pointInTime')){
							expandParams.pointInTime=url.pointInTime;
						}
						if(isDefined('url.renderMode')){
							expandParams.renderMode=renderMode;
						}
			...
			//These are relationships defined in the external component
			if(!expandAll){
				for(p in listToArray(arguments.expand)){
					if(!structKeyExists(arguments.itemStruct,'#p#')){
						if(arguments.entity.hasSynthedFn('get#p#Iterator')){
							var iterator=invoke(arguments.entity,'get#p#Iterator');
							setIteratorProps(iterator=iterator);
							if(!isObject(arguments.$)){
								arguments.$=getBean('$').init(arguments.siteid);
							}
							var returnArray=iteratorToArray(iterator=iterator,siteid=arguments.siteid,expand=arguments.expand,$=arguments.$,expanded=arguments.expanded);
							arguments.itemStruct['#p#']=packageIteratorArray(iterator=iterator,itArray=returnArray,method='findQuery',baseurl='',expanded=arguments.expanded);	
						} else if(arguments.entity.hasSynthedFn('get#p#')){
							arguments.itemStruct['#p#']=invoke(arguments.entity,'get#p#');
						}
					}
				}
			}
	...
	function findAll(siteid,entityName,params,expand='',expanded=0){
		...
		if(!arguments.expanded){
			var result=checkCacheForResponse('findAll' & arguments.entityName,params,$);
		}
		if(isDefined('result')){
			return result;
		} else {
			...
			if(arguments.entityName=='entity'){
				...
				for(var i in getConfig().entities){
					if(getServiceFactory().containsBean(i)){
						var instance=$.getBean(i);
						if(allowAccess(instance,$,false) && ($.event('scaffold')=='' || $.event('scaffold')==1 && allowAction(instance,$)) ){
			...
			if(isDefined('entity.applyGatewaySecurity')){
				entity.applyGatewaySecurity(feed);
			}
			...
			if(isDefined('arguments.params.countOnly') && isBoolean(arguments.params.countOnly) && arguments.params.countOnly){
				...
				var applyPermFilter=(isBoolean($.event('applyPermFilter')) && $.event('applyPermFilter')) || $.siteConfig('extranet');
				var iterator=feed.getIterator(applyPermFilter=applyPermFilter);
			...
			var result=packageIteratorArray(iterator,returnArray,'findall');
			if(!arguments.expanded){
				addResponseToCache(result,$);
			}
			return result;
	...
	function findMany(entityName,ids,siteid,params,expand='',expanded=0){
		...
		if(!arguments.expanded){
			var result=checkCacheForResponse('findMany' & arguments.entityName,params,$);
		}
		if(isDefined('result')){
			return result;
		} else {
			...
			var result=packageIteratorArray(iterator,finalArray,'findmany');
			if(!arguments.expanded){
				addResponseToCache(result,$);
			}
			return result;
	...
	function isValidEntityPropName(entity,propname){
		var transformedPropName=arrayToList(listToArray(listFirst(arguments.propname,'$'),'.'),'.');
		var allow=transformedPropName=='id' || arguments.entity.valueExists(transformedPropName) || arguments.entity.valueExists('extendData') || transformedPropName=='category' || arguments.propName=='categoryid' || arguments.propName == 'tag' || arguments.propName == 'taggroup';
		if(!allow && listLen(transformedPropName,'.')==2){
			var props=listToArray(transformedPropName,".");
			if(arguments.entity.hasSynthedFn('get#props[1]#') || getServiceFactory().containsBean(props[1]) && getBean(props[1]).valueExists(props[2])){
				allow=true;
			}
		}
		return allow;
	...
	function getNonParamList(){
		return 'applyChangesets,iseditroute,renderMode,expanddepth,expand,liveOnly,feedid,cacheid,_cacheid,cachedWithin,cachekey,distinct,fields,entityname,method,maxItems,pointInTime,pageIndex,itemsPerPage,sortBy,sortDirection,renderMode,contentpoolid,shownavonly,showexcludesearch,showtrace,includehomepage,feedname,useCategoryIntersect,useDataNamespace,editlayout,applyPermFilter,imagesizes,purgeCache,mura_upc,mura_ocs';
	}
	
	function findQuery(entityName,siteid,params,queryString=cgi.QUERY_STRING,expand='',expanded=0,expandedProp=''){
		var nonParamList=getNonParamList();
		...
		if(!arguments.expanded){
			var result=checkCacheForResponse('findQuery' & arguments.entityName,params,$);
		}
		if(isDefined('result')){
			return result;
		} else {
			...
			var started=false;
			var securityWrapStarted=false;
			
			if(arguments.entityName=='content'){
				for(p in queryArray){
					if(left(p,6)=='feedid'){
						checkProp=urlDecode(listFirst(p,'='));
						if(find('[',checkProp) && !find('$',checkProp)){
			...
			if(arguments.expanded){
				...
				for(var p in arguments.params){
					if(!listFindNoCase(nonParamList,p)){
				...
				for(var p in queryParams){
					...
					if(structKeyExists(params,p)){
						...
						//Lucee translates url var set to null as ""
						if(params[p]=='""'){
							params[p]='null';
						}
						...
						if(!listFindNoCase(nonParamlist,propName)){
							if(propName == 'sort'){
								advancedsort=listAppend(advancedsort,arguments.params[p]);
							} else if ((entity.getEntityName()=='user' || entity.getEntityName()=='group') && listFindNoCase('isPublic,type,siteid',propName)){
								if(propName!='siteid'){
									feed.setValue(propName,arguments.params[p]);
								}
							} else {
									if(entity.getEntityName()=='content' && propName=='type' && listFindNoCase('form,component',arguments.params[p])){
										feed.setType(arguments.params[p]);
									}
									if(entity.getEntityName()=='user' && propName=='groupid'){
										...
										if(!securityWrapStarted){
											securityWrapStarted=true;
									...
									} else if(isValidEntityPropName(entity,propname)){
										var condition="eq";
										...
										if(isValidEntityPropName(entity,propname)){
											if(!securityWrapStarted){
												securityWrapStarted=true;
												feed.addParam(relationship='openGrouping');
											}
			...
			entity.applyGatewaySecurity(feed=feed,$=$,Mura=$);

			setFeedProps(feed,arguments.params);
			
			if(isdefined('arguments.params.countOnly') && isBoolean(arguments.params.countOnly) && arguments.params.countOnly){
				var result={count=feed.getAvailableCount()};
				if(!arguments.expanded){
					addResponseToCache(result,$);
				}
				return result;
				...
				var iterator=feed.getIterator(applyPermFilter=$.siteConfig('extranet'));
				setIteratorProps(iterator=iterator);
				var returnArray=iteratorToArray(iterator=iterator,siteid=arguments.siteid,expand=arguments.expand,$=$,expanded=arguments.expanded,expandedProp=arguments.expandedProp);
				var result=packageIteratorArray(iterator=iterator,itArray=returnArray,method='findQuery',baseURL=baseURL,expanded=arguments.expanded);
				if(!arguments.expanded){
					addResponseToCache(result,$);
				}
				return result;
	...
	function iteratorToArray(iterator,siteid,expand='',$,expanded=0,expandedProp=''){
		var returnArray=[];
		var item='';
		...
		while(iterator.hasNext()){
			...
			if(item.getEntityName() == 'entity'){
				...
				arrayAppend(returnArray, getFilteredValues(entity=item,expanded=arguments.expanded,expandedProp=arguments.expandedProp,entityConfigName=entityConfigName,siteid=arguments.siteid,expandLinks=arguments.expand,pk=pk,$=$));
			}
		}
	...
	function setFeedProps(feed,params){
		...
		if(isDefined('arguments.params.contentpoolid') && len(arguments.params.contentpoolid)){
			if(arguments.params.contentpoolid == '*'){
				arguments.feed.setContentPoolID('*');
			} else {
				var poolrequest=[];
				...
				if(arrayLen(poolrequest)){
					arguments.feed.setContentPoolID(arrayToList(poolrequest));
				}
			}
		}
		...
		if(listFind('content,contentnav',arguments.feed.getEntityName())
			...
			&& len(params.changesetid)){
				arguments.feed.setActiveOnly(0);
			}
			...
			if(isDefined('params.type')
				...
				arguments.feed.setType(params.type);
			}

			if(isDefined('params.liveOnly')
				&& isBoolean(params.liveOnly)){
					arguments.feed.setLiveOnly(params.liveOnly);
			}
		}
			...
			for(var s in sort){
				if(len(s) > 1){
					...
					if(isContent && listFindNoCase('comments,random,rating',prop)){
						...
						if(prefix=='-'){
							arguments.feed.setSortDirection('desc');
						}
						break;
	...
	function findCrumbArray(entityName,id,siteid,iterator,expand='',expanded=0,expandedProp=''){
		...
		if(!arguments.expanded){
			var result=checkCacheForResponse('findCrumbArraycontent' & arguments.id,arguments,$);
		}
		if(isDefined('result') ){
			return result;
		} else {
			...
			while(arguments.iterator.hasNext()){
				...
				itemStruct=getFilteredValues(entity=item,expanded=arguments.expanded,expandedProp=arguments.expandedProp,entityConfigName=entityConfigName,siteid=arguments.siteid,expandLinks=arguments.expand,pk=pk,$=$);

				arrayAppend(returnArray, itemStruct );
			}

			var result=packageIteratorArray(arguments.iterator,returnArray,'findCrumbArray');
			if(!expanded){
				addResponseToCache(result,$);
			}
			return result;
	...
	function hasPendingRequest(contentid,changsetid='',siteid){
		return {count:getBean('contentManager').hasPendingRequest(argumentCollection=arguments)};
	}

	function auditTrail(contenthistid,siteid,expand='',expanded=0){
		if(!isDefined('url.fields')){
			url.fields=variables.defaultcontentfields & ",lastupdatebyid,lastupdateby";
		}

		var $=getBean('$').init(arguments.siteid);
		var entity=$.getBean('content').loadBy(contenthistid=arguments.contenthistid);
		var crumbdata=entity.getCrumbArray();
		...
		var iterator=entity.getAuditTrailIterator();
		...
		for(var i in returnArray){
			if(isDefined('i.links.self')){
				i.links.self=i.links.self & "?contenthistid=" & i.contenthistid;
				i.sourceid=sourceid;
				sourceid=i.contenthistid;
			}
		}

		return packageIteratorArray(iterator,returnArray,'findVersionAudit');
	}
	...
	function delete(entityName,id,siteid){
		...
		if($.event('entityName')=='content'){
			...
			if(len($.event(pk)) && (isValid('uuid',$.event(pk)) || isValid('integer',$.event(pk)) )){
				$.event('id',$.event(pk));
			}
	...
	function getEndpoint(mode='json',useProtocol=true,complete="true"){
		if(request.muraApiRequest){
			...
			if(!isDefined('request.apiEndpoint')){
				...
				if(useProtocol){
					if(utility.isHTTPS()){
						...
						if(configBean.getAdminSSL()){
							...
							var protocol=getBean('settingsManager').getSite(variables.siteid).getScheme(complete=arguments.complete) & ":";
						}
					}
				} else {
					var protocol='';
				...
				var domain=utility.getRequestHost();
				var port=utility.getRequestPort();

				var apiEndpoint="#protocol#//#domain##port##configBean.getContext()#";

				request.apiEndpoint="#apiEndpoint##indexFile##getAPIDir()#/#request.muraAPIRequestMode#/v1/#variables.siteid#";
				
				if(right(request.apiEndpoint,1) == "/"){
					request.apiEndpoint=left(request.apiEndpoint,len(request.apiEndpoint)-1);
				}
		...
		*/
		if(entity.getEntityName() != 'bean'){
			if(arguments.editablecheck &&  getCurrentUser().isLoggedIn() && getCurrentUser().isSystemUser()){
				if(allowAction(entity,getBean('m').init(entity.getSiteid()),true)){
			...
			if(entity.getEntityName()=='content'){
				...
				links['audit']="#baseurl#/auditTrail?contenthistid=#entity.getContentHistID()#";
			...
			if(arrayLen(getConfig().linkMethods)){
				for(var i in getConfig().linkMethods){
					if(isdefined('#i#')){
	...
	function findRelatedContent(id='',siteid,params,expand='',expanded=0,entity='',relatedContentSetID='',expandedProp=''){
		param name="arguments.params" default=url;
		...
		if(isSimpleValue(arguments.entity)){
			checkForChangesetRequest('content',arguments.siteid,$);
			if(structKeyExists(arguments.params,'contenthistid')){
				arguments.entity=$.getBean('content').loadBy(contenthistid=arguments.contenthistid);
			} else if (structKeyExists(arguments.params,'previewid')){
				arguments.entity=$.getBean('content').loadBy(contenthistid=arguments.previewid);
			} else {
		...
		if($.siteConfig('extranet')){
			args.applyPermFilter=true;
		}

		if(!arguments.expanded){
			var result=checkCacheForResponse('findRelatedcontent' & arguments.id & arguments.relatedContentSetID,arguments,$);
		}
		if(isDefined('result')){
			return result;
		} else {
			...
			if(arguments.relatedContentSetID=='' || arguments.relatedContentSetID=='*' || arguments.relatedContentSetID=='all'){
				...
				if(!listFindNoCase('00000000000000000000000000000000000',args.relatedcontentsetid) && !isValid('uuid',args.relatedcontentsetid)){
					args.name=args.relatedcontentsetid;
					structDelete(args,'relatedcontentsetid');
				}
				...
				if(!arguments.expanded){
					addResponseToCache(packagedItems,$);
				}
	...
	function findText(text='',siteid,sectionid='',categoryid='',tag='',mode='public',params,expand='',expanded=0,entity='',expandedProp=''){
		param name="arguments.params" default=url;
		...
		if(!arguments.expanded){
			var result=checkCacheForResponse('findText',arguments,$);
		}
		if(isDefined('result')){
			return result;
		} else {
			...
			if(len(arguments.text)){
				var feed=getFeed('content');

				feed.where()
					.openGrouping()
					.Prop('title').containsValue(arguments.text)
					.orProp('credits').containsValue(arguments.text)
					.orProp('summary').containsValue(arguments.text)
					.orProp('body').containsValue(arguments.text)
					.closeGrouping();
				
				if(len(arguments.categoryid)){
					feed.andProp('categoryid').isIn(arguments.categoryid);
				}

				if(len(arguments.tag)){
					feed.andProp('tag').isIn(arguments.tag);
				}

				if(len(arguments.sectionid)){
					feed.andProp('path').containsValue(arguments.sectionid);
				}

				setFeedProps(feed,arguments.params);

				iterator=feed.getIterator(applyPermFilter=$.siteConfig('extranet'));
			...
			var returnArray=iteratorToArray(iterator=iterator,siteid=arguments.siteid,expand=arguments.expand,$=$,expanded=arguments.expanded);
			var result=packageIteratorArray(iterator=iterator,itArray=returnArray,method='findText',expanded=arguments.expanded);
			if(!arguments.expanded){
				addResponseToCache(result,$);
			}
			return result;
	...
	function setImageURLs(entity,attr="fileid",$){
		...
		if(arguments.attr == 'fileid' && arguments.entity.hasImage()){
			...
			for(var i in ['small','medium','large','source']){
				if(!len($.event('imagesizes')) || listFindNoCase($.event('imagesizes'),i)){
					returnStruct['#i#']=arguments.entity.getImageURL(secure=secure,complete=1,size=i);
				}
			}
			...
			while(images.hasNext()){
				image=images.next();
				if(!len($.event('imagesizes')) || listFindNoCase($.event('imagesizes'),image.getName())){
			 ...
			 if(ListFindNoCase('jpg,jpeg,png,gif,svg,webp', fileEXT)){
	...
	function validate(data='{}',validations='{}') {
		...
		if(isDefined('data.entityname') && isDefined('data.siteid')){
			...
			errors=bean.loadBy(argumentCollection=args).set(arguments.data).validate(arguments.data.fields).getErrors();
	...
	function processAsyncObject(siteid){
		...
		if($.content().getType() != 'Variation' && isStruct( $.event('r')) && $.event('r').restrict){
			$.event('nocache',1);
		}
		...
		switch($.event('object')){
			case 'login':
				if(getHTTPRequestData().method == 'POST'){
					...
					if(isBoolean($.event('attemptChallenge')) && $.event('attemptChallenge')){
						...
						if(loginManager.remoteLogin($.event().getAllValues(),'')){
							...
							} else if(len($.event('returnurl'))){
								result={
									redirect=getBean('utility').sanitizeHREF($.event('returnurl')),
									statuscode=request.muraJSONRedirectStatusCode
								};
							} else {
								result={
				...
				if(!isStruct(result)){
					result=$.dspObject('login');

					if(!isStruct(result)){
						result={
							html=applyRemoteFormat(result),
							render="server"
						};
					}
			...
			case 'displayregion':
				result=$.dspObjects(argumentCollection=$.event().getAllValues());
			...
			default:
				...
				if(len($.event('objectparams')) && !isJson($.event('objectparams'))){
					...
					if(isDefined('url') && isStruct(url)){
						for(var u in url){
							if(!listFindNoCase('perm,contentid,contenthistid,object,objectid,siteid,nocache,editlayout',u)){
								args.params['#u#']=url['#u#'];
							}
						}
					}
					...
					if(isDefined('form') && isStruct(form)){
						for(var f in form){
							if(!listFindNoCase('perm,contentid,contenthistid,object,objectid,siteid,nocache,editlayout',f)){
				...
				*/
				...
				var wrapContent=(isBoolean($.event('decoupled')) && $.event('decoupled') ) ? false : true;
				...
				if(isdefined('request.muraJSONRedirectURL')){
					...
					if($.useLayoutManager() && isdefined('result.html') && result.render=='server'){
						if(wrapContent){
							...
							result={render='server',async=true,html=result.html};
						}
	...
	function checkForChangesetRequest(entityName,siteid,$=''){
		if(arguments.entityName=='content'){
			if(isSimpleValue(arguments.$)){
				arguments.$=getBean('$').init(arguments.siteid);
			}
			if($.event('renderMode')!='static' && !(isBoolean($.event('applyChangesets')) && !$.event('applyChangesets')) ){
				getBean('contentServer').processChangesetStatus(event=$.event(),checkForReload=false);
			}
		}
	}
		...
		if(arguments.mode=='JSON' && arguments._in=='query'){
			if(entity.getEntityName()=='content'){
				...
				arrayAppend(response,{
					...
					arrayAppend(response,{
						"name"= 'applyChangesets',
						"in"= "query",
						"description"= "Whether to apply any session based changesets to results",
						"required"= false,
						"type"= "boolean"
					});
	...
	function swagger(siteid,params){
		...
		if($.globalConfig('indexfileinurls') || $.globalConfig().getValue(property='indexFileInAPI',defaultValue=false)){
			var indexFile="/index.cfm";
		} else {
			var indexFile="";
		}
		...
		var result={
			...
			"info"= {
				...
				"termsOfService"= "https://www.murasoftware.com"//,
			// 	"contact"= {
...
*/			
		...
		var entityKeys=listToArray(ListSort(StructKeyList(getConfig().entities),'textnocase'));
	...
	function clientconfig(siteid){
		...
		if(isJSON(httpRequestData.content)){
			...
			$.siteConfig().saveClientConfigJSON(httpRequestData.content);
		}
		return $.siteConfig().readClientConfigJSON();
	}
