...
<cfcomponent extends="mura.baseobject" output="false" hint="This provides access the global configuration">
	...
	<cfset variables.autoupdateurl="https://github.com/murasoftware/mura/archive/master.zip"/>
	<cfset variables.instance.version="10"/>
	<cfset variables.instance.title="Mura"/>
	<cfset variables.instance.projectname="Mura"/>
	<cfset variables.instance.projectname="Mura"/>
	<cfset variables.instance.webroot=""/>
	<cfset variables.instance.webrootmap="muraWRM"/>
	<cfset variables.instance.mapdir="mura"/>
	<cfset variables.instance.datasource=""/>
	<cfset variables.instance.defaultthemeurl="https://github.com/murasoftware/mura-bootstrap4/archive/master.zip">
	<cfset variables.instance.stub=""/>
	<cfset variables.instance.cacheImageFileLookups=false/>
	...
	<cfset variables.instance.pluralimagenames=true/>
	...
	<cfset variables.instance.objectlayoutuom="%,px,rem,em">
	<cfset variables.instance.objectlayoutuomext="%,px,rem,em,vw,vh">
	<cfset variables.instance.preferreduom="px">
	...
	<cfset variables.instance.rsscollections=false>
	<cfset variables.instance.individualsitebuilds=false>
	<cfset variables.instance.sitelevelmodules=true>
	...
	<cffunction name="set" output="true">
		...
		<cfif isDefined('arguments.config.fileDir')>
			<cfset arguments.config.assetDir=arguments.config.fileDir>
		</cfif>
		...
		<cfif isDefined('arguments.config.s3assets') and len(arguments.config.s3assets) and listFirst(arguments.config.fileDir,":") neq "s3">
		...
		<cfif not structKeyExists(request.muraORMchecked,'#checkKey#')>
			...
			<cftry>
				...
				<cfloop condition="structKeyExists(levelObj,'extends')">
					...
					<cfif listFindNoCase('beanORM,beanORMVersioned,proxyDecoratorBean',listLast(levelObj.fullname,'.'))>
						<cfset isORM=true>
						<cfbreak>
					</cfif>
				...
				<cfif isORM and isdefined('entity.registerAsEntity')>

					<cfset entity.registerAsEntity()>

					<cfif checkSchema && not entity.getIsProxiedEntity()>
				...
				<cfcatch>
					<cfif listFindNoCase(arguments.componentPath, "dynamic_entities",".")>
				...
				<cfelseif listLast(rs.name,'.') eq 'cfc'>
					...
					<cftry>
						...
						<cfcatch>
							...
							<cfset writeLog(type="Error", log="exception", text="Error Registering Handler #package#.#beanName#")>
							<cfset logError(cfcatch)>
