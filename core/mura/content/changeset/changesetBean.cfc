...
component extends="mura.bean.bean" entityName="changeset" table="tchangesets" output="false" hint="This provides changeset bean functionality" {
	...
	remote function refresh(siteid,$=''){
		if(isSimpleValue(arguments.$)){
			arguments.$=getBean('$').init(arguments.siteid);
		}
		request.muraChangesetPreviewToolbar=false;
		getBean('contentServer').processChangesetStatus(arguments.$.event());
		return true;
	}
	...
	remote function status(){
		...
		param name="previewData.changesetidlist" default='';
		param name="previewData.previewmap" default={};
		param name="previewData.siteid" default=arguments.siteid;

		return {
			changesetids=listToArray(previewData.changesetidlist),
			previewmap=previewData.previewmap,
			siteid=previewData.siteid
		};
	}

	remote function apply(siteid,changesetid='',append=true,$=''){
		if(isSimpleValue(arguments.$)){
			arguments.$=getBean('$').init(arguments.siteid);
		}
		if(len(arguments.changesetid)){
			url.changesetid=arguments.changesetid;
			url.append=arguments.append;
		}
		refresh(argumentCollection=arguments);
		return true;
	}
	...
	public function hasAttachedExperience() {
		return (this.getServiceFactory().containsBean('marketingManager') && getExperienceDetails().hasNext());
	}

	public function getExperienceDetails() {
		return getBean('experienceDetail').getFeed().where().prop('changesetid').isEQ(get('changesetid')).getIterator();
	}
