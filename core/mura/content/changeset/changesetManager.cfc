<cfcomponent extends="mura.baseobject" output="false" hint="This provides service level logic functionality">
	...
	<cffunction name="getDBUtility" output="false">
		<cfif not isObject(variables.dbUtility)>
			<cfset variables.dbUtility=getBean('dbUtility')>
		</cfif>
		<cfreturn variables.dbUtility>
	</cffunction>
	...
	<cffunction name="getDescriptionColumnName" output="false">
		<cfreturn getDBUtility().getEscapedColumnName('description')>
	</cffunction>

	<cffunction name="getNameColumnName" output="false">
		<cfreturn getDBUtility().getEscapedColumnName('name')>
	</cffunction>
	...
	<cffunction name="read" output="false">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rs')#">
			select changesetID, siteID, #getNameColumnName()#, #getDescriptionColumnName()#, created, publishDate, published, lastupdate, lastUpdateBy, lastUpdateByID, remoteID, remotePubDate, remoteSourceURL, closeDate
	...
	<cffunction name="save" output="false">
		...
		<cfif rs.recordcount>
			<cfquery>
				...
				#getNameColumnName()#=<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.bean.getName()#">,
				#getDescriptionColumnName()#=<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.bean.getDescription()#">,
		...
		<cfelse>
			<cfquery>
				insert into tchangesets (changesetID, siteID, #getNameColumnName()#, #getDescriptionColumnName()#, created, publishDate,
	...
	<cffunction name="getQuery" output="false">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsChangeSets')#">
			select changesetID, siteID, #getNameColumnName()#, #getDescriptionColumnName()#, created, publishDate, published, lastupdate, lastUpdateBy, remoteID, remotePubDate, remoteSourceURL
			from tchangesets
			...
			and (
				...
				#getDescriptionColumnName()# like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#arguments.keywords#%">
	...
	<cffunction name="removeSessionPreviewData" output="false">
		...
		<cfif not isDefined('arguments.changesetid') or not len(arguments.changesetid)>
			<cfset local.currentUser.setValue("ChangesetPreviewData","")>
			<cfset request.muraChangesetPreviewToolbar=false>
			<cfset request.muraChangesetPreview=false>
			<cfset request.muraOutputCacheOffset=''>
			<cfset request.muraOutputCacheOffsetAdjusted=true>
			
		<cfelse>
			<cfif isStruct(local.currentUser.getValue("ChangesetPreviewData"))>
				<cfset local.data=local.currentUser.getValue("ChangesetPreviewData")>
				
				<cfparam name="local.data.changesetIDList" default="">
				
				<cfif not listFindNoCase(local.data.changesetIDList,arguments.changesetid)>
					<cfreturn false>
				</cfif>

				<cfset changesetIDList=local.data.changesetIDList>

				<cfset local.currentUser.setValue("ChangesetPreviewData","")>
				<cfset request.muraOutputCacheOffset=''>
				<cfset request.muraOutputCacheOffsetAdjusted=true>

				<cfif changesetIDList eq arguments.changesetid>
					<cfset request.muraChangesetPreviewToolbar=false>
					<cfset request.muraChangesetPreview=false>
					<cfreturn true>
				</cfif>
				...
				<cfloop list="#changesetIDList#" item="local.i">
					<cfif local.i neq arguments.changesetid>
						<cfset setSessionPreviewData(changesetid=local.i,append=true,showToolBar=request.muraChangesetPreviewToolbar)>
					</cfif>
				</cfloop>
				<cfreturn true>
			</cfif>
	...
	<cffunction name="setSessionPreviewData" output="false">
		...
		<cfif not local.changeset.getIsNew() and not local.changeset.getPublished()>
			...
			<cfif arguments.append and isStruct(local.currentUser.getValue("ChangesetPreviewData"))>
				...
				<cfset request.muraOutputCacheOffset=hash(local.data.changesetIDList)>
				<cfset request.muraOutputCacheOffsetAdjusted=true>
			<cfelse>
				...
				<cfset request.muraOutputCacheOffset=hash(data.changesetIDList)>
				<cfset request.muraOutputCacheOffsetAdjusted=true>
			</cfif>
			...
			<cfif isDate(local.changeset.getPublishDate())>
				...
				and (tchangesets.publishDate <= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#local.changeset.getPublishDate()#">
					<cfif arguments.append and isDefined('local.data.prereqList') and len(local.data.prereqList)>
					...
					<cfif local.assignments.recordcount>
						<cfloop query="local.assignments">
							...
							<cfif len(local.assignments.urltitle)>
								<cfset local.data.lookupMap[hash('urltitle' & local.assignments.urltitle)]=	local.assignments.contentHistID>
							</cfif>

							<cfif len(local.assignments.title)>
								<cfset local.data.lookupMap[hash('title' & local.assignments.title)]=	local.assignments.contentHistID>
							</cfif>

							<cfif len(local.assignments.remoteid)>
								<cfset local.data.lookupMap[hash('remoteid' & local.assignments.remoteid)]=	local.assignments.contentHistID>
							</cfif>

							<cfif not listFindNoCase('Component,Form,Variation',local.assignments.type) and len(local.assignments.filename)>
								<cfset local.data.lookupMap[hash('filename' & local.assignments.filename)]=	local.assignments.contentHistID>
							</cfif>
	...
	<cffunction name="publish" output="false">
		...
		<cfif changeset.exists() and not (changeset.hasPendingApprovals() or changeset.hasAttachedExperience())>
			...
			<cfset var pluginEvent=new mura.MuraScope(EventArgs)>
