<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides content utility methods">
	...
	<cffunction name="getRestrictGroups" output="false">
		...
		<cfargument name="includeAdmin"  type="boolean" default="false" />
		<cfset var rs = "">
		...
		<cfif not arguments.includeAdmin>and groupname <> 'Admin'</cfif>
	...
	<cffunction name="doesFileExist" returntype="boolean" output="false">
			...
			<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rs')#">
				...
				and filename = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.filename#"/> and active=1 and type in ('Folder','Page','Calendar','Gallery','File','Link','Form','File','Component')
	...
	<cffunction name="sendNotices" output="false">
		...
		<cfset var domain=getBean('utility').getRequestHost()>
		...
		<cfif (structKeyExists(data,"CompactDisplay") and data.compactDisplay eq "true")
			...
			<cfif variables.configBean.getSiteIDInURLS()>
				<cfset versionLink='#application.settingsManager.getSite(arguments.data.siteID).getScheme()#://#domain##variables.configBean.getServerPort()##variables.configBean.getContext()#/#arguments.data.siteid#/?contentID=#arguments.contentBean.getContentID()#&previewID=#arguments.contentBean.getContentHistID()#'>
			<cfelse>
				<cfset versionLink='#application.settingsManager.getSite(arguments.data.siteID).getScheme()#://#domain##variables.configBean.getServerPort()##variables.configBean.getContext()#/?contentID=#arguments.contentBean.getContentID()#&previewID=#arguments.contentBean.getContentHistID()#'>
			</cfif>
		<cfelse>
			<cfset versionLink='#application.settingsManager.getSite(arguments.data.siteID).getScheme()#://#domain##variables.configBean.getServerPort()##variables.configBean.getContext()##variables.configBean.getAdminDir()#/?muraAction=cArch.edit&parentid=#arguments.data.parentid#&&topid=#arguments.data.topid#&siteid=#arguments.data.siteid#&contentid=#arguments.contentBean.getcontentid()#&contenthistid=#arguments.contentBean.getcontenthistid()#&moduleid=#arguments.data.moduleid#&type=#arguments.data.type#&ptype=#arguments.data.ptype#'>
		</cfif>
		...
		<cfset historyLink='#application.settingsManager.getSite(arguments.data.siteID).getScheme()#://#domain##variables.configBean.getServerPort()##variables.configBean.getContext()##variables.configBean.getAdminDir()#/?muraAction=cArch.hist&parentid=#arguments.data.parentid#&&topid=#arguments.data.topid#&siteid=#arguments.data.siteid#&contentid=#arguments.contentBean.getcontentid()#&moduleid=#arguments.data.moduleid#&type=#arguments.data.type#'>
	...
	#site.getScheme()#://#domain##site.getServerPort()##site.getContext()##site.getContentRenderer().getURLStem(arguments.contentBean.getSiteID(),historyID)#
	<cfif sendVersionLink>
	VIEW EDITED VERSION:
	#site.getScheme()#://#domain##site.getServerPort()##site.getContext()##site.getContentRenderer().getURLStem(arguments.contentBean.getSiteID(),versionID)#
	...
	<cffunction name="formatFilename" output="false">
		...
		<cfelse>
			...
			<cfset lastCharacter = right(arguments.filename,1) />

			<cfif lastCharacter EQ '-' >
	...
	<cffunction name="isOnDisplay" output="false">
				...
				<cfif request.muraChangesetPreview>
					...
					<cfif isStruct(previewData) and structKeyExists(previewData,'publishDate')>
		...
		<cfif len(arguments.find) and arguments.find neq "/">
			...
			<cfquery name="rs">
				...
				and (
					...
					or body like <cfqueryparam value="%#jsonFind#%" cfsqltype="cf_sql_varchar"> 
				)
			</cfquery>

			<cfloop query="rs">
				<cfset newbody = replaceNoCase(BODY,"#arguments.find#","#arguments.replace#","ALL")>
				<cfset newbody = replaceNoCase(BODY,"#jsonFind#","#jsonReplace#","ALL")>
				<cfquery>
			...
			<cfquery name="rs">
				...
				and type in ('Page','Calendar','Folder','Gallery','Link','Component','Form','Form')
			</cfquery>
			...
			<cfquery name="rs">
				...
				where siteid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.siteID#"/>
				and params like <cfqueryparam value="%#jsonFind#%" cfsqltype="cf_sql_varchar">
			</cfquery>
		
			<cfloop query="rs">
				<cfset newbody = replaceNoCase(params,"#jsonFind#","#jsonReplace#","ALL")>
				<cfquery>
				update tcontentobjects set params=<cfqueryparam value="#newbody#" cfsqltype="cf_sql_varchar">
				where  1=1
				<cfloop list="contenthistid,objectid,object" index="local.i">
				 and #local.i# = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rs[local.i]#"/>
				</cfloop>
				<cfloop list="orderno,columnid" index="local.i">
				 and #local.i# = <cfqueryparam cfsqltype="cf_sql_numeric" value="#rs[local.i]#"/>
				</cfloop>
				</cfquery>
			</cfloop>
			<!---
			...
			<cfquery name="rs">
				...
				and (
					...
					or tclassextenddata.attributevalue like <cfqueryparam value="%#jsonFind#%" cfsqltype="cf_sql_varchar">
				)
			</cfquery>
			...
			<cfloop query="rs">
				...
				<cfset newAttributeValue = replaceNoCase(rs.attributeValue,"#jsonFind#","#jsonReplace#","ALL")>
				<cfset newStringValue=replaceNoCase(rs.stringvalue,"#arguments.find#","#arguments.replace#","ALL")>
				<cfset newStringValue=replaceNoCase(rs.stringvalue,"#jsonFind#","#jsonReplace#","ALL")>
