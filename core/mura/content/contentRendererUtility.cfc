<cfcomponent extends="mura.baseobject" output="false" hint="This provides content rendering utiility methods">
	...
	<cffunction name="getCurrentQueryString" output="false">
		...
		<cfset var qrystr=''>
		<cfset var item = "" />
		<cfset var _filterVarsList='PAGE,FIELDS,ISEDITROUTE,EXPAND,EXPANDDEPTH,CACHEDWITHIN,CACHEKEY,RENDERMODE,EDITLAYOUT,ENTITYNAME,METHOD,NOCACHE,PATH,DELETECOMMENTID,APPROVEDCOMMENTID,LOADLIST,INIT,SITEID,DISPLAY,#ucase(application.appReloadKey)#,#filterVars#'>
	...
	<cffunction name="getCurrentURL" output="false">
		...
		<cfif not isDefined('request.servletEvent')>
			<cfreturn "./">
		</cfif>
		...
		<cfset var qrystr=arguments.renderer.getCurrentQueryString(argumentCollection=arguments)>
		
		<cfset var indexFile=listLast(cgi.path_info,"/")>
		<cfif not find('.',indexFile) or indexFile eq "index.cfm">
			<cfset indexFile="">
		</cfif>
		
		<cfif arguments.complete>
			<cfreturn arguments.renderer.getMuraScope().siteConfig().getWebPath(secure=application.utility.isHTTPS(),complete=1) & arguments.renderer.getURLStem(request.servletEvent.getValue('siteID'),request.servletEvent.getValue('currentFilename')) & indexFile & qrystr>
		<cfelse>
			<cfreturn arguments.renderer.getMuraScope().siteConfig('context') & arguments.renderer.getURLStem(request.servletEvent.getValue('siteID'),request.servletEvent.getValue('currentFilename'))  & indexFile & qrystr >
		</cfif>
	...
	<cffunction name="renderObjectInManager" output="false">
		...
		<cfloop collection="#arguments.objectparams#" item="local.i">
			<cfif len(local.i) and not listFindNoCase('runtime,object,objectid,instanceid',local.i) and not isNull(arguments.objectparams[local.i])>
		...
		<cfif $.siteConfig().hasDisplayObject(arguments.object)>
			...
			<cfelse>
				...
				<cfif listLast(filePath,".") neq "cfm">
					...
					<cfset var eventHandler=new "#componentPath#"()>
	...
	<cffunction name="dspObjects" output="false">
		...
		<cfset var tracePoint=initTracePoint("Render Region: {regionid:#columnID#}")>
		<cfset var event=arguments.renderer.getEvent()>
		<cfset var $=arguments.renderer.getMuraScope()>
		<cfset var rsObjects="">
		...
		<cfif perm and arguments.renderer.getShowToolbar() and arguments.renderer.showInlineEditor>
			...
			<cfset theRegion.name=regionLabel>
			<cfset theRegion.local.perm=perm>
			...
			<cfif arguments.returnFormat neq 'array'>
				...
				<cfset theRegion.inherited.header='<div class="mura-region-inherited" data-perm="#perm#"><div class="frontEndToolsModal mura"><span class="mura-edit-label mi-lock">#regionLabel#: #esapiEncode('html_attr',application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.inherited'))#</span></div>'>
		...
		<cfelseif arguments.returnFormat neq 'array'>
		...
		<cfif listFindNoCase('login,create-profile',event.getValue('contentBean').getURLTitle()) or (((not event.getValue('r').restrict)
	...
	<cffunction name="createHREF" output="false">
		...
		<cfif not arguments.secure>
			<cfset arguments.secure=application.utility.isHTTPS()>
		</cfif>
		...
		<cfif arguments.useEditRoute>
			<cfif renderer.hasEditRoute()>
				<cfset begin=begin &renderer.editroute>
			</cfif>
		</cfif>
	
		<cfswitch expression="#arguments.type#">
			<cfcase value="Variation">
				<cfset href=arguments.bean.getRemoteURL() & arguments.querystring>
			</cfcase>
			<cfcase value="Link,File">
				<cfif not request.muraExportHTML>
					<cfif arguments.renderer.directLinks and arguments.type eq "Link">
						<cfscript>
							var linkurl=arguments.bean.getBody();
							var curqs=arguments.queryString;
	...
	<cffunction name="addToHTMLBodyStartQueue" output="false">
		...
		<cfargument name="renderer">
		<cfset var event=arguments.renderer.getEvent()>
		<cfset var q=event.getValue(property='HTMLBodyStartQueue',defaultValue=[])>
		<cfif not arrayFind(q,arguments.text)>
			<cfif arguments.action eq "append">
				<cfset arrayAppend(q,arguments.text)>
			<cfelse>
			...
			<cfset event.setValue('HTMLBodyStartQueue',q) />
		</cfif>
	</cffunction>

	<cffunction name="renderObjectCustomOption" output="false">
		<cfargument name="object">
		...
		<cfargument name="hasModuleTemplateAccess" default="false">

		<cfset var moduleDeleteIcon = "" />

		<cfif hasModuleTemplateAccess>
			<cfset moduleDeleteIcon = '<i class="mi-trash module-delete-icon"></i>' />
		</cfif>

		<cfif not isDefined('arguments.objectlabel')>
			<cfset arguments.objectlabel=arguments.objectname>
		</cfif>
		<cfreturn '<div class="mura-sidebar__objects-list__object-item mura-objectclass mura-customtemplate" data-params="#esapiEncode('html_attr',arguments.params)#" data-object="#esapiEncode('html_attr',arguments.object)#" data-objectid="#esapiEncode('html_attr',arguments.objectid)#" data-objectname="#esapiEncode('html_attr',arguments.objectname)#" data-filtername="#esapiEncode('html_attr',lcase(arguments.objectlabel))#" data-objecticonclass="#esapiEncode('html_attr',arguments.objecticonclass)#"><i class="#esapiEncode('html_attr',arguments.objecticonclass)#"></i>#esapiEncode('html',arguments.objectlabel)##moduleDeleteIcon#</div>'>
	</cffunction>
	...
	<cffunction name="renderObjectClassOption" output="false">
		...
		<cfreturn '<div class="mura-sidebar__objects-list__object-item mura-objectclass" data-object="#esapiEncode('html_attr',arguments.object)#" data-objectid="#esapiEncode('html_attr',arguments.objectid)#" data-objectname="#esapiEncode('html_attr',arguments.objectname)#" data-filtername="#esapiEncode('html_attr',lcase(arguments.objectname))#" data-objecticonclass="#esapiEncode('html_attr',arguments.objecticonclass)#"><i class="#esapiEncode('html_attr',arguments.objecticonclass)#"></i> #esapiEncode('html',arguments.objectlabel)#</div>'>
	</cffunction>
