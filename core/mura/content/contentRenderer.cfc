<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides core rendering functionality">
	...
	<cfset this.showMetaList="jpg,jpeg,png,gif,svg,webp">
	<cfset this.imageInList="jpg,jpeg,png,gif,svg,webp">
	<cfset this.defaultCollectionDisplayList="Date,Title,Image,Summary,Credits"/>
	<cfset this.defaultInheritedRegions=""/>
	<cfset this.directImages=true/>
	<cfset this.directLinks=true/>
	...
	<cfset this.collectionDefaultLayout="default">
	<cfset this.SSR=true>
	...
	<cfset this.subHead1="h1"/>
	<cfset this.subHead2="h2"/>
	<cfset this.subHead3="h3"/>
	<cfset this.subHead4="h4">
	<!--- This is duplicated for when the page title gets set to h1 --->
	<cfset this.subHead5="h5">
	...
	<cfset this.formFilePlaceholder="Select File">
	...
	<cffunction name="init" output="false">
		...
		<cfargument name="preinit" required="true" default="false">

		<cfif arguments.preInit>
			<cfreturn this>
		</cfif>
		...
		<cfelse>
			<cfset variables.event=new mura.servletEvent()>
		</cfif>
		...
		<cfif not isDefined('this.editroute')>
			<cfset this.editroute=getConfigBean().getEditRoute() />
		</cfif>
	...
	<cffunction name="getClientRenderVariables" output="false">
		<cfreturn {
			...
			formFilePlaceholder=this.formFilePlaceholder
		}>
	...
	<cffunction name="postMergeInit" output="false">
		<cfscript>
			...
			if(isValid('variableName',variables.event.getValue('siteID'))){
				if(isDefined('application.muraClientConfig.sites.#variables.event.getValue('siteID')#.rendererProperties')){
					renderProps=application.muraClientConfig.sites[variables.event.getValue('siteID')].rendererProperties;
					if(isStruct(renderProps)){
						for ( var key in renderProps ) {
							this.injectMethod('#key#',renderProps[key]);
						}
				...
				renderProps=getBean('settingsManager').getSite(variables.event.getValue('siteID')).getClientRenderProps();
				if(isStruct(renderProps)){
					for ( var key in renderProps ) {
						this.injectMethod('#key#',renderProps[key]);
					}
	...
	<cffunction name="hasEditRoute" output="false">
		<cfreturn len(this.editroute)>
	</cffunction>
	...
	<cffunction name="dspSequentialNav" output="false">
		...
		<cfif rsSection.recordcount and ((variables.event.getValue('r').restrict and variables.event.getValue('r').allow) or (not variables.event.getValue('r').restrict))>
			...
			<li ><a #iif(len(this.aCurrentClass),de(' class="#this.aCurrentClass#"'),de(""))# href="/#esapiEncode('html_Attr',rsSection.filename[prev])#/">&laquo; #getSite().getRBFactory().getKey("sitemanager.prev")#</a></li>
			</cfif>
			<cfloop query="rsSection">
			<cfsilent>
			...
			<li><a #iif(len(this.aCurrentClass),de(' class="#this.aCurrentClass#"'),de(""))# href="/#esapiEncode('html_Attr',rsSection.filename[next])#/">#getSite().getRBFactory().getKey("sitemanager.next")# &raquo;</a></li>
			</cfif>
			</ul></cfoutput>
			</cfsavecontent>
		</cfif>
	...
	<cffunction name="dspObject_Include" output="false">
		...
		<cfparam name="objectparams.object" default="#arguments.object#">
		<cfparam name="objectparams.objectname" default="#arguments.objectname#">
		<cfparam name="objectparams.objectid" default="#arguments.objectid#">
					...
					<cfelse>
						...
						<cfcase value="File">
							<cfif variables.event.getValue('contentBean').getContentType() eq "Image"
								and listFind("jpg,jpeg,gif,png,webp",lcase(variables.event.getValue('contentBean').getFileExt()))>
	...
	<cffunction name="dspContentTypeBody" output="false">
		...
		<!-- this is to protect against a possible issue with redis cache returning empty structs as arrays-->
	...
	<cffunction name="jsonencode" output="false">
		...
		<cfreturn new mura.json().jsonencode(arguments.arg)>
	</cffunction>

	<cffunction name="getCurrentQueryString" output="false">
		...
		<cfset arguments.renderer=this>
		<cfreturn variables.contentRendererUtility.getCurrentQueryString(argumentCollection=arguments)>
	</cffunction>
	...
	<cffunction name="getCurrentURL" output="false">
		...
		<cfif not len(arguments.domain)>
			<cfset arguments.domain=getBean('utility').getRequestHost()>
		</cfif>
		<cfset arguments.renderer=this>
	...
	<cffunction name="addToHTMLBodyStartQueue" output="false">
		<cfargument name="text">
		<cfargument name="action" default="append">
		<cfset arguments.renderer=this>
		<cfset variables.contentRendererUtility.addToHTMLBodyStartQueue(argumentCollection=arguments)>
	</cffunction>
	...
	<cffunction name="getShowToolbar" output="false">
		...
		<cfelse>
			...
			<cfset var showSetPreview=(
					request.muraChangesetPreviewToolbar
				and (
					this.showMemberToolBar or this.showAdminToolBar
				)
			)>
			<cfset var showLoggedInUserToolbar=(
				(
					this.showMemberToolBar and not currentUser.isSystemUser() or this.showAdminToolBar and currentUser.isSystemUser()
				) and (
					getBean('permUtility').getModulePerm("00000000000000000000000000000000000",variables.event.getValue('siteID'))
					or currentUser.isSuperUser()
				)
			)>			

			<cfset var result=this.enableFrontEndTools
				and (
					showSetPreview or showLoggedInUserToolbar
		...
		<cfif getRenderHTMLQueues()>
			...
			<cfelseif arguments.queueType eq "FOOT">
				<cfif (getShowModal() or variables.event.getValue("muraChangesetPreviewToolbar")) and not request.muraExportHTML>
					<cfif getShowModal()>
						<cfset tracePoint=initTracePoint("/#application.configBean.getWebRootMap()##application.configBean.getAdminDir()#/core/utilities/frontendtools/ui/index.cfm")>
						<cfsavecontent variable="headerStr"><cfinclude template="/#application.configBean.getWebRootMap()##application.configBean.getAdminDir()#/core/utilities/frontendtools/ui/index.cfm"></cfsavecontent>
						<cfset commitTracePoint(tracePoint)>
					</cfif>
				</cfif>
			</cfif>
	...
	<cffunction name="renderHTMLBodyStartQueue" output="false" hint="deprecated">
		<cfreturn renderHTMLQueue("BodyStart")>
	</cffunction>
	...
	<cffunction name="redirect" output="false">
		...
		<cfif request.muraApiRequest || (isDefined('request.returnFormat') && request.returnFormat=='JSON')>
			<cfset request.muraJSONRedirectURL=arguments.location>
			<cfset request.muraJSONRedirectStatusCode=arguments.statusCode>
		<cfelse>
	...
	<cffunction name="getURLForFile" output="false">
		...
		<cfscript>
			...
			if ( not rsFileData.recordcount ) {
				...
				return '#application.configBean.getContext()##application.configBean.getIndexPath()#/_api/render/file/?method=#arguments.method#&amp;fileID=#arguments.fileid#';
			}
		</cfscript>
	</cffunction>
	...
	<cfscript>
		...
		public function outputMuraCSS(includeskin=true, version='10', complete=false, useProtocol=true) {
			if(listFind("7.2,8",arguments.version)){
				arguments.version='10';
			}
		...
		function getLayoutOptions(){
			if (isdefined('this.layoutOptions') && isArray(this.layoutOptions)){
