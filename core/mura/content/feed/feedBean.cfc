...
component extends="mura.bean.beanFeed" entityName="feed" table="tcontentfeeds" output="false" hint="This provides content feed bean functionality" {
	...
	public function addParam(required string field="", required string relationship="and", required string criteria="", required string condition="EQUALS", required string datatype="") output=false {
		...
		if ( !len(arguments.dataType) ) {
			...
			if(listLen(arguments.field,'.')==2){
				var tempEntity=listFirst(arguments.field,'.');
				...
				if(entity.hasTable() && entity.getTable()==tempEntity){
					...
					if(entity.hasSynthedFn('get#tempEntity#')){
						tempEntity=entity.getSynthedFn('get#tempEntity#').args.prop;
					}
	...
	public function getQuery(required aggregation="false", required applyPermFilter="false", countOnly="false", menuType="default", required from="", required to="", required cachedWithin="#variables.instance.cachedWithin#", required boolean allowPublicOffLine=false) output=false {
	...
	public function getIterator(required aggregation="false", required applyPermFilter="false", required from="", required to="", required cachedWithin="#variables.instance.cachedWithin#", countOnly="false", menuType="default", required boolean allowPublicOffLine=false) output=false {
	...
	public function getAvailableDisplayList(listCol="attribute") output=false {
		...
		var SSR=getBean('settingsManager').getSite(siteid).getContentRenderer().SSR;
		if(!SSR){
			var attrList="Date,Title,Image,Summary,Body,ReadMore,Credits,Tags,Type,SubType";
			var labelList="Date,Title,Image,Summary,Body,Read More,Credits,Tags,Type,SubType";
		} else {
			var attrList="Date,Title,Image,Summary,Body,ReadMore,Credits,Tags,Type,SubType";
			var labelList="Date,Title,Image,Summary,Body,Read More,Credits,Tags,Type,SubType";
		}
	...
	public function getRemoteData() output=false {
		return getBean('feedUtility').getRemoteFeedData(feedURL=variables.instance.channellink,maxItems=variables.instance.maxitems,authtype=variables.instance.authtype,siteid=variables.instance.siteid);
	}
