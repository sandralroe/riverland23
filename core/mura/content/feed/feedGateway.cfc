<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides content feed gateway queries">
	...
	<cffunction name="getFeed" output="false">
		...
		<cfargument name="allowPublicOffLine" required="true" type="boolean" default="false">
		<cfset var c ="" />
		<cfset var rsFeed ="" />
		...
		<cfset var searchText=arguments.feedBean.getSearchText()>
		<cfset var rsParams=arguments.feedBean.getAdvancedParams() />
		<cfset var started =false />
		<cfset var isNullVal=false>
		<cfset var param =new mura.queryParam()/>
		...
		<cfset var castdatatype="varchar">
		<cfset var palias="">
		<cfset var talias="">
		...
		<cfif arguments.feedBean.exists()>
			<cfset var tracePoint=initTracePoint(detail="Loading content query: #arguments.feedBean.getFeedID()#")>
		<cfelse>
			<cfset var tracePoint=initTracePoint(detail="Loading content query")>
		</cfif>
		...
		<cfloop query="rsParams">
			<cfif listLen(rsParams.field,".") eq 2 and not find('$',rsParams.field)>
				...
				<cfif not listFindNoCase("tcontent,tcontentstats,tfiles,tparent,tcontentcategoryassign,tcontenttags,tcontentcategories",jointable) and not listFind(jointables,jointable)>
					...
					<cfif getServiceFactory().containsBean(jointable)>
						<cfset jointable=getBean(jointable).getTable() & ' ' & originalJoinTable>
					</cfif>
					<cfif len(originalJoinTable) and originalJoinTable neq 'tcontent' and not listFind(jointables,jointable)>
			...
			<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsFeed',blockFactor=blockFactor,cachedWithin=arguments.feedBean.getCachedWithin())#">
				...
				SELECT
					...
					<cfif not arguments.countOnly>
						...
						<cfelse>
							...
							tcontent.remotesource, tcontent.remoteURL,tcontent.remotesourceURL, tcontent.keypoints, tcontent.remoteid,
							tcontent.contentID, tcontent.parentID, tcontent.active, tcontent.approved, tcontent.isLocked, tcontent.contentHistID,tcontent.moduleid,
							tcontent.target, tcontent.targetParams,
							tcontent.releaseDate, tcontent.lastupdate,tcontent.summary,tcontent.istemplate,
							tfiles.fileSize,tfiles.fileExt,tfiles.credits as assocFileCredits ,tcontent.fileid,
							...
							tfiles.filename as AssocFilename,tcontent.displayInterval,tcontent.display,tcontent.changesetid,
							tcontent.sortBy,tcontent.sortDirection,
				...
				FROM
					...
					<cfloop list="#jointables#" index="jointable">
						...
						<cfif arrayLen(arguments.feedBean.getJoins())>
							<cfloop from="1" to="#arrayLen(local.specifiedjoins)#" index="local.i">
								<cfif not len(local.specifiedjoins[local.i].tableAlias) and local.specifiedjoins[local.i].table eq listFirst(jointable,' ') or local.specifiedjoins[local.i].tableAlias eq listFirst(jointable," ")>
									<cfset started=true>
						...
						<cfif not started>
							<cfif listFindNoCase(histtables,jointable)>
								inner join #jointable# #tableModifier# on (tcontent.contenthistid=#listLast(jointable, ' ')#.contenthistid)
							<cfelse>
								inner join #jointable# #tableModifier# on (tcontent.contentid=#listLast(jointable, ' ')#.contentid)
							</cfif>
						</cfif>
					</cfloop>
					...
					<cfloop from="1" to="#arrayLen(local.specifiedjoins)#" index="local.i">
						<cfif len(local.specifiedjoins[local.i].clause)>
							#local.specifiedjoins[local.i].jointype# join #local.specifiedjoins[local.i].table# <cfif len(local.specifiedjoins[local.i].tableAlias)>#local.specifiedjoins[local.i].tableAlias#</cfif> #tableModifier# 
							on (
								#local.specifiedjoins[local.i].clause#
								<cfif local.specifiedjoins[local.i].table eq 'tcontent' and local.specifiedjoins[local.i].loadkey eq 'contentid'>and #local.specifiedjoins[local.i].tableAlias#.active=1</cfif>
							)
						</cfif>
					</cfloop>
				...
				WHERE
					...
					<cfif arguments.feedBean.getType() eq "Remote">
						<cfthrow message="#esapiEncode('html_attr',application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.thisfunction'))#">
						...
						<cfloop query="rsParams">
							...
							<cfif param.getIsValid()>
								...
								<cfif listLen(param.getField(),".") gt 1>
									...
									<cfelse>
											<cfif param.getColumn() eq 'tcontent.body'>
												<cfset var jsonParam=contentGateway.serializeJSONParam(param.getCriteria())>
												(
											</cfif>
												#param.getFieldStatement()#
											...
											<cfif param.getColumn() eq 'tcontent.body'>
												or #param.getFieldStatement()#
													#param.getCondition()#
													<cfif isListParam>(</cfif>
													<cfqueryparam cfsqltype="cf_sql_#param.getDataType()#" value="#contentGateway.renderTextParamValue(jsonParam)#" list="#iif(isListParam,de('true'),de('false'))#" null="#iif(contentGateway.renderTextParamValue(jsonParam) eq 'null',de('true'),de('false'))#">
													<cfif isListParam>)</cfif>
												<!--- having a length or 3 or more characters makes it worth checking content obects--->
												<cfif len(jsonParam) gt 2>
													or tcontent.contenthistid in (
														select tcontentobjects.contenthistid from tcontentobjects
														where tcontentobjects.siteid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.feedBean.getContentPoolID()#"/>
														and #contentGateway.renderTextParamColumn('tcontentobjects.params')# 
														#param.getCondition()# 
														<cfif isListParam>(</cfif>
														<cfqueryparam cfsqltype="cf_sql_#param.getDataType()#" value="#contentGateway.renderTextParamValue(jsonParam)#" list="#iif(isListParam,de('true'),de('false'))#" null="#iif(contentGateway.renderTextParamValue(jsonParam) eq 'null',de('true'),de('false'))#">
														<cfif isListParam>)</cfif>
								...
								<cfelseif param.getField() eq 'persona'>
									tcontent.contentHistID IN (
										select mxp_personapoints.contenthistid from mxp_personapoints
										inner join mxp_persona on (mxp_personapoints.personaid=mxp_persona.personaid)
										where mxp_personapoints.points > 0 and
										<cfif param.getCondition() eq 'like' and variables.configBean.getDbCaseSensitive()>
											upper(mxp_persona.name)
										<cfelse>
											mxp_persona.name
										</cfif>
										#param.getCondition()#
										<cfif isListParam>(</cfif>
										<cfqueryparam cfsqltype="cf_sql_varchar" value="#param.getCriteria()#" list="#iif(isListParam,de('true'),de('false'))#">
										<cfif isListParam>)</cfif>
										<cfset openGrouping=false>
									)
								<cfelseif param.getField() eq 'personaid' or param.getField() eq 'stageid'>
									tcontent.contentHistID IN (
										select mxp_personapoints.contenthistid from mxp_personapoints
						
										where mxp_personapoints.points > 0
										and personaid 
										#param.getCondition()#
										<cfif isListParam>(</cfif>
										<cfqueryparam cfsqltype="cf_sql_varchar" value="#param.getCriteria()#" list="#iif(isListParam,de('true'),de('false'))#">
								...
								<cfelseif param.getField() eq 'stage'>
									tcontent.contentHistID IN (
										select mxp_stagepoints.contenthistid from mxp_stagepoints
										inner join mxp_stage on (mxp_stagepoints.stageid=mxp_stage.stageid)
										where  mxp_stagepoints.points > 0 and
										<cfif isNumeric(param.getCriteria())>
											<cfset castfield="orderno">
											<cfset castdatatype="numeric">
										<cfelse>
											<cfset castfield="name">
											<cfset castdatatype="varchar">
										</cfif>
										<cfif castdatatype eq 'varchar' and param.getCondition() eq 'like' and variables.configBean.getDbCaseSensitive()>
											upper(mxp_stage.name)
										<cfelse>
											mxp_stage.#castfield#
										</cfif>
										#param.getCondition()#
										<cfif isListParam>(</cfif>
										<cfqueryparam cfsqltype="#castdatatype#" value="#param.getCriteria()#" list="#iif(isListParam,de('true'),de('false'))#">
										<cfif isListParam>)</cfif>
										<cfset openGrouping=false>
									)
								<cfelseif listFindNoCase('personaid,stageid',param.getField())>
									<cfset castfield=(param.getField() eq 'personaid') ? 'persona' : 'stage'>
									tcontent.contentHistID IN (
										select mxp_#castfield#points.contenthistid from mxp_#castfield#points
										where mxp_#castfield#points.points > 0
										and #castfield#id 
										#param.getCondition()#
										<cfif isListParam>(</cfif>
										<cfqueryparam cfsqltype="cf_sql_varchar" value="#param.getCriteria()#" list="#iif(isListParam,de('true'),de('false'))#">
					...
					<cfif len(searchText) >
						and ( 
							UPPER(tcontent.body) like UPPER(<cfqueryparam cfsqltype="cf_sql_varchar" value="%#searchText#%"/>)
							or UPPER(tcontent.title) like UPPER(<cfqueryparam cfsqltype="cf_sql_varchar" value="%#searchText#%"/>)
						)
					</cfif>
						...
						<cfif arguments.feedBean.getSortBy() neq '' or arguments.feedBean.getOrderBy() neq ''>
							...
							<cfelse>
								<cfswitch expression="#arguments.feedBean.getSortBy()#">
									...
									<cfdefaultcase>
										<cfif mxpRelevanceSort>
											total_points #REReplace(arguments.feedBean.getSortDirection(),"[^A-Za-z]","","all")#, tcontent.releaseDate #REReplace(arguments.feedBean.getSortDirection(),"[^A-Za-z]","","all")#, tcontent.lastUpdate #REReplace(arguments.feedBean.getSortDirection(),"[^A-Za-z]","","all")#
		...
		<cfif not arguments.feedBean.isAggregateQuery()>
			<cfif not arguments.countOnly and arguments.applyPermFilter or not arguments.feedBean.getLiveOnly() and not arguments.allowPublicOffLine>
				<cfset rsFeed=variables.permUtility.queryPermFilter(rawQuery=rsFeed,siteID=arguments.feedBean.getSiteID(), liveOnly=arguments.feedBean.getLiveOnly(), allowPublicOffLine=arguments.allowPublicOffLine)>
			</cfif>
	...
	<cffunction name="renderDateTimeParamType" output="false">
