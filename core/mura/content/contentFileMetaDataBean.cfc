	...
	*/
	...
	function getURLForFile(method='inline'){
		if ( getValue('isNew') ) {
			...
			return '#application.configBean.getContext()##application.configBean.getIndexPath()#/_api/render/file/?method=#arguments.method#&amp;fileID=#getValue('fileid')#';
		}
	}

	function hasImageFileExt(){

		return listFindNoCase("png,jpg,jpeg,svg,gif,webp",getValue('fileExt'));
	}

	function hasCroppableImageFileExt(){

		return listFindNoCase("png,jpg,jpeg,webp",getValue('fileExt'));
	}
