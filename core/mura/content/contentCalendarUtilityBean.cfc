component extends='mura.baseobject' hint="This provides content calendar utility methods" {
  ...
  public any function getCalendarItems(
    ...
    , returnFormat='query'
		, showNavOnly=0
		, showExcludeSearch=0
    , searchtext='#variables.$.event('searchtext')#'
    , useCategoryIntersect='#variables.$.event('useCategoryIntersect')#'
    ...
    local.applyPermFilter=false;
    
    if(len(arguments.calendarid)){
      var calendars= variables.$.getBean('feed')
        .setContentPoolID(variables.$.siteConfig('contentPooliD'))
        .setShowNavOnly(0)
        .setMaxItems(0)
        .setNextN(0)
        .setShowExcludeSearch(1)
        .where().prop('contentid').isIn(arguments.calendarid)
        .getIterator();


      if(calendars.hasNext()){
        while(calendars.hasNext()){
          local.contentBean=calendars.next();
          local.applyPermFilter = local.applyPermFilter || (variables.$.siteConfig('extranet') == 1
          ...
          if ( !(local.contentBean.getIsNew() || local.contentBean.getType() != 'Calendar') ) {
              arrayAppend(allowable,local.contentBean.getContentID());
          }
        }
      }
    }
    ...
    if ( !arrayLen(allowable) ) {
      return QueryNew('url,contentid,menutitle,displaystart,displaystop,allday');
    }
    ...
    // End Date
    if ( !IsDate(arguments.end) || ( IsDate(arguments.end) && Fix(ParseDateTime(arguments.end)) < Fix(ParseDateTime(arguments.start)) )  ) {
    ...
    if ( Len(arguments.categoryid) ) {
      ...
      feed.setUseCategoryIntersect(useCategoryIntersect);
    }

    arguments.searchtext = trim(arguments.searchtext);
    if ( Len(arguments.searchtext) ){
      feed.setSearchText(arguments.searchtext);
    }
    ...
    for ( local.i=1; local.i<=local.rs.recordcount; local.i++ ) {
      // add URL to rs
      local.rs['url'][i] = variables.$.createHref(filename=local.rs['filename'][i],complete=1,siteid=local.rs['siteid'][i]);
