<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides content gateway queries">
	...
	<cffunction name="getCrumblist" output="false">
		...
		<cfif request.muraChangesetPreview>
			<cfset arguments.useCache=false>
		</cfif>
		...
		<cfif not request.muraChangesetPreview and site.getCache()>
			...
			<cfif NOT arguments.useCache or NOT cacheFactory.has( key )>
				<cfif arguments.useCache>
					<cfset commitTracePoint(initTracePoint(detail="DATA CACHE HIT: {class: content crumbdata, key: #key#}"))>
				</cfif>
			...
			<cfelse>
				<cftry>
					...
					<cfif not isArray(crumbdata)>
						<cfset commitTracePoint(initTracePoint(detail="DATA CACHE MISS: {class: content crumbdata, key: #key#}"))>
						<cfset crumbdata=buildCrumblist(contentid=arguments.contentID,siteid=arguments.siteID,path=arguments.path) />
						<cfif arrayLen(crumbdata) and arrayLen(crumbdata) lt 50>
							<cfset crumbdata=cacheFactory.get( key, crumbdata ) />
						</cfif>
	...
	<cffunction name="buildCrumblist" returntype="array" output="false">
		...
		<cfset var DAO=getBean('contentDAO')> 
		...
		<cfif not len(arguments.path)>
			...
			<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsCrumbData')#">
				...
				tcontent.subtype, tcontent.target, tcontent.targetParams, tcontent.remoteid,
				...
				tcontent.sortDirection,tfiles.fileExt,tfiles.credits as assocFileCredits ,tfiles.alttext as assocFileAltText,tapprovalassignments.chainID,tapprovalassignments.exemptID,tcontent.moduleid
				from tcontent
				...
				where 
				tcontent.contentid=<cfqueryparam cfsqltype="cf_sql_varchar" value="#ID#"/>
				and tcontent.siteid= <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.siteID#"/>
				#DAO.renderActiveClause("tcontent",arguments.siteID)#
			</cfquery>
			...
			<cfset crumb.remoteid=rsCrumbData.remoteid />
			...
			<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsCrumbData')#">
				...
				,tfiles.fileExt,tfiles.credits as assocFileCredits ,tfiles.alttext as assocFileAltText,tapprovalassignments.exemptID,tcontent.moduleid
	...
	<cffunction name="getKids" output="false">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsKids')#">
			...
			tcontent.*
			<cfelse>
			tcontent.title, tcontent.releasedate, tcontent.created, tcontent.menuTitle, tcontent.lastupdate,tcontent.summary, tcontent.tags,tcontent.filename, tcontent.type,tcontent.subType, tcontent.siteid,
			tcontent.contentid, tcontent.contentHistID, tcontent.moduleid, tcontent.approved, tcontent.active, tcontent.target, tcontent.targetParams,
			...
			tcontent.fileid, tcontent.credits, tcontent.remoteSource, tcontent.remoteSourceURL, tcontent.remoteURL,tcontent.remoteid,
			tfiles.fileSize,tfiles.fileExt,tfiles.credits as assocFileCredits , tcontent.audience, tcontent.keypoints
			,tcontentstats.rating,tcontentstats.totalVotes,tcontentstats.downVotes,tcontentstats.upVotes
			,tcontentstats.comments, '' as parentType, <cfif doKids> qKids.kids<cfelse>null as kids</cfif>,tcontent.path, tcontent.nextn,
			tcontent.majorVersion, tcontent.minorVersion, tcontentstats.lockID, tcontentstats.lockType, tcontent.expires,
			tfiles.filename as AssocFilename,tcontent.displayInterval,tcontent.display,tcontent.changesetid,tcontent.isTemplate,
			CASE WHEN tcontentfilemetadata.altText is not null THEN tcontentfilemetadata.altText ELSE tfiles.alttext END AS assocFileAltText	
		</cfif>

			<cfif mxpRelevanceSort>
			, (<cfif len(stageid)>stagetotal.stage_points + </cfif>personatotal.persona_points) as total_points
			</cfif>
			...
			<cfswitch expression="#arguments.sortBy#">
				...
				<cfdefaultcase>
					<cfif mxpRelevanceSort>
						total_points #REReplace(arguments.sortDirection,"[^0-9A-Za-z\._,\- ]","","all")#, tcontent.releaseDate #REReplace(arguments.sortDirection,"[^0-9A-Za-z\._,\- ]","","all")#,tcontent.lastUpdate #REReplace(arguments.sortDirection,"[^0-9A-Za-z\._,\- ]","","all")#
	...
	<cffunction name="getDraftList" output="false">
		...
		draft.lastUpdateBy,tfiles.fileExt,  tfiles.credits as assocFileCredits,tfiles.alttext as assocFileAltText,draft.changesetID, draft.majorVersion, draft.minorVersion, tcontentstats.lockID, tcontentstats.lockType, draft.expires
		...
		<cfquery name="rsDraftList" dbtype="query">
		...
		<cfquery name="rsDraftList" dbtype="query" maxrows="#arguments.limit#">
			...
			order by #REReplace(arguments.sortBy,"[^0-9A-Za-z\._,\- ]","","all")#  #REReplace(arguments.sortDirection,"[^0-9A-Za-z\._,\- ]","","all")#
		</cfquery>
	...
	<cffunction name="getApprovals" output="false">
		...
		module.SiteID, draft.SiteID, draft.SiteID, draft.targetparams,draft.lastUpdate,
		draft.lastUpdateBy,tfiles.fileExt, tfiles.credits as assocFileCredits ,tfiles.alttext as assocFileAltText, draft.changesetID, draft.majorVersion, draft.minorVersion, tcontentstats.lockID, tcontentstats.lockType, draft.expires,
		tapprovalrequests.status AS approvalStatus, draft.displayStart, tchangesets.publishDate, tapprovalrequests.chainid, tapprovalrequests.requestid,tapprovalrequests.groupid
		...
		<cfif not getCurrentUser().isAdminUser() and not getCurrentUser().isSuperUser()>
			and tapprovalrequests.groupid in (<cfqueryparam list="true" cfsqltype="cf_sql_varchar" value="#listAppend(arguments.membershipids,getCurrentUser().getValue('userid'))#">)
		</cfif>
		...
		draft.lastUpdateBy,tfiles.fileExt, tfiles.credits ,tfiles.alttext, draft.changesetID, draft.majorVersion, draft.minorVersion, tcontentstats.lockID, tcontentstats.lockType, draft.expires,
		tapprovalrequests.status, draft.displayStart, tchangesets.publishDate, tapprovalrequests.chainid, tapprovalrequests.requestid,tapprovalrequests.groupid
		</cfquery>
	...
	<cffunction name="getNest" output="false">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsNest')#">
			SELECT tcontent.ContentHistID, tcontent.ContentID, tcontent.moduleid, tcontent.remoteid, tcontent.Approved, tcontent.filename, tcontent.Active, tcontent.Type, tcontent.subtype, tcontent.OrderNo, tcontent.ParentID,
			...
			tfiles.fileSize,tfiles.FileExt,tfiles.credits as assocFileCredits ,tfiles.ContentType,tfiles.ContentSubType, tcontent.siteID, tcontent.featureStart,tcontent.featureStop,tcontent.template,tcontent.childTemplate,
			tcontent.majorVersion, tcontent.minorVersion, tcontentstats.lockID, tcontentstats.lockType, tcontent.expires,
			tcontentstats.rating,tcontentstats.totalVotes, tcontentstats.comments,
			tfiles.filename as AssocFilename,tcontent.displayInterval, tcontent.fileid,tcontent.remoteurl,tcontent.isTemplate,
			CASE WHEN tcontentfilemetadata.altText is not null THEN tcontentfilemetadata.altText ELSE tfiles.alttext END AS assocFileAltText
			FROM tcontent
			...
			<cfif arguments.aggregation>
				group by tcontent.ContentHistID, tcontent.ContentID, tcontent.moduleid, tcontent.remoteid, tcontent.Approved, tcontent.filename, tcontent.Active, tcontent.Type, tcontent.subtype, tcontent.OrderNo, tcontent.ParentID,
				tcontent.Title, tcontent.menuTitle, tcontent.lastUpdate, tcontent.lastUpdateBy, tcontent.lastUpdateByID, tcontent.Display, tcontent.DisplayStart,
				...
				tfiles.fileSize,tfiles.FileExt,tfiles.credits,tfiles.alttext,tfiles.ContentType,tfiles.ContentSubType, tcontent.created, tcontent.siteID, tcontent.featureStart,tcontent.featureStop,tcontent.template,tcontent.childTemplate,
				tcontent.majorVersion, tcontent.minorVersion, tcontentstats.lockID, tcontentstats.lockType, tcontent.expires,
	...
	<cffunction name="getTop" output="true">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsTop')#">
			...
			tcontent.isLocked,tcontent.sortBy,tcontent.sortDirection,tcontent.releaseDate,tfiles.fileEXT, tfiles.credits as assocFileCredits ,tfiles.alttext as assocFileAltText, tcontent.featurestart, tcontent.featurestop,tcontent.template,tcontent.childTemplate,
	...
	<cffunction name="getHist" output="false" hint="I get all versions">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsHist')#">
			...
			tcontent.display, tcontent.displaystart, tcontent.displaystop, tcontent.moduleid, tcontent.isnav, tcontent.notes,tcontent.isfeature,tcontent.featurestart,tcontent.featurestop,tcontent.inheritObjects,tcontent.filename,tcontent.target,tcontent.targetParams,tcontent.releaseDate,
	...
	<cffunction name="getPrivateSearch" output="false">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsPrivateSearch',maxrows=1000)#">
			SELECT tcontent.ContentHistID, tcontent.ContentID, tcontent.Approved, tcontent.filename, tcontent.Active, tcontent.Type, tcontent.OrderNo, tcontent.ParentID, tcontent.siteid,
			...
			tcontent.islocked,tcontent.releaseDate,tfiles.fileSize,tfiles.fileExt,tfiles.credits as assocFileCredits ,  2 AS Priority, tcontent.nextn, tfiles.fileid,tcontent.isTemplate,
			tcontent.majorVersion, tcontent.minorVersion, tcontentstats.lockID, tcontentstats.lockType, tcontent.expires,tfiles.filename as assocFilename,
			CASE WHEN tcontentfilemetadata.altText is not null THEN tcontentfilemetadata.altText ELSE tfiles.alttext END AS assocFileAltText,
			...
			tfiles.fileext in ('png','gif','jpg','jpeg','webp','svg') AND
			</cfif>

			<cfif kw neq '' or arguments.tag neq ''  or arguments.categoryid neq ''>
			tcontent.Active = 1
				<cfif listFindNoCase("image,file",arguments.searchType)>
					AND tcontent.siteID in (
							...
							or siteid=<cfqueryparam cfsqltype="cf_sql_varchar" value="#filePoolId#"/>
							or filePoolID=<cfqueryparam cfsqltype="cf_sql_varchar" value="#filePoolId#"/>			
						)
				<cfelse>
					AND tcontent.siteid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.siteID#"/>
				</cfif>
				...
				<cfif len(arguments.categoryid)>
					<cfset var categoryLen=listLen(arguments.categoryid)>
					<cfset var c="">
					AND tcontent.contenthistID in (
						select distinct tcontentcategoryassign.contentHistID from tcontentcategoryassign #tableModifier#
						inner join tcontentcategories #tableModifier#
				...
				GROUP BY tcontent.ContentHistID, tcontent.ContentID, tcontent.Approved, tcontent.filename, tcontent.Active, tcontent.Type, tcontent.OrderNo, tcontent.ParentID, tcontent.siteid,
				tcontent.Title, tcontent.menuTitle, tcontent.lastUpdate, tcontent.lastUpdateBy, tcontent.lastUpdateByID, tcontent.Display, tcontent.DisplayStart, tcontent.subtype,
				tcontent.DisplayStop,  tcontent.isnav, tcontent.restricted,tcontent.isfeature,tcontent.inheritObjects,
				tcontent.target,tcontent.targetParams,tcontent.islocked,tcontent.releaseDate,tfiles.fileSize,tfiles.fileExt,tfiles.credits,tfiles.alttext, tcontent.nextn, tfiles.fileid,tcontent.isTemplate,
				...
				tcontent.Title, tcontent.menuTitle, tcontent.lastUpdate, tcontent.lastUpdateBy, tcontent.lastUpdateByID, tcontent.Display, tcontent.DisplayStart, tcontent.subtype,
				tcontent.DisplayStop,  tcontent.isnav, tcontent.restricted, count(tcontent2.parentid) AS hasKids,tcontent.isfeature,tcontent.inheritObjects,tcontent.target,tcontent.targetParams,
				tcontent.islocked,tcontent.releaseDate,tfiles.fileSize,tfiles.fileExt, tfiles.credits as assocFileCredits , 1 AS Priority, tcontent.nextn, tfiles.fileid,tcontent.isTemplate,
				tcontent.majorVersion, tcontent.minorVersion, tcontentstats.lockID, tcontentstats.lockType, tcontent.expires,tfiles.filename as assocFilename,
				CASE WHEN tcontentfilemetadata.altText is not null THEN tcontentfilemetadata.altText ELSE tfiles.alttext END AS assocFileAltText,
				CASE WHEN tcontent.title = <cfqueryparam cfsqltype="cf_sql_varchar" value="#renderTextParamValue(arguments.keywords)#">
					or tcontent.menuTitle = <cfqueryparam cfsqltype="cf_sql_varchar" value="#renderTextParamValue(arguments.keywords)#"> THEN 0 ELSE 1 END AS superSort
	...
	<cffunction name="getPublicSearch" output="false">
		...
		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='rsPublicSearch',maxrows=1000)#">
			...
			tcontent.remoteURL,tfiles.fileSize,tfiles.fileExt,tfiles.credits as assocFileCredits ,tcontent.fileID,tcontent.audience,tcontent.keyPoints,
		...
		<cfif arguments.applyPermFilter>
			<cfset rsPublicSearch=variables.permUtility.queryPermFilter(rawQuery=rsPublicSearch,siteID=arguments.siteid)>
		</cfif>
	...
	<cffunction  name="getLastHistoryIdByContent" output="false">
		<cfargument name="ContentID" type="string" required="true">
		<cfset var lastHistoryId = ''>
		<cfset var dbType=variables.configBean.getDBType()>

		<cfquery attributeCollection="#variables.configBean.getReadOnlyQRYAttrs(name='lastHistoryId')#">
			<cfif dbType eq "oracle">select * from (</cfif>
			SELECT <cfif dbType eq "mssql">Top 1</cfif> contenthistid
			FROM tcontent
			WHERE contentid = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.ContentID#"/>
			AND istemplate = 1
			AND active = 1
			<cfif listFindNoCase("mysql,postgresql", dbType)>limit 1</cfif>
			<cfif dbType eq "nuodb">fetch 1</cfif>
			<cfif dbType eq "oracle">) where ROWNUM <= 1 </cfif>
		</cfquery>

		<cfreturn lastHistoryId	 />
	</cffunction>
	...
	<cffunction name="getRelatedContent" output="false">
		...
		<cfargument name="applyPermFilter" default="false" />
		...
		<cfif arguments.entitytype eq 'content' or not len(arguments.entitytype)>
			...
			SELECT tcontent.title, tcontent.releasedate, tcontent.created, tcontent.menuTitle, tcontent.lastupdate, tcontent.active, tcontent.approved, tcontent.lastupdatebyid, tcontent.summary, 
			tcontent.filename, tcontent.type, tcontent.contentid,  tcontent.moduleid, tcontent.parentid, tcontent.contenthistid, tcontent.changesetid, tcontent.tags,
			...
			tcontent.fileid, tcontent.credits,  tcontent.remoteid, tcontent.remoteSource, tcontent.remoteSourceURL, tcontent.remoteURL, tcontent.subtype,
			tfiles.fileSize,tfiles.fileExt,tfiles.credits as assocFileCredits ,tfiles.alttext as assocFileAltText,tcontent.path, tcontent.siteid, tcr.contentid as relatedFromContentID,
			tcr.relatedContentSetID, tcr.orderNo, tcontent.displayInterval, tcontent.display,tparent.type parentType
			<cfif mxpRelevanceSort>
			, (<cfif len(stageid)>stagetotal.stage_points + </cfif>personatotal.persona_points) as total_points
			</cfif>
			FROM  tcontent Left Join tfiles ON (tcontent.fileID=tfiles.fileID)
			...
			<cfif arguments.applyPermFilter>
				<cfset rsRelatedContent=variables.permUtility.queryPermFilter(rawQuery=rsRelatedContent,siteID=arguments.siteID)>
			</cfif>
	...
	<cffunction name="renderActiveClause" output="false">
	...
	<cffunction name="renderMobileClause" output="false">
		<cfset var sql="">
		<cfsavecontent variable="sql">
		<cfoutput>
	...
	<cffunction name="serializeJSONParam">
		<cfargument name="value">
		<cfset arguments.value=serializeJSON(arguments.value)>
		
		<cfif isNumeric(arguments.value)>
			<cfreturn arguments.value>
		<cfelseif len(arguments.value) lte 2>
			<cfreturn "">
		<cfelse>
			<cfreturn mid(arguments.value,2,len(arguments.value)-2)>
		</cfif>
	</cffunction>
