...
<cfcomponent extends="mura.cfobject" output="false" hint="This provides content service level logic functionality">
	...
	<cffunction name="getContentVersion" output="false">
		...
		<cfset var tracepoint=initTracePoint(detail="Loading contentBean by contenthistid: #arguments.contentHistID#")>
		<cfif site.getCache()>
	...
	<cffunction name="getActiveContentByFilename" output="false">
		...
		<cfset var tracepoint=initTracePoint(detail="Loading contentBean by filename: #arguments.filename#")>
		...
		<cfif len(arguments.filename)>
			...
			<cfif request.muraChangesetPreview and isDefined('sessionData.mura.ChangesetPreviewData.siteid') and sessionData.mura.ChangesetPreviewData.siteid eq arguments.siteid>
				<cfif isDefined('sessionData.mura.ChangesetPreviewData.lookupMap')>
					<cfset var lookupHash=hash('filename' & arguments.filename)>
					...
					<cfif structKeyExists(sessionData.mura.ChangesetPreviewData.lookupMap,'#lookupHash#')>
						...
						<cfif bean.exists()>
							<cfset commitTracePoint(initTracePoint(detail="DATA CHANGESET HIT: {class: contentBean, key: #key#}"))>
							<cfset commitTracePoint(tracepoint)>
							<cfreturn bean>
						<cfelse>
		...
		<cfelse>
			...
			<cfset bean=getActiveContent(argumentCollection=arguments)>
		</cfif>
		<cfset commitTracePoint(tracepoint)>
		<cfreturn bean>
	</cffunction>
	...
	<cffunction name="getActiveByRemoteID" output="false">
		...
		<cfset var useCache=site.getCache()>
		<cfset var sessionData=getSession()>
		<cfset var tracepoint=initTracePoint(detail="Loading contentBean by remoteid: #arguments.remoteid#")>

		<cfif request.muraChangesetPreview and isDefined('sessionData.mura.ChangesetPreviewData.siteid') and sessionData.mura.ChangesetPreviewData.siteid eq arguments.siteid>
			<cfif isDefined('sessionData.mura.ChangesetPreviewData.lookupMap')>
				<cfset var lookupHash=hash('remoteid' & arguments.remoteid)>
	...
	<cffunction name="getActiveByTitle" output="false">
		...
		<cfset var useCache=site.getCache()>
		<cfset var sessionData=getSession()>
		<cfset var tracepoint=initTracePoint(detail="Loading contentBean by title: #arguments.title#")>

		<cfif request.muraChangesetPreview and isDefined('sessionData.mura.ChangesetPreviewData.siteid') and sessionData.mura.ChangesetPreviewData.siteid eq arguments.siteid>
			<cfif isDefined('sessionData.mura.ChangesetPreviewData.lookupMap')>
				<cfset var lookupHash=hash('title' & arguments.title)>
	...
	<cffunction name="getActiveByURLTitle" output="false">
		...
		<cfset var useCache=site.getCache()>
		<cfset var sessionData=getSession()>
		<cfset var tracepoint=initTracePoint(detail="Loading contentBean by urltitle: #arguments.urltitle#")>

		<cfif request.muraChangesetPreview and isDefined('sessionData.mura.ChangesetPreviewData.siteid') and sessionData.mura.ChangesetPreviewData.siteid eq arguments.siteid>
			<cfif isDefined('sessionData.mura.ChangesetPreviewData.lookupMap')>
				<cfset var lookupHash=hash('urltitle' & arguments.urltitle)>
	...
	<cffunction name="getActiveContent" output="false">
		...
		<cfset var useCache=site.getCache()>
		<cfset var sessionData=getSession()>
		<cfset var tracepoint=initTracePoint(detail="Loading contentBean by contentid: #arguments.contentID#")>
	...
	<cffunction name="getCrumbList" returntype="array" output="false">
		...
		<cfset var array ="" />
		<cfset var tracepoint=initTracePoint(detail="Loading content crumbdata by contentid: #arguments.contentID#")>
		<cfset array=variables.contentGateway.getCrumbList(argumentCollection=arguments) />
		<cfset commitTracePoint(tracepoint)>
		<cfif arguments.sort eq "desc">
			<cfset  tracepoint=initTracePoint(detail="Setting content crumbdata reverse sort")>
	...
	<cffunction name="hasPendingRequest" output="false">
		<cfargument name="contentid" required="true">
		<cfargument name="changesetid" required="true" default="">
		...
		<cfquery name="local.rsApprovalsCancel">
			...
			and tcontent.approved=0
			and tcontent.changesetID<cfif len(arguments.changesetid)>=<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.changesetid#"><cfelse> is null </cfif>
			and tcontent.siteID=<cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.siteid#">
			and tapprovalrequests.status='Pending'
		</cfquery>
		<cfreturn local.rsApprovalsCancel.recordcount>
	</cffunction>
	...
	<cffunction name="add" output="false">
		...
		<cfset var pluginEvent=new mura.MuraScope(arguments.data).getEvent()>
		...
		<cfif not refused>
			...
			<cfset var selfAssignedChain=false>
		
			<cfif variables.configBean.getValue(property="approvalChainSelfSelect",defaultValue=false) and len(newBean.getChainID())>
				<cfset var requiresApproval=true>
				<cfset var chainid= newBean.getChainID()>
				<cfset selfAssignedChain=true>
			<cfelse>
				<cfif newBean.getIsNew()>
			...
			<cfif (not newBean.getApprovalChainOverride() or selfAssignedChain) and (newBean.getApproved() or len(newBean.getChangesetID())) and requiresApproval>
			...
			<cfif listFindNoCase('Variation,Component,Form,Module',newBean.getType()) or newBean.getURLTitle() eq '' or newBean.getModuleID() neq '00000000000000000000000000000000000'>
				<cfset newBean.setURLTitle(getBean('contentUtility').formatFilename(newBean.getTitle()))>
			</cfif>

			<cfif listFindNoCase('Variation,Component,Form,Module',newBean.getType()) or newBean.getHTMLTitle(useDefault=false) eq '' or newBean.getModuleID() neq '00000000000000000000000000000000000' >
			...
			<cfif  ListFindNoCase(this.TreeLevelList,newBean.getType())>
				...
				<cfif newBean.getApproved()>
					<cfset variables.pluginManager.announceEvent(eventToAnnounce="onBeforeContentPublish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
				</cfif>
			</cfif>
			...
			<cfif newBean.getApproved()>
				<cfset variables.pluginManager.announceEvent(eventToAnnounce="onBefore#newBean.getType()#Publish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
				<cfset variables.pluginManager.announceEvent(eventToAnnounce="onBefore#newBean.getType()##newBean.getSubType()#Publish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
			</cfif>
			...
			<cfif structIsEmpty(newBean.getErrors())>
				...
				<cfif isObject(pluginEvent.getValue('approvalRequest'))>
					...
					<cfif approvalRequest.getIsNew() or (not newBean.getIsNew() and currentBean.getActive() and currentBean.getApproved())>
						...
						<cfset approvalRequest.setSiteID(newBean.getSiteID())>
				...
				<cfelse>
					...
					<cfelse>
						<cfif newBean.getIsNew()>
							<cfset variables.categoryManager.setCategories(arguments.data,newBean.getcontentID(),
							newBean.getcontentHistID(),arguments.data.siteid,getCategoriesByHistID(newBean.getTemplateHistID())) />
						<cfelse>
							<cfset variables.categoryManager.setCategories(arguments.data,newBean.getcontentID(),
				...
				<cfif newBean.getType() neq 'Module'>
					...
					<!--- 
						If the version has been assigned to a change set cancel any approval request for
						current changeset or previous changeset if chosen
					--->
					<cfset var changesetRemovalList=newBean.getChangesetID()>
					<cfif not newBean.getIsNew() and isBoolean(newBean.getValue("removePreviousChangeset")) and newBean.getValue("removePreviousChangeset") and isValid("uuid",previousChangesetID)>
						<cfset changesetRemovalList=listAppend(changesetRemovalList,previousChangesetID)>
					</cfif>

					<!--- 
						Get any approvals that is associated with a version that
							1. Has been assigned to a change set in the changestRemovalList
							2. Matches the same changeset status
					--->
				
					<cfif len(changesetRemovalList) or doSaveApproval>
						...
						<cfquery name="local.rsApprovalsCancel">
							...
							and (
								<cfif doSaveApproval>
									tcontent.changesetID<cfif len(newBean.getChangesetID())>=<cfqueryparam cfsqltype="cf_sql_varchar" value="#newBean.getChangesetID()#"><cfelse> is null </cfif>
									<cfset started=true>
								</cfif>
								<cfif len(changesetRemovalList) and (newBean.getChangesetID() neq changesetRemovalList or not doSaveApproval)>
									<cfif started> or </cfif> 
									tcontent.changesetID in (<cfqueryparam cfsqltype="cf_sql_varchar" list="true" value="#changesetRemovalList#">)
								</cfif>
							)
					...
					<!---
						Update version in changestRemovalList changesetid to null
					--->
					<cfif len(changesetRemovalList)>
						<cfquery>
							update tcontent set changesetID=null
							where contentID=<cfqueryparam cfsqltype="cf_sql_varchar" value="#newBean.getContentID()#">
							and tcontent.changesetID=<cfqueryparam list="true" cfsqltype="cf_sql_varchar" value="#changesetRemovalList#">
				...
				<cfif newBean.gettype() neq 'File' and isDefined('arguments.data.deleteFile') and len(newBean.getFileID())>
					<cfset variables.fileManager.deleteIfNotUsed(newBean.getFileID(),newBean.getContentHistID(),newBean.getSiteID())>
				...
				<cfif variables.configBean.getPurgeDrafts() and newBean.getApproved() and not newBean.getIsNew()>
					...
					<cfset variables.fileManager.deleteIfNotUsed(deletedList,valueList(rsDrafts.contentHistID),newBean.getSiteID())>
				</cfif>
				...
				<cfelseif not newBean.getIsNew() and not newBean.getapproved() and not currentBean.getapproved()>
					<cfset newBean.setActive(1)>
					<cfset variables.contentDAO.archiveActive(newbean.getcontentID(),arguments.data.siteid)>
				</cfif>
				...
				<cfif  ListFindNoCase(this.TreeLevelList,newBean.getType())>
					...
					<cfif newBean.getApproved()>
						<cfset variables.pluginManager.announceEvent(eventToAnnounce="onContentPublish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
						<cfset variables.pluginManager.announceEvent(eventToAnnounce="onAfterContenPublish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
					</cfif>
				</cfif>
...
--->
				...
				<cfif newBean.getApproved()>
					<cfset variables.pluginManager.announceEvent(eventToAnnounce="on#newBean.getType()#Publish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
					<cfset variables.pluginManager.announceEvent(eventToAnnounce="onAfter#newBean.getType()#Publish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
					<cfset variables.pluginManager.announceEvent(eventToAnnounce="on#newBean.getType()##newBean.getSubType()#Publish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
					<cfset variables.pluginManager.announceEvent(eventToAnnounce="onAfter#newBean.getType()##newBean.getSubType()#Publish",currentEventObject=pluginEvent,objectid=newBean.getContentID())>
				</cfif>
	...
	<cffunction name="deletehistall" output="false" hint="Clears an item's version history">
		...
		<cfset variables.filemanager.deleteIfNotUsed(deleteFileList,deleteIDList,currentBean.getSiteID())>
	...
	<cffunction name="delete" output="false" hint="Deletes a version from version history">
		...
		<cfset pluginEvent.setValue("bean",versionBean)/>
	...
	<cffunction name="createModuleTemplate" output="false">
		<cfset rs=variables.contentDAO.createModuleTemplate(argumentCollection=arguments)>

		<cfreturn rs>
	</cffunction>

	<cffunction name="deleteModuleTemplate" output="false">
		<cfset rs=variables.contentDAO.deleteModuleTemplate(argumentCollection=arguments)>
	...
	<cffunction  name="getPrivateSearchRemote" access="remote">
	...
	<cffunction name="getLastHistoryIdByContent" output="false">
		...
		<cfset var rs =variables.contentGateway.getLastHistoryIdByContent(arguments.ContentID) /> />
	...
	<cffunction name="traverseSite">
		...
		<cfloop query="rs">
			...
			<cfelse>
				...
				<cfset servlet=new mura.servlet()/>
	...
	<cfif isDefined('form.files') and isArray(form.files)>
		<cftry>
			...
			<cfloop from="1" to="#arrayLen(form.files)#" index="f">
				...
				<cfoutput>#jsonUtil.encode(local.returnStr)#</cfoutput>
	...
	<cffunction name="getKidsIterator" output="false">
			...
			<cfset var tracepoint=initTracePoint(detail="Loading content kids: #arguments.parentid#")>
			<cfset var iterator=variables.contentGateway.getKidsIterator(arguments.moduleID, arguments.siteid, arguments.parentID, arguments.type, arguments.today, arguments.size, arguments.keywords, arguments.hasFeatures, arguments.sortBy, arguments.sortDirection, arguments.categoryID, arguments.relatedID, arguments.tag, arguments.aggregation,arguments.applyPermFilter,arguments.taggroup,arguments.useCategoryIntersect)>
			<cfset commitTracePoint(tracepoint)>
			<cfreturn iterator>
		</cffunction>
	...
	<cffunction name="hasImage" output="false">
		...
		<cfreturn len(arguments.bean.getValue('fileID')) && listFindNoCase('jpg,jpeg,png,gif,svg,webp',arguments.bean.getValue('fileEXT')) || arguments.usePlaceholder && len(variables.settingsManager.getSite(arguments.bean.getValue('siteid')).getPlaceholderImgID())>
	</cffunction>
	...
	<cffunction name="getImageURL" output="false">
		...
		<cfscript>
			if(arguments.bean.getType() == 'File' && !ListFindNoCase('png,jpg,jpeg,gif,svg,webp',arguments.bean.getFileExt())){
			...
			<cfif history.recordcount or data.pendingchangesets.recordcount>
				...
				<cfif not getCurrentUser().isAdminUser() and not getCurrentUser().isSuperUser()>
					<cfquery name="data.yourapprovals" dbtype="query">
						...
						where approvalGroupID in (<cfqueryparam list='true' cfsqltype='cf_sql_varchar' value='#listAppend(sessionData.mura.membershipids,getCurrentUser().getValue('userid'))#'>) order by lastupdate asc
					</cfquery>
	...
	<cfif NOT arguments.contentBean.getIsNew()>
		...
		<cfset cache.purge(arguments.contentBean.getSiteID() & arguments.contentBean.getContentID() & 'renderstartredirects')>
		<cfset cache.purge(arguments.contentBean.getSiteID() & arguments.contentBean.getFilename() & 'requeststartredirects')>
	...
	<cffunction name="getTabList" output="false">
			<cfargument name="legacyTabs" default="false" type="boolean">
			<cfset var tabList = "Primary,Basic,Summary,Image,Publishing,Scheduling,Layout,Categorization,Tags,Related Content,Remote,Extended Attributes">
			<cfif arguments.legacyTabs>
				<cfset tabList = listAppend(tablist,getLegacyTabList())>
			</cfif>	
			<cfreturn tabList>
	</cffunction>

	<cffunction name="getLegacyTabList" output="false">
		<cfset var tabList = "Advanced,Layout & Objects,List Display Options,Mobile,SEO,Usage Report">
		<cfreturn tabList>
	</cffunction>
	...
	<cffunction name="getTemplateFromParent" output="false">
		<cfargument name="parentid" type="string" required="true">
		<cfargument name="siteid" type="string" required="true">
		<cfset var parent=getBean('content').loadBy(contentID=arguments.parentid, siteid=arguments.siteid)>
		<cfif parent.exists()>
			<cfif trim(parent.getTemplate()) neq "">
				<cfreturn parent.getTemplate()>
			</cfif>
			<cfreturn getTemplateFromParent(parent.getParentID(), arguments.siteid)>
		<cfelse>
			<cfreturn ""/>
		</cfif>
	</cffunction>
