...
component extends="mura.bean.beanExtendable" entityName="content" table="tcontent" output="false" hint="This provides content functionality" {
	...
	property name="categoryAssignments" fieldtype="one-to-many" cfc="contentCategoryAssign" fkcolumn="contenthistid" loadkey="contenthistid";
	...
	property name="fileMetaData" fieldtype="one-to-one" relatesto="fileMetaData" fkcolumn="fileid" loadkey="fileid";
	...
	public function init() output=false {
		...
		if ( isDefined("sessionData.mura.isLoggedIn") && sessionData.mura.isLoggedIn ) {
	...
	public function validate() output=false {
		...
		if( get('type')!= 'Module'){
			...
			var incomingFilename=getBean('contentUtility').formatFilename(get('URLtitle'));
			
			if(!listFindNoCase('00000000000000000000000000000000END,00000000000000000000000000000000000,00000000000000000000000000000000003,00000000000000000000000000000000004,0000000000000000000000000000000009',getValue('parentid'))){
				incomingFilename=getParent().get('filename') & "/" & incomingFilename;
			}

			if(len(incomingFilename)==255 && getBean('contentUtility').doesFileExist(get('siteid'),incomingFilename,get('contentid'))){		
				variables.instance.errors.filenamelength=variables.settingsManager.getSite(variables.instance.siteID).getRBFactory().getKey("sitemanager.content.filenamelength");
			} else if(len(incomingFilename)>=255){
				variables.instance.errors.filenamelength=variables.settingsManager.getSite(variables.instance.siteID).getRBFactory().getKey("sitemanager.content.filenamelength");
			}
	...
	public function getAll(required autocomplete="#variables.instance.extendAutoComplete#"){
	...
	public function setTitle(required string title) output=false {
		arguments.title=left(trim(arguments.title),255);
	...
	public function setMenuTitle(required string menutitle) output=false {
		arguments.menutitle=left(trim(arguments.menutitle),255);
		if ( len(arguments.menutitle) ) {
			variables.instance.menutitle = arguments.menutitle;
		}else{
			variables.instance.menutitle = '';
		}
		return this;
	...
	public function setURLTitle(required string urltitle) output=false {
		arguments.urltitle=left(trim(arguments.urltitle),255);
		if ( len(arguments.urltitle) ) {
			variables.instance.urltitle = arguments.urltitle;
		}
		else {
			variables.instance.urltitle = '';
		}
	...
	public function setHTMLTitle(required string htmltitle) output=false {
		arguments.htmltitle=left(trim(arguments.htmltitle),255);
		if ( len(arguments.htmltitle) ) {
			variables.instance.htmltitle = arguments.htmltitle;
		}else {
			variables.instance.htmltitle = '';
		}
		return this;
	...
	public function getHTMLTitle(useDefault=true) output=false {
		if (NOT arguments.useDefault OR  len(variables.instance.HTMLTitle) ) {
	...
	public function getAuditTrailIterator() output=false {
		var auditArray=[];
		var it=getBean("contentIterator");
		arrayAppend(auditArray,this);
		var version=getSource();
	
		while(version.exists()){
			arrayAppend(auditArray,version);
			version=version.getSource();
		}

		it.setArray(auditArray);
	...
	public function getRelatedContentQuery(required boolean liveOnly="true", required date today="#now()#", string sortBy="orderno", string sortDirection="asc", string relatedContentSetID="", string name="", boolean reverse="false", required boolean navOnly="false", required any cachedWithin="#createTimeSpan(0,0,0,0)#", applyPermFilter=false) output=false {
		return variables.contentManager.getRelatedContent(variables.instance.siteID, getContentHistID(), arguments.liveOnly, arguments.today, arguments.sortBy, arguments.sortDirection, arguments.relatedContentSetID, arguments.name, arguments.reverse, getContentID(),arguments.navOnly,arguments.cachedWithin,'',this, applyPermFilter);
	}

	public function getRelatedContentIterator(required boolean liveOnly="true", required date today="#now()#", string sortBy="orderno", string sortDirection="asc", string relatedContentSetID="", string name="", boolean reverse="false", required boolean navOnly="false", required any cachedWithin="#createTimeSpan(0,0,0,0)#",entitytype="content", applyPermFilter=false) output=false {
		return variables.contentManager.getRelatedContentIterator(variables.instance.siteID, getContentHistID(), arguments.liveOnly, arguments.today, arguments.sortBy, arguments.sortDirection, arguments.relatedContentSetID, arguments.name, arguments.reverse, getContentID(),arguments.navOnly,arguments.cachedWithin,'',this, applyPermFilter);
	}
	...
	public function setObjectParams(objectParams) output=false {
		...
		} else if ( isStruct(arguments.objectParams) ) {
	...
	public function getURL(required querystring="", required boolean complete="false", required string showMeta="0", required string secure="0",useEditRoute=false) output=false {
	...
	public function getIsOnDisplay(
		display="#variables.instance.display#",
		displayStart="#variables.instance.displayStart#",
		displayStop="#variables.instance.displayStop#",
		type="#variables.instance.type#",
		moduleAssign="#variables.instance.moduleAssign#"
		...
		if(request.muraChangesetPreview){
			var ChangesetPreviewData=getCurrentUser().getValue("ChangesetPreviewData");
			if(isStruct(ChangesetPreviewData) && structKeyExists(ChangesetPreviewData,'publishDate')){
				nowAdjusted=ChangesetPreviewData.publishDate;
			}
		}
		...
		return arguments.display == 1 or
			(
				arguments.display == 2 && arguments.displayStart <= nowAdjusted
				AND (arguments.displayStop == "" || arguments.displayStop >= nowAdjusted)
			)
			and (listFind("Page,Folder,Gallery,File,Calendar,Link,Form",arguments.type) || listFind(arguments.moduleAssign,'00000000000000000000000000000000000'));
	}
