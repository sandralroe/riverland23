...
<cffunction name="empty" output="false">
	...
	<cfset var pluginEvent = new mura.MuraScope() />
	<cfset var i="">
...
<cfargument name="objectClass" default="">
	...
	<cfif arguments.objectClass eq "content">
		...
		<cfloop from="1" to="8" index="i">
			<cfset arguments.deleted.set("objectlist#i#",getBean('contentManager').formatRegionObjectsString(arguments.deleted.getDisplayRegion(i)))>
		</cfloop>
...
<cffunction name="getIterator" output="false">
	<cfset var rs=getQuery(argumentCollection=arguments)>
	<cfset var it=new trashIterator()>
...
<cfargument name="restored" hint="Object that has been pulled from trash and has since been saved">
	...
	<cfif doPurge>
		...
		<cfquery>
			delete from ttrash where objectID=<cfqueryparam cfsqltype="cf_sql_varchar" value="#invoke(arguments.restored,'get#IDString#')#" />
		</cfquery>
