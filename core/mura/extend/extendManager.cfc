<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides class extension manager service level logic functionality">
	...
	<cffunction name="getSubTypeBean">
		<cfset var subtype=new mura.extend.extendSubType(variables.configBean) />
		<cfreturn subtype />
	</cffunction>
	...
	<cffunction name="getExtendedData">
		...
		<cfif structKeyExists(arguments,"type") and  structKeyExists(arguments,"subtype") and  structKeyExists(arguments,"siteID")>
			<cfreturn new mura.extend.extendData(
			...
			<cfloop query="rs">
				...
				<cfif (structKeyExists(arguments.data,key) and len(arguments.data[key]))
					...
					<cfif len(fileID)>
						...
						<cfset fileManager.deleteIfNotUsed(fileID,arguments.baseID,arguments.data.siteid)/>
	...
	<cffunction name="deleteExtendedData" output="false">
		<cfargument name="baseid">
		<cfargument name="dataTable" required="true" default="tclassextenddata">
		<cfargument name="siteid" deafult="">
		...
		<cfset fileManager.deleteIfNotUsed(valueList(rsFiles.attributeValue),arguments.baseID,arguments.siteid)/>
	...
	<cffunction name="getExtendedAttributeList" output="false">
		...
		<!---
		<cfif variables.configBean.getDBType() eq 'Oracle'>
			<cfset rs=getBean('utility').fixOracleClobs(rs)>
		</cfif>
		--->
		<cfreturn rs>
	</cffunction>
		...
		<cfloop query="rsSubTypes">
			...
			<cfif arrayLen(relatedSets)>
				<cfloop from="1" to="#arrayLen(relatedSets)#" index="s">
					...
					<cfif len(sourceRelatedSet.getAvailableSubTypes()) and sourceRelatedSet.getAvailableSubTypes() neq destRelatedSet.getAvailableSubTypes()>
						<cfset destRelatedSet.setAvailableSubTypes(sourceRelatedSet.getAvailableSubTypes())>
						<cfset isdirty=true>
					</cfif>
					<cfif isNumeric(sourceRelatedSet.getOrderNo()) and sourceRelatedSet.getOrderNo() neq destRelatedSet.getOrderNo()>
