<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="Provide extend set CRUD functionality">
	...
	<cffunction name="getAttributeBean">
		<cfset var attribute = new mura.extend.extendAttribute(variables.configBean,variables.contentRenderer) />
	...
	<cffunction name="loadAttributes">
		...
		<cfif rsAttributes.recordcount>
			...
			<cfset tempArray=new mura.queryTool(rsAttributes).toArray() />

			<cfloop from="1" to="#rsAttributes.recordcount#" index="a">
				<cfset attribute=new mura.extend.extendAttribute(variables.configBean,variables.contentRenderer) />
