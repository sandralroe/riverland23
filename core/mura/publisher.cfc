<!--- license goes here --->
<cfcomponent extends="mura.baseobject" output="false" hint="This provides a utility to import bundles">
	...
	<cffunction name="getToWork" output="false">
		...
		<cfset var toSite=getBean('settingsManager').getSite(arguments.tositeid)>
		...
		<cfif not structKeyExists(arguments,"keyFactory")>
			<cfset arguments.keyFactory=new mura.publisherKeys(arguments.keyMode,application.utility)>
		</cfif>
			...
			<cfif len(bundleablebeans)>
				...
				<cfloop list="#bundleablebeans#" index="bb">
					<cfif getServiceFactory().containsBean(bb) and not listFindNoCase(bbList,bb)>
						<cfset logText("Begin restoring bean data #bb#")>
						...
						<cfset logText("End restoring bean data #bb#")>
					</cfif>
				</cfloop>
			</cfif>
		</cfif>
		...
		<cfif StructKeyExists(arguments,"Bundle")>
			...
			<cfif arguments.contentMode neq "none">
				<cfthread name="cleaningFileCache#createUUID()#_#arguments.toSiteID#" siteid="#arguments.toSiteID#" timeout="3600000">
					<cftry>
						<cfset getBean("fileManager").cleanFileCache(siteid)>
						<cfcatch>
			...
			<cfif listFindNoCase("All,Theme",arguments.renderingMode)>
				...
				<cfif rssite.recordcount
					...
					<cfset application.settingsManager.buildSite(siteid=toSiteID,resetEvents=true)>
				<cfelse>
	...
	<cffunction name="getToWorkPartial" output="false">
		...
		<cfset var rsthasstructuredassets=""/>
		...
		<cfset rststructuredassets = arguments.Bundle.getValue("rststructuredassets")>
		...
		<cfloop query="rsthierarchy">
			...
			<cfloop query="rsContentObjects">
				<cfset contentBean.addDisplayObject( regionID=rsContentObjects.columnid,object=rsContentObjects.object,objectID=arguments.keyFactory.get(rsContentObjects.objectID),name=rsContentObjects.name,params=translateObjectParams(rsContentObjects.params,arguments.keyFactory),orderNo=rsContentObjects.orderno )>
			</cfloop>
		...
		<cfloop query="rsContentObjects">
				...
				<cfquery datasource="#arguments.toDSN#" >
					...
					<cfqueryparam cfsqltype="cf_sql_LONGVARCHAR" null="#iif(rsContentObjects.params neq '',de('no'),de('yes'))#" value="#translateObjectParams(rsContentObjects.params,arguments.keyFactory)#">
					)
				</cfquery>
				<cfcatch></cfcatch>
				</cftry>
	...
	<cffunction name="getToWorkSite" output="false">
		...
		<cfset var rstimagesizes="">
			...
			<!--- pushed tables --->
			<cfset logText("Begin restoring site data")>
			<!--- tcontent --->
			...
			<cfset logText("Begin restoring tcontent data")>
			<cfquery datasource="#arguments.toDSN#">
			...
			<cfset logText("End restoring tcontent")>
			<!--- tcontentobjects --->

			<cfset logText("Begin restoring tcontentobjects")>
			<cfquery datasource="#arguments.toDSN#">
			...
			<cfloop query="rstContentObjects">
				<cfquery datasource="#arguments.toDSN#">
					...
					<cfqueryparam cfsqltype="cf_sql_VARCHAR" value="#arguments.toSiteID#">,
					<cfqueryparam cfsqltype="cf_sql_LONGVARCHAR" null="#iif(rstContentObjects.params neq '',de('no'),de('yes'))#" value="#translateObjectParams(rstContentObjects.params,keys)#">
					)
				</cfquery>
			</cfloop>
			<cfset logText("End restoring tcontentobjects")>
			<!--- tcontenttags --->
			<cfset logText("Begin restoring tcontenttags")>
			<cfquery datasource="#arguments.toDSN#">
			...
			<cfset logText("End restoring tcontenttags")>
			<!--- tsystemobjects--->
			<cfif not isDate(arguments.lastDeployment)>
				<cfset logText("Begin restoring tsystemobjects")>
				<cfquery datasource="#arguments.toDSN#">
				...
				<cfset logText("End restoring tsystemobjects")>
			</cfif>

			<!--- tcontentcategoryassign --->
			<cfset logText("Begin restoring tcontentcategoryassign")>
			<cfquery datasource="#arguments.toDSN#">
			...
			<cfset logText("End restoring tcontentcategoryassign")>
			<!--- tcontentrelated --->
			<cfset logText("Begin restoring tcontentrelated")>
			<cfquery datasource="#arguments.toDSN#">
			...
			<cfset logText("End restoring tcontentrelated")>
	...
	<cffunction name="getToWorkChangeSets" output="false">
		...
		<cfset logText("Begin restoring tchangesets")>
		<cfif not StructKeyExists(arguments,"Bundle")>
		...
		<cfset logText("End restoring tchangesets")>
	</cffunction>
	...
	<cffunction name="getToWorkImageSizes" output="false">
		...
		<cfset logText("Begin restoring timagesizes")>
		<cfif not StructKeyExists(arguments,"Bundle")>
		...
		<cfset logText("End restoring timagesizes")>
	</cffunction>
	...
	<cffunction name="getToWorkContentCategories" output="false">
		...
		<cfif application.settingsManager.getSite(arguments.tositeid).getCategoryPoolID() eq arguments.tositeid>
			<!--- tcontentcategories --->
			<cfset logText("Begin restoring tcontentcategories")>
			<cfif not StructKeyExists(arguments,"Bundle")>
			...
			<cfset logText("End restoring tcontentcategories")>
		</cfif>
	...
	<cffunction name="getToWorkFeeds" output="false">
		...
		<!--- tcontentfeeds --->
		<cfset logText("Begin restoring tcontentfeeds")>
		<cfif not StructKeyExists(arguments,"Bundle")>
		...
		<cfset logText("End restoring tcontentfeeds")>
		<!--- tcontentfeeditems --->
		<cfset logText("Begin restoring tcontentfeeditems")>
		<cfquery datasource="#arguments.toDSN#">
		...
		<cfset logText("End restoring tcontentfeeditems")>
		<!--- tcontentfeedadvancedparams --->
		<cfset logText("Begin restoring tcontentfeedadvancedparams")>
		<cfquery datasource="#arguments.toDSN#">
		...
		<cfset logText("End restoring tcontentfeedadvancedparams")>
	</cffunction>
	...
	<cffunction name="getToWorkFiles" output="false">
		...
		<cfset logText("Begin restoring tfiles")>
		...
		<cfset logText("End restoring tfiles")>
				...
				<cfloop query="rstclassextend">
					...
					<cfif rsCheck.recordcount>
						<cfquery datasource="#arguments.toDSN#">
							...
							adminonly=<cfqueryparam cfsqltype="cf_sql_INTEGER" null="#iif(rstclassextend.adminOnly neq '',de('no'),de('yes'))#" value="#rstclassextend.adminOnly#">,
							</cfif>
							lastUpdateBy='System'
							where subTypeID = <cfqueryparam cfsqltype="cf_sql_VARCHAR" value="#keys.get(rstclassextend.subTypeID)#">
						</cfquery>
	...
	<cffunction name="publish">
		...
		<cfset var keys=new mura.publisherKeys('publish',application.utility)>
	...
	<cffunction name="copy" output="no">
		...
		<cfset var pluginEvent= new mura.event(variables) />
		...
		<cfset keys=new mura.publisherKeys('copy',application.utility)>
	...
	<cffunction name="copySiteFiles">
		...
		<cfloop query="rs">
			...
			<cfelse>
				<!--- <cftry> --->
					<cfset fileWriter.copyFile(source="#rs.directory##fileDelim##rs.name#", destination=replaceNoCase('#rs.directory##fileDelim#',fileWriter.conditionalExpandPath(arguments.baseDir),fileWriter.conditionalExpandPath(arguments.destDir)))>
					...
					<cfset fileWriter.renameFile(source="#replaceNoCase('#rs.directory##fileDelim#',arguments.baseDir,arguments.destDir)##rs.name#", destination="#replaceNoCase('#rs.directory##fileDelim#',fileWriter.conditionalExpandPath(arguments.baseDir),fileWriter.conditionalExpandPath(arguments.destDir))##newFile#")>
				<!--- 	<cfcatch></cfcatch>
				</cftry> --->
			</cfif>
		</cfloop>
