	...
	<div id="svLoginContainer" class="mura-login-container #this.loginWrapperClass#">
		<div class="#this.loginWrapperInnerClass#">
			...
			<cfif StructKeyExists(session, 'mfa') and Len(session.mfa.username)>
				<cfset objStrikes = new mura.user.userstrikes(session.mfa.username, application.configBean) />
					...
					<form role="form" id="login" class="mura-login-form #this.loginFormClass# <cfif this.formWrapperClass neq "">#this.formWrapperClass#</cfif>" name="frmLogin" method="post" novalidate="novalidate">
								...
								<div class="#this.loginFormGroupWrapperClass#">
									<div class="#this.loginFormSubmitWrapperClass#">
										...
										<cfif listFindNoCase(providersAddedList, 'google')>
											<a href="#$.getBean('googleLoginProvider').generateAuthUrl()#" title="#variables.$.rbKey('login.loginwithgoogle')#" class="mura-login-auth-btn ggl">
										...
										<cfif listFindNoCase(providersAddedList, 'facebook')>
											<a href="#$.getBean('facebookLoginProvider').generateAuthUrl()#" title="#variables.$.rbKey('login.loginwithfacebook')#" class="mura-login-auth-btn fb">
							            ...
							            <cfif listFindNoCase(providersAddedList, 'github')>
							                <a href="#$.getBean('githubLoginProvider').generateAuthUrl()#" title="#variables.$.rbKey('login.loginwithgithub')#" class="mura-login-auth-btn gh">
							                  	<i class="mi-github"></i>
							                  	<span>#variables.$.rbKey('login.loginwithgithub')#</span>
							                </a>
							            </cfif>
										<cfif listFindNoCase(providersAddedList, 'microsoft')>
										  <a href="#$.getBean('microsoftLoginProvider').generateAuthUrl()#" title="#variables.$.rbKey('login.loginwithmicrosoft')#" class="mura-login-auth-btn ms">
										  	<i class="mi-ms"></i>
						...
						<div>
							...
							<div class="req #this.loginFormGroupWrapperClass#">
								...
								<div class="#this.loginFormFieldWrapperClass#">
									<input class="#this.loginFormFieldClass#" type="text" id="txtUsername" placeholder="#variables.$.rbKey('user.username')#" name="username" data-required="true" maxlength="255" data-message="#htmlEditFormat(variables.$.rbKey('user.usernamerequired'))#" autofocus>
								</div>
							</div>
							...
							<div class="req #this.loginFormGroupWrapperClass#">
								...
								<div class="#this.loginFormFieldWrapperClass#">
									<input class="#this.loginFormFieldClass#" type="password" id="txtPassword" name="password" maxlength="255" placeholder="#variables.$.rbKey('user.password')#" data-required="true" data-message="#htmlEditFormat(variables.$.rbKey('user.passwordrequired'))#" autocomplete="off">
								</div>
							</div>

							<cfif not variables.$.getBean('configBean').getValue(property='MFA',defaultValue=false) and variables.$.getBean('configBean').getValue(property='rememberme',defaultValue=true)>
								<!--- Remember Me --->
								<div class="#this.loginFormGroupWrapperClass#">
									<div class="#this.loginFormPrefsClass#">
