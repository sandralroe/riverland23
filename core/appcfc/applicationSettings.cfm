...
this.sessionManagement = !(left(cgi.path_info,11) == '/_api/rest/' || (isdefined('httpData.method') && httpData.method=='options'));
...
if ( !this.sessionManagement ) {
 ...
 Find( "mj12bot", request.userAgent ) OR
 Find( "petalbot", request.userAgent )
 );
...
try {
	...
	if(isDefined('e')){
		writeLog(type="Error", log="exception", text="If the following error is a missing include , no action is required");
...
if(request.muraInDocker && (len(getSystemEnvironmentSetting('MURA_DATABASE')) || len(getSystemEnvironmentSetting('MURA_DBCONNECTIONSTRING')))){
	...
	/* SETUP PRIMARY DATASOURCE */

	/* SETUP PRIMARY CONNECTIONSTRING */
	if (len(getSystemEnvironmentSetting('MURA_DBCONNECTIONSTRING'))) {
		...
		/* 
			SETUP NODATABASE DATASOURCE WITH CONNECTIONSTRING 
			This is used to check if the database in the datasource is defined
		*/
		...
		/* END SETUP NODATABASE DATASOURCE WITH CONNECTIONSTRING 	*/
		/* END SETUP PRIMARY WITH CONNECTIONSTRING */
	} else {
		/* SETUP PRIMARY DATASOURCE WITHOUT CONNECTIONSTRING */
		if(!structKeyExists(this.datasources,'#getSystemEnvironmentSetting('MURA_DATASOURCE')#')){
			this.datasources['#getSystemEnvironmentSetting('MURA_DATASOURCE')#']={
					'#driverVarName#' = driverName
			...
			*/
			...
			/* END SETUP NODATABASE DATASOURCE */
		}
		/* END SETUP PRIMARY DATASOURCE WITHOUT CONNECTIONSTRING */
	}
	...
	/* END SETUP PRIMARY DATASOURCE */

	/* SETUP READONLY DATASOURCE */
	readonlyDatasource=getSystemEnvironmentSetting('MURA_READONLYDATASOURCE');

	if (len(getSystemEnvironmentSetting('MURA_ROCONNECTIONSTRING')) && len(readonlyDatasource) && !structKeyExists(this.datasources,'#readonlyDatasource#')){
		this.datasources['#readonlyDatasource#']={
			'#driverVarName#' = driverName
			, '#connectionStringVarName#' = getSystemEnvironmentSetting('MURA_ROCONNECTIONSTRING')
			, 'username' = getSystemEnvironmentSetting('MURA_DBUSERNAME')
			, 'password' = getSystemEnvironmentSetting('MURA_DBPASSWORD')
		};

		if (len(getSystemEnvironmentSetting('MURA_DBCLASS'))) {
			this.datasources['#readonlyDatasource#'].class = getSystemEnvironmentSetting('MURA_DBCLASS');
		}

		if (len(getSystemEnvironmentSetting('MURA_DBBUNDLENAME'))) {
			this.datasources['#readonlyDatasource#'].bundleName = getSystemEnvironmentSetting('MURA_DBBUNDLENAME');
		}

		if (len(getSystemEnvironmentSetting('MURA_DBBUNDLEVERSION'))) {
			this.datasources['#readonlyDatasource#'].bundleVersion = getSystemEnvironmentSetting('MURA_DBBUNDLEVERSION');
		}

		if (server.coldfusion.productname == 'lucee') {
			if(len(getSystemEnvironmentSetting('MURA_DBTIMEZONE'))){
				this.datasources["#readonlyDatasource#"].timezone=getSystemEnvironmentSetting('MURA_DBTIMEZONE');
			}
	
			if(len(getSystemEnvironmentSetting('MURA_DBSTORAGE'))){
				this.datasources["#readonlyDatasource#"].storage=getSystemEnvironmentSetting('MURA_DBSTORAGE');
			}
	
			if(len(getSystemEnvironmentSetting('MURA_DBCONNECTIONLIMIT'))){
				this.datasources["#readonlyDatasource#"].connectionLimit=getSystemEnvironmentSetting('MURA_DBCONNECTIONLIMIT');
			}
	
			if(len(getSystemEnvironmentSetting('MURA_DBCONNECTIONTIMEOUT'))){
				this.datasources["#readonlyDatasource#"].connectionTimeout=getSystemEnvironmentSetting('MURA_DBCONNECTIONTIMEOUT');
			}
	
			if(len(getSystemEnvironmentSetting('MURA_DBCUSTOM')) && isJSON(getSystemEnvironmentSetting('MURA_DBCUSTOM'))){
				this.datasources["#readonlyDatasource#"].custom=deserializeJSON(getSystemEnvironmentSetting('MURA_DBCUSTOM'));
			}
		}
	}
	/* END SETUP READONLY DATASOURCE */
...
if (server.coldfusion.productname == 'lucee') {
	...
	} else if(len(getSystemEnvironmentSetting('MURA_MEMCACHEDSESSIONSERVERV2'))){
		this.cache.connections["muramemcachedsessionsv2"] = {
			...
			, custom: {
				...
				"compress":"false",
				"failure_mode":"Retry",
				"servers":getSystemEnvironmentSetting('MURA_MEMCACHEDSESSIONSERVERV2')
			}
			, "default": ''
		};
		this.sessionCluster = true;
		this.sessionStorage = "muramemcachedsessionsv2";
	}
...
this.compression=evalSetting(getINIProperty('compression',false));
