<cfscript>
	...
	param name="application.appBundleRestore" default={};
	...
	if ( left(server.coldfusion.productversion,5) == "9,0,0" || listFirst(server.coldfusion.productversion) < 9 ) {
		writeOutput("Mura requires Adobe Coldfusion 9.0.1 or greater compatibility");
		abort;
	}
	...
	if ( application.setupComplete ) {
		...
		try {
			if ( !structKeyExists(variables.iniProperties,"encryptionkey") || !len(variables.iniProperties["encryptionkey"]) ) {
				...
				new mura.IniFile(variables.iniPath).set( variables.iniProperties.mode, "encryptionkey", variables.iniProperties.encryptionkey );
			}
		} catch (any cfcatch) {
		}
		*/
		...
		*/
		...
		variables.iniProperties.webroot = expandPath("/muraWRM");
		variables.tracer=new mura.baseobject();
		...
		if(application.configBean.getValue(property='overrideSecureCookies',defaultValue=false)){
			application.configBean.setValue('secureCookies',false);
		}
	
		//new muraWRM.core.modules.v1.filebrowser.model.beans.filebrowser();
		...
		variables.serviceFactory=new mura.bean.beanFactory("/mura",{
				...
				transientPattern = "(Iterator|Feed|Bean|executor|MuraScope|Event|dbUtility|extendObject|Facade)$"
				});
		...
		variables.serviceFactory.addBean("useMode",application.configBean.getUseFileMode());
		...
		variables.serviceFactory.addBean("preInit",false);
		...
		if ( application.configBean.getCreateRequiredDirectories() ) {
			...
			if ( !local.hasTheme ) {
				...
				try {
					...
					writeLog(type="Error", log="exception", text="Error pullling theme from remote");
		...
		variables.serviceFactory.addAlias("currentUser","sessionUserFacade");
		...
		variables.serviceFactory.addAlias("entityCategoryAssign","beanCategoryAssign");
		variables.serviceFactory.addAlias("entityTagAssign","beanTagAssign");
		...
		variables.serviceFactory.addAlias("proxy","proxyBean");
		variables.serviceFactory.addAlias("proxyCredential","proxyCredentialBean");
		variables.serviceFactory.addAlias("proxyEvent","proxyEventBean");
		variables.serviceFactory.addAlias("lambda","lambdaBean");
		variables.serviceFactory.addAlias("lambdaenv","lambdaEnvBean");
		variables.serviceFactory.addAlias("oauthProvider","oauthProviderBean");
		variables.serviceFactory.addAlias("oauthSetting","oauthSettingBean");
		variables.serviceFactory.addAlias("moduleTemplate","contentModuleTemplateBean");
		application.serviceFactory=variables.serviceFactory;
		...
		if ( !isDefined('url.applyDBUpdates') ) {
			dbUtility=application.serviceFactory.getBean('dbUtility');
			if(
				!dbUtility.tableExists('tplugins') 
				|| !dbUtility.tableExists('toauthsettings')
			){
		...
		if ( application.appAutoUpdated || isdefined('url.applyDBUpdates') ) {
			...
			variables.tracepoint=variables.tracer.initTracepoint("Reading cached object mappings");
			...
			variables.serviceFactory.getBean('entityTagAssign');
			variables.serviceFactory.getBean('entityCategoryAssign');
			variables.serviceFactory.getBean("proxy");
			variables.serviceFactory.getBean("proxyCredential");
			variables.serviceFactory.getBean("proxyEvent");
			variables.serviceFactory.getBean("lambda");
			variables.serviceFactory.getBean("lambdaenv");
			variables.serviceFactory.getBean("oauthProvider");
		}
	
		variables.serviceFactory.getBean('contentCategoryAssign');
	
		param name="application.muraClientConfig" default={};

		clientConfig=(len(application.configBean.getValue('externalConfig'))) ? application.configBean.getValue('externalConfig') : application.configBean.getValue('clientConfig');
		
		if (len(clientConfig)) {
			config='';
			if(isValid('url',clientConfig)){
				httpService=application.configBean.getHTTPService();
				httpService.setMethod("get");
				httpService.setCharset("utf-8");
				httpService.setURL(clientConfig);
				config=httpService.send().getPrefix().filecontent;
			} else if (fileExists(clientConfig)) {
				config=fileRead(clientConfig,'utf-8');
			}
			
			if(isJSON(config)){
				application.muraClientConfig=deserializeJSON(config);
			} else {
				writeLog(type="Error", log="exception", text="Error reading external config from  '#clientConfig#': #config#");
			}
		}
	
		if(	isdefined('url.applyDBUpdates')
				&& isDefined('application.muraClientConfig.global.entities')
				&& isArray(application.muraClientConfig.global.entities)
			){
			entities=application.muraClientConfig.global.entities;
		...
		variables.serviceList="utility,pluginManager,settingsManager,contentManager,contentRenderer,contentUtility,contentGateway,categoryManager,clusterManager,contentServer,changesetManager,scriptProtectionFilter,permUtility,emailManager,loginManager,mailinglistManager,userManager,dataCollectionManager,feedManager,sessionTrackingManager,favoriteManager,raterManager,dashboardManager,autoUpdater";
		//  The ad manager has been removed, but may be there in certain legacy conditions
		...
		for(variables.i in listToArray(variables.serviceList)){
			...
			if(variables.i=='pluginManager'){
				application["#variables.i#"].setValue('settingsManager',application.serviceFactory.getBean("settingsManager"));
			}
			variables.tracer.commitTracepoint(variables.tracepoint);
		}
		
		application.Mura=application.serviceFactory.getBean('mura');
		application.Mura.status={};
		...
		if ( listFindNoCase('Railo,Lucee',application.configBean.getCompiler()) ) {
			...
			variables.siteMonitorTask="#application.configBean.getWebRoot()##application.configBean.getIndexPath()#/_api/sitemonitor/";
		}
		try {
			if ( isBoolean(variables.iniProperties.ping) && variables.iniProperties.ping ) {
				...
				local.updateurl = "http://" & application.Mura.getBean('utility').getRequestHost() & application.configBean.getContext() & "#application.configBean.getIndexPath()#/_api/sitemonitor/";
		...
		application.plugins=structNew();
		variables.pluginEvent=new mura.event();
		...
		if(projectSiteID != 'default'  && !application.settingsManager.siteExists(projectSiteID)){
			...
			domain=application.Mura.getBean('utility').getRequestHost();
			...
			application.settingsManager.create({
				...
				domain=domain,
				site=application.configBean.getValue(property='title',defaultValue='Mura'),
				orderno=1,
				autocreated=true
				});
		...
		//make this a MURA_DEFAULTBUNDLEURL
		if(application.contentGateway.getPageCount(projectSiteID).counter == 1){
			if(len(application.configBean.getDefaultBundleURL()) && isValid('url',application.configBean.getDefaultBundleURL())){
				try {
					local.httpService=application.configBean.getHTTPService();
					local.httpService.setURL(application.configBean.getDefaultBundleURL());
					local.httpService.setGetAsBinary("yes");
					local.bundle=httpService.send().getPrefix();
					local.fileWriter.writeFile(file="/tmp/MuraBundle.zip",output=local.bundle.filecontent);
					local.bundleLoc="/tmp/MuraBundle.zip";
				} catch (Any e) {
					writeLog(type="Error", log="exception", text="Error pullling bundle from remote");
					writeLog(type="Error", log="exception", text="#serializeJSON(e)#");
				...
				if( len(application.configBean.getDefaultBundleFile()) ){
					local.bundleLoc=application.configBean.getDefaultBundleFile();
				} else {
					if ( request.muraInDocker ) {
						local.bundleLoc="/tmp/MuraBundle.zip";
					} else {
			...
			if (isDefined('local.bundleLoc') && fileExists(local.bundleLoc)){
				application.settingsManager.restoreBundle(
					bundleFile=local.bundleLoc,
					keyMode=application.Mura.getBean('configBean').getValue(property='defaultBundleKeyMode',defaultValue='publish'),
					siteID=projectSiteID,
					contentMode='all',
					pluginMode=application.Mura.getBean('configBean').getValue(property='defaultBundlePluginMode',defaultValue='none'),
					usersMode=application.Mura.getBean('configBean').getValue(property='defaultBundleUsersMode',defaultValue='none'),
					renderingMode=application.Mura.getBean('configBean').getValue(property='defaultBundleRenderingMode',defaultValue='none')
				);
				if(application.Mura.getBean('configBean').getValue(property='defaultBundleDelete',defaultValue=true)){
					local.fileWriter.deleteFile(local.bundleLoc);
				}
			}
		}
		...
		if(!application.configBean.getValue(property="individualsitebuilds", defaultValue=false)){
			...
			for(i=1;i <= variables.rsSites.recordcount;i++){
				application.settingsManager.getSite(variables.rssites.siteid[i]).runSiteAndThemeApplicationLoad();
			}
			commitTracePoint(variables.tracePoint1);
		}
	
		if(!application.configBean.getValue(property='readonly',defaultValue=false) && isDefined('url.updatelinks')){
		...
		if(isDefined('application.muraClientConfig.global.rendererProperties')){
			renderProps=application.muraClientConfig.global.rendererProperties;
			if(isStruct(renderProps)){
				if(structKeyExists(renderProps,'siteidinurls') && isBoolean(renderProps.siteidinurls)){
					application.configBean.setValue('siteidinurls',renderProps.siteidinurls);
				}
			}
		}
		...
		application.appBundleRestore={};
	
		application.coreversion=application.configBean.getVersion();
	}
	</cfscript>