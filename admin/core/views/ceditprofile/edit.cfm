							...
							<div class="block-content">
				      		...
				      		<label>#rc.$.rbKey('user.type')#</label>
								...
								<div class="mura-control-group">
								    ...
								    <input id="fname" name="fname" maxlength="50" type="text" value="#esapiEncode('html_attr',rc.userBean.getfname())#" required="true" message="#rc.$.rbKey('user.fnamerequired')#"/>
						    </div>

								<div class="mura-control-group">
						      	<label>#rc.$.rbKey('user.lname')#*</label>
						      	<input id="lname" name="lname" type="text" maxlength="50" value="#esapiEncode('html_attr',rc.userBean.getlname())#" required="true" message="#rc.$.rbKey('user.lnamerequired')#"/>
						    </div>

								<div class="mura-control-group">
							      	<label>#rc.$.rbKey('user.company')#</label>
							      	<input id="organization" name="company" maxlength="100" type="text" value="#esapiEncode('html_attr',rc.userBean.getcompany())#" />
								</div>

								<div class="mura-control-group">
							      <label>#rc.$.rbKey('user.jobtitle')#</label>
							      <input id="jobtitle" name="jobtitle" type="text" maxlength="50" value="#esapiEncode('html_attr',rc.userBean.getjobtitle())#" />
								</div>

								<div class="mura-control-group">
							      <label>#rc.$.rbKey('user.email')#*</label>
							      <input id="email" name="email" type="text" validate="email" maxlength="255" message="#rc.$.rbKey('user.emailvalidate')#" value="#esapiEncode('html_attr',rc.userBean.getemail())#"/>
								</div>

								<div class="mura-control-group">
							      	<label>#rc.$.rbKey('user.mobilephone')#</label>
							      	<input id="mobilePhone" name="mobilePhone" maxlength="50" type="text" value="#esapiEncode('html_attr',rc.userBean.getMobilePhone())#"/>
								</div>

								<div class="mura-control-group">
						      		<label>#rc.$.rbKey('user.username')#**</label>
						      		<input id="username" name="usernameNoCache" maxlength="50" type="text" value="#esapiEncode('html_attr',rc.userBean.getusername())#" message="#rc.$.rbKey('user.usernamerequired')#" />
						    </div>
								...
								<div class="mura-control-group">
							      	...
							      	<input name="passwordNoCache"  autocomplete="new-password" validate="match" matchfield="password2" type="password" value=""  message="#rc.$.rbKey('user.passwordmatchvalidate')#" autocomplete="off"/>
						    </div>

								<div class="mura-control-group">
							      	<label>#rc.$.rbKey('user.newpasswordconfirm')#**</label>
							      	<input name="password2" autocomplete="new-password" type="password" value="" message="#rc.$.rbKey('user.passwordconfirmvalidate')#" autocomplete="off"/>
						    </div>


						    <div class="mura-control-group">
						      	<label>#rc.$.rbKey('user.image')#</label>
						      	<input type="file" name="newFile" validate="regex" regex="(.+)(\.)(jpg|JPG|jpeg|JPEG|png|PNG|webp|WEBP)" message="Your logo must be a .JPG" value=""/>
						        <cfif len(rc.userBean.getPhotoFileID())>
							        	<a href="./index.cfm?muraAction=cArch.imagedetails&userid=#rc.userBean.getUserID()#&siteid=#rc.userBean.getSiteID()#&fileid=#rc.userBean.getPhotoFileID()#"><img id="assocImage" src="#application.configBean.getContext()##application.configBean.getIndexPath()#/_api/render/medium/?fileid=#rc.userBean.getPhotoFileID()#&cacheID=#createUUID()#" /></a>
