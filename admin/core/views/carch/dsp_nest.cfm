...
<cfset isFileIcon=rsnest.type eq 'File' and listFirst(icon,"-") neq "icon" and not listFindNoCase("jpg,jpeg,png,gif,webp",listLast(rsnest.assocfilename,"."))>
...
<dt class="mura-grid-item">
	...
	<cfsilent>
		<cfif verdict neq 'none' and listFindNoCase("jpg,jpeg,png,gif,webp",listLast(rsnest.assocfilename,"."))>
			<cfset atooltip=true>
			<cfset atitle="<img onload='$(this).handlers().tooltip();' onerror='this.onerror=null;$(this).handlers().tooltip();' class='image-preview' height='86' width='86' src='#$.getURLForImage(fileid=rsnest.fileid,size='small',siteid=rsnest.siteid,fileext=rsnest.fileExt,useProtocol=false)#'>">
		<cfelse>
	...
	<cfif not listFindNoCase('none,read',verdict)>
		<a class="<cfif isFileIcon>file </cfif>draftprompt title-nested" title="#atitle#" href="./?muraAction=cArch.edit&contenthistid=#rsnest.ContentHistID#&contentid=#rsnest.ContentID#&type=#rsnest.type#&parentid=#rsnest.parentID#&topid=#esapiEncode('url',attributes.topid)#&siteid=#esapiEncode('url',attributes.siteid)#&moduleid=#rsnest.moduleid#&startrow=#esapiEncode('url',attributes.startrow)#"<cfif rsnest.type eq 'File'> data-filetype="#lcase(left(rsnest.fileExt,4))#"</cfif>  <cfif atooltip>rel="tooltip" data-html="true"</cfif>>
	<cfelse>
		<a class="<cfif rsnest.type eq 'File'>file </cfif> title-nested"<cfif rsnest.type eq 'File'> data-filetype="#lcase(left(rsnest.fileExt,4))#"</cfif> <cfif atooltip>rel="tooltip" data-html="true" title="#atitle#"</cfif>>
	</cfif>
	   ...
	   <cfswitch expression="#rsnest.type#">
				<cfcase value="Page,Folder,Calendar,Gallery">
			   	<cfif $.getContentRenderer().useLayoutManager()>
			   		<li class="edit-layout"><a title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.edit-layout")#" href="#application.settingsManager.getSite(attributes.siteid).getWebPath(complete=1)##$.getContentRenderer().editroute##$.getURLStem(attributes.siteid,rsnest.filename)#?editlayout=true"><i class="mi-th"></i></a></li>
			   	</cfif>	
		   		<li class="preview"><a title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.view")#" href="#application.settingsManager.getSite(attributes.siteid).getWebPath(complete=1)##$.getContentRenderer().editroute##$.getURLStem(attributes.siteid,rsnest.filename)#"><i class="mi-globe"></i></a></li>
		   </cfcase>
		   <cfcase value="File,Link">
		   		<li class="preview"><a title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.view")#" href="##" onclick="return preview('#application.settingsManager.getSite(attributes.siteid).getWebPath(complete=1)##$.getContentRenderer().editroute##$.getURLStem(attributes.siteid,rsnest.filename)#','#esapiEncode('javascript',rsnest.targetParams)#');"><i class="mi-globe"></i></a></li>
		   </cfcase>
		   <cfcase value="Variation">
		   		<li class="preview"><a title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.view")#" href="#esapiEncode('html_attr',rsnest.remoteurl)#"><i class="mi-globe"></i></a></li>
		   </cfcase>
		   <cfdefaultcase>
