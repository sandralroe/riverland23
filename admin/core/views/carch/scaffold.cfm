...
<script src="#$.globalConfig('rootPath')##application.configBean.getAdminDir()#/assets/js/scaffold/scaffolder.js"></script>
...
<div class="block-content">
		...
		<div>
			...
			<span v-if="entityname">
				...
				<ul class="breadcrumb" v-if="entityname!='entity'">
						...
						<li v-if="data.parentproperties && data.parentproperties.entityname != 'entity'">
							<strong><a href="##" onclick="return false;"><i class="mi-cube"></i>{{data.parentproperties.displayname}}
					...
					<tbody>
						<tr v-if="data.list.length && !listtransition" v-for="(object,index) in data.list">
								...
								<td class="var-width" v-for="item in data.listview" @click="(entityname == 'entity') ? showList(object.entityname) : ( (item.fieldtype == 'many-to-one' || item.fieldtype == 'one-to-one' ) ? showForm(object[item.name].entityname,object[item.name].id)  : showForm(object.entityname,object.id) )">
										...
										<span v-else-if="item.rendertype == 'codeeditor'" v-html="object[item.name]"></span>
				...
				<ul v-if="data.list.length" class="pagination">
					...
					<li class="pull-right">
						<select name="itemsper" class="itemsper" @change="applyItemsPer">
							<option value='25' :selected="this.$parent.itemsper == 25 ? 'selected' : null">25</option>
							<option value='50' :selected="this.$parent.itemsper == 50 ? 'selected' : null">50</option>
							<option value='100' :selected="this.$parent.itemsper == 100 ? 'selected' : null">100</option>
							<option value='9999' :selected="this.$parent.itemsper == 9999 ? 'selected' : null">All</option>
						</select>
					</li>
	...
	<template id="scaffold-form-template">
		...
		<ul class="breadcrumb">
			...
			<li v-if="data.parentproperties && data.parentproperties.displayname && data.parentproperties.entityname != 'entity'"><a @click="clickBack" href="##" onclick="return false;"><i class="mi-cube"></i>{{data.parentproperties.displayname}}</a></li>
			...
			<template v-for="property in data.properties">
				...
				<div v-else>
					...
					<scaffold-field-codeeditor v-else-if="property.rendertype === 'codeeditor'" :property=property :model=data.model :entity=data.entity></scaffold-field-codeeditor>
	...
	<template id="scaffold-field-codeeditor">
		<div>
			<div v-if="model.errors && model.errors[property.name]" class="help-block-inline">
				{{model.errors[property.name]}}
			</div>
