...
<cfscript>
	...
	if(!structKeyExists(session,'flatViewArgs')){
		session.flatViewArgs={};
	}
...
<div id="navFilters">
	...
	<div id="navFilterControls">
		<div class="mura-layout-row">
			...
			<!--- categories --->
			<cf_dsp_categories_filter divid="mura-list-tree" divclass="mura-6 mura-control-group" divlabel="#application.rbFactory.getKeyValue(session.rb,"sitemanager.categories")#" placeholder="Select categories">
			<!--- /categories --->
	...
	<table class="mura-table-grid">
		...
		<tr data-siteid="#item.getSiteID()#" data-contentid="#item.getContentID()#" data-contenthistid="#item.getContentHistID()#" data-sortby="#item.getSortBy()#" data-sortdirection="#item.getSortDirection()#" data-moduleid="#esapiEncode('html_attr',item.getModuleID())#" data-type="#item.getType()#" class="mura-node-data">
				...
				<ul class="siteSummary">
					<cfif not listFindNoCase('none,read',verdict) or $.event('report') eq 'mydrafts'>
						...
						<cfswitch expression="#item.gettype()#">
							<cfcase value="File">
							<li class="download"><a title="#application.rbFactory.getKeyValue(session.rb,'sitemanager.download')#" href="#application.configBean.getContext()##application.configBean.getIndexPath()#/_api/render/file/?fileID=#item.getFileID()#&method=attachment" onclick="return confirmDialog('#esapiEncode('javascript',application.rbFactory.getKeyValue(session.rb,'sitemanager.downloadconfirm'))#',this.href)"><i class="mi-download"></i></a></li>
							</cfcase>
							<cfdefaultcase>
							<cfif listFindNoCase("Page,Folder,Calendar,Gallery",item.gettype())>
						   	<cfif $.getContentRenderer().useLayoutManager()>
									<li class="edit-layout"><a title="#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.edit-layout')#" href="#item.getURL(complete=1,querystring="editlayout=true",useEditRoute=true)#"><i class="mi-th"></i></a></li>
								</cfif>
							</cfif>
							<li class="preview"><a title="#application.rbFactory.getKeyValue(session.rb,'sitemanager.view')#" href="#item.getURL(complete=1,useEditRoute=true)#"><i class="mi-globe"></i></a></li>
							</cfdefaultcase>
						</cfswitch>
						</cfif>
					...
					<cfelse>
						...
						<cfif item.getmoduleid() eq '00000000000000000000000000000000000' or (item.getmoduleid() eq '00000000000000000000000000000000099' and item.getType() eq 'Variation')>
						<li class="preview"><a title="Preview" href="#item.getURL(complete=1,useEditRoute=true)#"><i class="mi-globe"></i></a></li>
						</cfif>
			...
			<cfif listFindNoCase("png,jpg,jpeg,gif,webp",item.getFileExt())>
