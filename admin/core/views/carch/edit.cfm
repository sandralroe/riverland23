...
<cfelseif rc.compactDisplay eq "true">
	...
	jQuery(document).ready(function(){
		...
		jQuery(".close-frontend-modal").on("click",function(){
			frontEndProxy.post({cmd:'close'});
		});
	});
	</script>
</cfif>
...
<script>
	function handleBigUI(){
		
		// create hidden BigUI with button, close link
		var renderBigUI = function(el){
			var idstr = $(el).attr('id');
			var labelstr = $(el).attr('data-label');
			if(!$('[data-rel="' + idstr + '"]').length){
				$(el).before('<a class="bigui__launch" data-rel="' + idstr + '" href="##">' + labelstr + '</a>');
				$(el).prepend('<a class="bigui__close"><cfoutput>#application.rbFactory.getKeyValue(session.rb,'sitemanager.done')#</cfoutput> <i class="mi-angle-right"></i></a>')
		...
		function openBigUI(){
			...
			$('#extendset-container-primary').hide();
			$('.bigui').hide();
			$('#' + rel).show();
			return false;
		}
		...
		function closeBigUI(){
			$('#extendset-container-primary').show();
			$(this).parents('.bigui').hide();
		}

		$('.bigui__close')
			.off('click',closeBigUI)
			.on('click',closeBigUI);
	}

	$(document).on('onlocaleload',handleBigUI);
	$(document).ready(function(){
		...
		<cfif len($.event('bigui'))>
			var biguistr = '#esapiEncode("javascript",trim($.event('bigui')))#';
		<cfelseif rc.parentBean.getType() is 'calendar' and (rc.contentBean.getDisplay() eq 2 or rc.contentBean.getIsNew())>
		  var biguistr = 'schedule';
		<cfelse>
			var biguistr = '';
		</cfif>
		if (biguistr.length){		
			setTimeout(function(){
				$('a.bigui__launch[data-rel="bigui__' + biguistr + '"]').trigger('click').closest('.mura-panel').find('.mura-panel-title a.collapsed').trigger('click');
				if (biguistr == 'schedule'){
					$('##editDates').show();
				}
			}, 500);
		}
	...
	<div class="mura-actions">
		<div class="form-actions">
			<cfif rc.compactDisplay eq "true">
				<button type="button" class="btn close-frontend-modal"><i class="mi-ban"></i> #esapiEncode('html',application.rbFactory.getKeyValue(session.rb,"sitemanager.content.cancel"))#</button>
			<cfelse>
				<cfif $.event('frontEnd')>
					<cfset rc.cancelhref="#rc.contentBean.getURL(secure=rc.$.getBean('utility').isHTTPs(),complete=1,queryString='previewid=#rc.contentBean.getContentHistID()#',useEditRoute=true)#">
				<cfelse>
			...
			<button type="button" class="btn"
				onclick="
					siteManager.validateAltUrls().then(function(hasErrors) {
						if(!hasErrors) {
							return saveDraftPrompt();
						}
					});"><i class="mi-edit"></i> #esapiEncode('html',application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savedraft"))#</button>
			...
			<button type="button" class="btn" onclick="
				siteManager.validateAltUrls().then(function(hasErrors) {
					if(!hasErrors) {
						...
						siteManager.ckContent(draftremovalnotice);
					}
				});
						"
				><i class="mi-globe"></i> #esapiEncode('html',application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savedraftandpreview"))#</button>
			</cfif>
			<cfif assignChangesets>
				<button type="button" class="btn<cfif (hasChangesets and (not currentChangeset.getIsNew() or pendingChangesets.recordcount)) or len(rc.contentBean.getChangesetID())> mura-primary</cfif>"
				onclick="
					siteManager.validateAltUrls().then(function(hasErrors) {
						if(!hasErrors) {
							document.contentForm.approved.value=0;
							saveToChangeset('#rc.contentBean.getChangesetID()#','#esapiEncode('html',rc.siteID)#','');
						} 
					})"
				>
					<cfif requiresApproval>
			...
			<cfif rc.perm eq 'editor' and not $.siteConfig('EnforceChangesets')>
				<button type="button" class="btn<cfif not ((hasChangesets and (not currentChangeset.getIsNew() or pendingChangesets.recordcount)) or len(rc.contentBean.getChangesetID()))> mura-primary</cfif>"
				onclick="siteManager.validateAltUrls().then(function(hasErrors) {
						if(!hasErrors) {
...
<div id="dspStatusContainer">
	...
	<cfif hasChangesets and (not currentChangeset.getIsNew() or pendingChangesets.recordcount)>
		<div class="alert alert-info">
			<span>
				...
				<cfset loopCt = 0>				
				<cfloop query="pendingChangesets">
					<strong><a href="?muraAction=cArch.edit&moduleID=#esapiEncode('url',rc.moduleID)#&siteID=#esapiEncode('url',rc.siteID)#&topID=#esapiEncode('url',rc.topID)#&contentID=#esapiEncode('url',rc.contentID)#&return=#esapiEncode('url',rc.return)#&contentHistID=#pendingChangesets.contentHistID#&parentID=#esapiEncode('url',rc.parentID)#&startrow=#esapiEncode('url',rc.startrow)#&type=#esapiEncode('url',rc.type)#&compactDisplay=#esapiEncode('url',rc.compactDisplay)#">#esapiEncode('html',pendingChangesets.changesetName)#</a></strong>
					<cfset loopct += 1>
					<cfif loopct lt pendingChangesets.recordCount>,</cfif>
				</cfloop>
				</cfif>
				<cfif not currentChangeset.getIsNew()><br>#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.changesetversionnotify")#: <strong>#esapiEncode('html',currentChangeset.getName())#</strong></cfif>
			</span>
		</div>
	</cfif>
	...
	<div class="block block-constrain" id="mura-content-body-block">
		...
		<div class="block-content tab-content">
			...
			<div id="mura-content-title-render" data-placeholder="#esapiEncode('html_attr', rc.contentbean.getType())# #esapiEncode('html',application.rbFactory.getKeyValue(session.rb,"sitemanager.content.title"))#">#esapiEncode('html_attr',rc.contentBean.gettitle())#</div>

				<div id="mura-content-body-render" style="display:none;"><div class="mura-content-body-inner" <cfif listFindNoCase('Link,File,Variation',rc.contentBean.getType())> id="mura-content-body-inner"</cfif>>#bodyContent#</div></div>
...
//syncronized inputText

var isMenuTitleSynchronized = isUrlTitleSynchronized = isHtmlTitleSynchronized = false;
function initializeSynchronizedValues() {
	...
	var realTitle = this.realTitle;
	if(realTitle.value !== '') {
		isMenuTitleSynchronized = isUrlTitleSynchronized = isHtmlTitleSynchronized = false;
		return;
	}
	if(menuTitle && (menuTitle.value === realTitle.value || !menuTitle.value)) {
		this.isMenuTitleSynchronized = true;
	}
	if(urlTitle &&  (!urlTitle.value ||  validateHiddenUrlTitle() )) {
		this.isUrlTitleSynchronized = true;
	}
	if(htmlTitle && (htmlTitle.value === realTitle.value || !htmlTitle.value)){
		this.isHtmlTitleSynchronized = true;
	}
}

function writeAdditionalTitles(value) {
	var menuTitle      = document.getElementById('menuTitle');
	var htmlTitle      = document.getElementById('htmlTitle');
	if(isMenuTitleSynchronized) {
		menuTitle.value = value;
	}
	if(isUrlTitleSynchronized) {
		writeUrlTitle(value);
	}
	if(isHtmlTitleSynchronized) {
		htmlTitle.value = value;
	}
}

function writeUrlTitle(value) {
	var urlTitle   = document.getElementById('urlTitle');
	if(urlTitle){
		value = replaceToUrlFormat(value);
		urlTitle.value = value;
	}
}
function replaceToUrlFormat(value) {
	//remove html and trim string
	value = value.replace(/<[^>]*>?/gm, '').trim();
	
	//replace blank spaces with dashes ("-") and lower case string
	value = value.replace(/\s+/g, '-').toLowerCase();
	//remove all punctuation
	//the double pound in the regex is for escaping coldfusion pound
	value = value.replace(/[.,\/##@!$%"\^&<>\*;:?{}=\_`~()]/g,"");
	value = value.replace(/\s{2,}/g," ");
	//remove consecutive hyphens
	value = value.replace(/-{2,}/g, '-');
	//remove las chracter if it is an hyphen
...
//validate url title vs realtitle when onload
function validateHiddenUrlTitle() {
	var urlTitleHidden = document.getElementById('urlTitleHidden');
	var urlTitle      = document.getElementById('urlTitle');''

	if(urlTitleHidden && urlTitle){
		urlTitleHidden.value = replaceToUrlFormat(urlTitleHidden.value);	
		return urlTitleHidden.value.toLowerCase() === urlTitle.value.toLowerCase();
	} else  {
		return true;
	}
}
...
titleBlock.onclick = function(event){
	...
	if (titleBlock.className !== 'editing'){
		...
		holder.value = titleBlock.innerText;
		holder.maxLength = 255;
		...
		holder.onkeyup = function(event){
			...
			writeAdditionalTitles(newTitle);
		...
		holder.onblur = function(event){
			...
			//remove holder to avoid stacking too many one time used inputs
			holder.remove();
...
realTitle.onkeyup = function(event){
	titleBlock.innerText = realTitle.value;
	writeAdditionalTitles(realTitle.value);
	titleBlock.className = '';
}

realTitle.oninput  = function(event) {
	console.log('oninput ');
	document.getElementById('editAdditionalTitles').style.display = ""
}

if(menuTitle){
	menuTitle.onkeyup = function(event) {
		var value = event.target.value;
		if(value === realTitle.value || !value) {
			isMenuTitleSynchronized = true;
		}else {
			isMenuTitleSynchronized = false;
		}
	}
}

if(urlTitle){
	urlTitle.onkeyup = function(event) {
		var value = event.target.value;
		if(!value) {
			isUrlTitleSynchronized = true;
		}else {
			isUrlTitleSynchronized = false;
		}
...
if(htmlTitle){
	htmlTitle.onkeyup = function(event) {
		var value = event.target.value;
		if(value === realTitle.value || !value) {
			isHtmlTitleSynchronized = true;
		}else {
			isHtmlTitleSynchronized = false;
		}
...
$('.clicktocopy').prepend('<i class="mi-copy" title="#esapiEncode('html_attr',application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.clicktocopy'))#"></i>');

$('.clicktocopy').click(function(){
	var copiedicon = '<i class="mi-check" title="#esapiEncode('html_attr',application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.copytoclipboard'))#"></i>';
...
Mura(function(){
	Mura.startUserStateListener(60000);
})
</script>
