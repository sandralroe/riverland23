...
<cfhtmlhead text='<script type="text/javascript" src="assets/js/tab-view.js"></script>'>
 --->
		...
		<cfoutput>
			<div class="mura-actions">
				...
				<cfif rc.siteBean.getsiteid() eq ''>
					<button type="button" class="btn mura-primary" onclick="
						if(document.getElementById('alertMessage').parentElement.style.display !== 'none') {
							window.scrollTo(0, 0);
							bounce();
							return false;
						}
						submitForm(document.forms.form1,'add');"><i class="mi-plus-circle"></i> Add</button>
				<cfelse>
		...
		<cfif arrayLen(extendSets)>
			<cfset tabLabelList='Basic,Contact Info,Shared Resources,Admin Modules,Email,Images,Extranet,Display Regions,Extended Attributes,Deploy Bundle,Delegated Access'>
			<cfset tabList='tabBasic,tabContactinfo,tabSharedresources,tabModules,tabEmail,tabImages,tabExtranet,tabDisplayregions,tabExtendedAttributes,tabBundles,tabDelegatedAccess'>
		<cfelse>
			<cfset tabLabelList='Basic,Contact Info,Shared Resources,Modules,Email,Images,Extranet,Display Regions,Deploy Bundle,Delegated Access'>
			<cfset tabList='tabBasic,tabContactinfo,tabSharedresources,tabModules,tabEmail,tabImages,tabExtranet,tabDisplayregions,tabBundles,tabDelegatedAccess'>
		</cfif>
		...
		<div id="tabBasic" class="tab-pane active">
				<div class="block block-bordered">
					  ...
					  <!---
					  <p class="help-block" style="display:table-cell;background-color:red !important">uolo.</p>
					  --->
					  ...
					  <div class="block-content">
							<div class="mura-control-group">
								...
								<cfif rc.siteid eq ''>
									...
									<input onBlur="doesSiteAlreadyExists()" name="siteid" id="siteid" type="text" value="#rc.siteBean.getsiteid()#" size="25" maxlength="25" required="true" onchange="removePunctuation(this);">
								<cfelse>
							...
							<div class="mura-control-group">
								...
								<input onBlur="doesSiteAlreadyExists()"  name="site" id="site" type="text" value="#esapiEncode('html_attr',rc.siteBean.getsite())#" size="50" maxlength="50">
							</div>
					...
					<!--- Use SSL (Sitewide) --->
						...
						--->
							...
							<select name="enableLockdown">
									...
									<option value="archived"<cfif rc.siteBean.getEnableLockdown() eq "archived"> selected</cfif>>Archived</option>
								</select>
						</div>
		...
		<div id="tabModules" class="tab-pane">
			<div class="block block-bordered">
			  ...
			  <div class="block-content">
					...
					<cfif not rc.siteBean.getIsNew() and  not rc.siteBean.getContentRenderer().useLayoutManager()>
						<div class="mura-control-group">
				...
				<div class="mura-control-group">
					<label>Mura ORM Scaffolding (BETA)</label>
					<label class="radio inline">
		...
		<div id="tabEmail" class="tab-pane">
			...
			<div class="block block-bordered">
				<h2>#application.rbFactory.getKeyValue(session.rb,'siteconfig.email.contentscripts')#</h2>
				<div class="mura-control-group">
					<span></span>
					<p class="help-block">#application.rbFactory.getKeyValue(session.rb,'siteconfig.email.dynamiccontent')#</p>
				</div>
				<div class="mura-control-group">
   					<label>#application.rbFactory.getKeyValue(session.rb,'siteconfig.email.pendingscript')#</label>
					<p></p>
   					<textarea rows="6" name="contentPendingScript">#esapiEncode('html',rc.siteBean.getContentPendingScript())#</textarea>
	   			</div>

				<div class="mura-control-group">
					<label>#application.rbFactory.getKeyValue(session.rb,'siteconfig.email.approvalscript')#</label>
					<p></p>
					<textarea rows="6" name="contentApprovalScript">#esapiEncode('html',rc.siteBean.getContentApprovalScript())#</textarea>
				</div>

				<div class="mura-control-group">
					<label>#application.rbFactory.getKeyValue(session.rb,'siteconfig.email.rejectionscript')#</label>
					<p></p>
					<textarea rows="6" name="contentRejectionScript">#esapiEncode('html',rc.siteBean.getContentRejectionScript())#</textarea>
				</div>

				<div class="mura-control-group">
				<label>#application.rbFactory.getKeyValue(session.rb,'siteconfig.email.canceledscript')#</label>
					<p></p>
			...
			$("input[type='radio'][name='useDefaultSMTPServer']").each(function(){
				$(this).click(function() {
					...
					if( jQuery('input[name=useDefaultSMTPServer]:checked').val() == 1 ){
						jQuery('##testEmailSettings').attr('disabled', 'disabled').hide();
					} else {
						jQuery('##testEmailSettings').removeAttr('disabled').show();
					}
		...
		$(document).ready(function(){
			...
			<div class="mura-control-group">
				...
				<div class="mura-control justify">
					<cf_fileselector name="newPlaceholderImg" property="placeholderImgID" bean="#rc.siteBean#" deleteKey="deletePlaceholderImg" compactDisplay="#rc.compactDisplay#" locked="0" examplefileext="" maxUploadFileSize="#application.configBean.getValue('maxuploadfilesize')#" >
				</div>
			</div>
						...
						<cfset attributesArray=extendSetBean.getAttributes() />
							...
							<div class="mura-control-group">
									...
									<cfif attributeBean.getType() eq "File" and len(attributeValue) and attributeValue neq 'useMuraDefault'>
											<cfif listFindNoCase("png,jpg,jpeg,webp",application.serviceFactory.getBean("fileManager").readMeta(attributeValue).fileExt)>
											<a href="./index.cfm?muraAction=cArch.imagedetails&siteid=#rc.siteBean.getSiteID()#&fileid=#esapiEncode('url',attributeValue)#"><img id="assocImage" src="#application.configBean.getContext()##application.configBean.getIndexPath()#/_api/render/small/?fileid=#esapiEncode('url',attributeValue)#&cacheID=#createUUID()#" /></a>
										</cfif>
											<a href="#application.configBean.getContext()##application.configBean.getIndexPath()#/_api/render/file/?fileID=#esapiEncode('url',attributeValue)#" target="_blank">[Download]</a>
				...
				<div class="mura-control-group">
					...
					<div class="mura-control justify">
						<div class="mura-input-set">
							...
							<input type="button" value="Browse Server" class="mura-finder btn" data-serverpath="true" data-resourcetype="root" data-target="serverBundlePath"/>
						</div>
					</div>
					<cfif application.configBean.getPostBundles()>
						<p class="help-block">
	...
	<div id="tabDelegatedAccess" class="tab-pane">
		<div class="block block-bordered">
			<!-- block header -->
			<div class="block-header">
				<h3 class="block-title">Delegated Access</h3>
			</div>
			<div class="block-content">
				<div class="mura-control-group">
					<label>Allowed Domain:</label>
					<input name="alloweddomain" type="text" value="#esapiEncode('html_attr',rc.oauthSettingBean.getalloweddomain())#" size="255" maxlength="255">
				</div>

				<div class="mura-control-group">
					<label>Allowed Admin Domain:</label>
					<input name="allowedadmindomain" type="text" value="#esapiEncode('html_attr',rc.oauthSettingBean.getallowedadmindomain())#" size="255" maxlength="255">
				</div>

				<div class="mura-control-group">
					<label>Allowed Admin Group Email List:</label>
					<input name="allowedadmingroupemaillist" type="text" value="#esapiEncode('html_attr',rc.oauthSettingBean.getallowedadmingroupemaillist())#" size="255" maxlength="255">
				</div>

				<div class="mura-control-group">
					<label>Allowed Super User Email List:</label>
					<input name="alloweds2emaillist" type="text" value="#esapiEncode('html_attr',rc.oauthSettingBean.getalloweds2emaillist())#" size="255" maxlength="255">
				</div>
...
<script>
	function doesSiteAlreadyExists() {
		var siteIdInput   = document.getElementById('siteid');
		var siteNameInput = document.getElementById('site');
		var alertMessage  = document.getElementById('alertMessage');
		<cfoutput>
			var siteIdMessage   = '#esapiEncode("javascript",rc.$.rbKey('siteconfig.addsite.alreadyexists.siteid'))#';
			var siteNameMessage = '#esapiEncode("javascript",rc.$.rbKey('siteconfig.addsite.alreadyexists.sitename'))#';
		</cfoutput>
		if(siteIdInput === null) {
			return;
		}
		$.get('..<cfoutput>#application.configBean.getAdminDir()#</cfoutput>/?muraAction=cSettings.sitelist', function(data) {
			var displayMessage = false;
			var jsonData = JSON.parse(data.trim());
			if(jsonData.data.SITE.includes(siteNameInput.value)) {
				alertMessage.innerText = siteNameMessage;
				displayMessage = true;
			}
			if(jsonData.data.SITEID.includes(siteIdInput.value)) {
				alertMessage.innerText = siteIdMessage;
				displayMessage = true;
			}
			if(displayMessage) {
	...
	function bounce() {
		...
		var animation = setInterval(function() {
			if(appliedStyle) {
				appliedStyle = false;
				alertMessage.parentElement.style.marginBottom = '';
			}else {
				appliedStyle = true;
				alertMessage.parentElement.style.marginBottom = '1px';
			}
		}, 20);

		setTimeout(function(){ clearInterval(animation); }, 740);
