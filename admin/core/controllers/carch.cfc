component extends="controller" output="false" {
	...
	public string function importcomponent(struct rc="#StructNew()#") output=false {
		...
		if(structCount(form) && form.componentzip != '') {
			...
			if(!isJSON(componentJSON)) {
				rc.errormessage="Upload did not contain an exported Mura component";
				return;
			}
			...
			location("?muraAction=cArch.list&siteid=#esapiEncode('url',rc.siteid)#&topid=00000000000000000000000000000000003&moduleid=00000000000000000000000000000000003&activeTab=0",false);
		}
	...
	public function saveQuickEdit(rc) output=false {
		...
		if ( rc.$.validateCSRFTokens(context=rc.contentid & "quickedit") ) {
			...
			if ( arguments.rc.attribute == "isnav" ) {
				...
				// per schedule is not done viq quick edit
	...
	public function edit(rc) output=false {
		...
		if(local.currentBean.getIsNew() && (isValid('uuid',arguments.rc.templateid)|| rc.templateid=='00000000000000000000000000000000001')){
			...
			structDelete(templateData,'title');
			structDelete(templateData,'menutitle');
			...
			structDelete(templateData,'isnew');
		...
		if ( !arguments.rc.ajaxrequest ) {
			...
			if ( (arguments.rc.closeCompactDisplay != 'true'  || arguments.rc.murakeepediting) && arguments.rc.action != 'multiFileUpload' ) {
				...
				if ( structIsEmpty(arguments.rc.contentBean.getErrors()) ) {
					...
					if (!arguments.rc.murakeepediting || arguments.rc.preview) {
						if(arguments.rc.preview || isBoolean(arguments.rc.frontend) && arguments.rc.frontend && !listFindNoCase("Link,File",arguments.rc.contentBean.getType())){
							var queryString=(arguments.rc.preview)?"?previewid=#arguments.rc.contentBean.getContentHistID()#":"";
							if(arguments.rc.contentBean.getType()=='variation'){
								location(url="#arguments.rc.contentBean.getRemoteURL()##queryString#",addtoken=false);
							} else {
								location(url=arguments.rc.contentBean.getURL(complete=true,secure=application.settingsManager.getSite(arguments.rc.contentBean.getSiteID()).getUseSSL(),queryString=queryString,useEditRoute=true),addtoken=false);
							}
	...
	public function lockFile(rc) {
		if ( rc.$.validateCSRFTokens(context=rc.contentid & "lockfile") ) {
			...
			or listFindNoCase(session.mura.memberships,"s2") ) {
				...
				location(url="#variables.configBean.getContext()##application.configBean.getIndexPath()#/_api/render/file/?fileid=#local.contentBean.getFileID()#&method=attachment", addtoken=false);
			}
		}
		abort;
	}
