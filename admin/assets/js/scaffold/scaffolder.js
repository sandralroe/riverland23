Mura(function() {
	...
	var MuraORMScaffold = {
			...
			if(ident == 'new') {
				...
				.then(function(entity) {
					...
					entity.get('properties').then(function(properties){
						...
						data.parentproperties=properties.properties;
						...
						for(var i = 0;i < data.properties.length;i++) {
							...
							if(data.properties[i].datatype=='json' 
								&& data.model[data.properties[i].name]
								&& typeof data.model[data.properties[i].name] != 'string') {
								
								data.model[data.properties[i].name]=(data.model[data.properties[i].name]===null) ? '' : JSON.stringify( data.model[data.properties[i].name], null, '\t');
							}
						}
							...
							.then(
								function(properties) {
									...
									for(var i = 0;i < data.properties.length;i++) {
										...
										if(data.properties[i].datatype=='json'){
											if(data.model[data.properties[i].name] && typeof data.model[data.properties[i].name] != 'string'){
												data.model[data.properties[i].name]=(data.model[data.properties[i].name]===null) ? '' : JSON.stringify( data.model[data.properties[i].name], null, '\t');
											}	
										}
									}
		...
		save: function( listener,entityname,model ) {
			...
			.then(
				function( savedmodel ) {
					listener(savedmodel,savedmodel.hasErrors());
				},
				function( failed ) {
		...
		getRenderType: function( properties ) {
			if(properties.datatype == 'text')
				return "textarea";
			else if(properties.datatype == 'json'){
				return "codeeditor";
		...
		processProperties: function( data ) {
			...
			for(var x = 0;x < data.properties.length;x++) {
				...
				if(item.optionlist &&  item.optionlist.split){
					item.optionlist = item.optionlist.split('^');
				}

				if(item.optionvaluelist && item.optionvaluelist.split){
	...
	Vue.component('scaffold-related-many-one', {
		...
		mounted: function() {
			...
			MuraScaffold.feed( this.proplist,this.property.relatesto, 200 );
		},
		data: function(){
			return {
				data: {},
	...
	Vue.component('scaffold-field-codeeditor', {
		template: '#scaffold-field-codeeditor',
		props: ['property','model','entity','list'],
		mounted: function() {
			setCodeEditors();
		}
		...
		methods: {
			...
			setStoredState:function(state){
				if(typeof state=='object'){
					localStorage.setItem('mura_scaffolder',encodeURIComponent(JSON.stringify(state)));
				}
			},
			...
			getStoredState:function(){
				...
				try{
					var storedParams=localStorage.getItem('mura_scaffolder');
			...
			clickSave: function( entityname ) {
				...
				Mura("textarea.mura-code,textarea.codeEditor").forEach(function(){
					var input=Mura(this);
					if(codeInstances && typeof codeInstances[input.attr('name')]){
						if(input.length){
						...
						.then(
							function(properties) {
								...
								for(var i = 0;i < self.data.properties.length;i++) {
									...
									if(self.data.properties[i].datatype=='json' &&  self.data.model[self.data.properties[i].name] && typeof self.data.model[self.data.properties[i].name] != 'string') {
										self.data.model[self.data.properties[i].name]=JSON.stringify(self.data.model[self.data.properties[i].name]);
									}
