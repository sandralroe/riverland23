...
<body id="#rc.originalcircuit#" class="mura-admin header-navbar-fixed no-constrain #trim(rc.bodyclass)#">
	...
	<!-- Page Container -->
	<div id="page-container" class="<cfif len(session.siteid) and hasAdminAccess>sidebar-l</cfif> sidebar-mini sidebar-o side-overlay-hover side-scroll header-navbar-fixed">
		...
		<!-- Main Container -->
		<main id="main-container" class="<cfif len(session.siteid) and hasAdminAccess>block-constrain</cfif>">
		    ...
		    <div class="content">
		     	...
		     	<cfif request.action neq "core:cLogin.main" and isdefined('session.siteID')
					...
					<cfif rc.renderMuraAlerts>
						...
						<cfif not len(application.settingsManager.read(session.siteID).getEnableLockdown())
		...
		<cfif request.action neq "core:cLogin.main" and isDefined("session.siteid")>
			...
			$(window).on("load", function() {
				...
				if (localStorage.getItem("adminsidebar") == 'on') {
			...
			$(document).ready(function(){
		        ...
		        var frameParent = $('##mura-content-body-render');
			      	...
			      	resizeTabPane();
			 				localStorage.setItem("admincontrolwidth",acw);
					}
				});

				// persist side navigation expand/collapse
				if (localStorage.getItem("adminsidebar") === null) {
					 localStorage.setItem("adminsidebar", "off");
				}
				$('*[data-action=sidebar_mini_toggle]').click(function(){
					// adjust sidebar as needed
					resizeTabPane();
					// set adminsidebar cookie
					if(localStorage.getItem("adminsidebar") == 'off'){
						localStorage.setItem("adminsidebar", 'on');
						$('##page-container').removeClass('sidebar-mini');
					} else {
						localStorage.setItem("adminsidebar", 'off');
				...
				jQuery(sitefilter).keyup(function(){
					...
					if (str == ''){
						...
						jQuery(sitelist).find('li').each(function(i,item) {
							var label = (jQuery(item).find('a').text()).toLowerCase();
				...
				<!---  height handled separately for edit content view --->
				<cfif rc.originalcircuit eq "cArch" and rc.originalfuseaction eq "edit">
					...
					var setBlockHeight = function(){
						...
						var paddingH = 21;
						let alertsH = 0;
						let alertMsgs = $("##main-container > div.content > div.alert");
						if (alertMsgs.length == 1){
							let contentPadding = parseInt($('##main-container:first div.content:first').css('padding-top')) ;
							alertsH = contentPadding + alertMsgs.outerHeight(true);
						} else {
							alertMsgs.each(function() {
								alertsH += $(this).outerHeight();
							});
						}
						var windowH = $(window).height();
						var navbarH = $('##header-navbar:first').outerHeight();
						var headerH = $('.mura-header:first').outerHeight();
						var footerH = $('.mura-actions:first').outerHeight();
						...
						var subtr = alertsH + navbarH + headerH + footerH + paddingH;
						$(bc).last().css('height','calc(100vh - '+subtr+'px)');
					};
				...
				if (jQuery('##newContentMenu').length > 0){
				  if(!(jQuery(e.target).parents().hasClass('addNew')) && !(jQuery(e.target).parents().hasClass('add')) && !(jQuery(e.target).hasClass('add'))){
					 ...
					 if(!window.gridMode) {
						var item=document.getElementById('fileitem-'+window.index);
						if(item && item.parentElement && item.parentElement.parentElement){
